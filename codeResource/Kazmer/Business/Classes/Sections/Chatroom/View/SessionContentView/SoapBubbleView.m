...
@interface ButtonCoverView : UIButton
...
@implementation SoapBubbleView

//: - (instancetype)initSessionMessageContentView
- (instancetype)initTimeName
{
...
- (void)supersession:(NIMMessageModel *)data
{
    //: [super refresh:data];
    [super refresh:data];
...
- (UIImage *)location:(UIControlState)state noticeDisable:(BOOL)outgoing
{
    //: return nil;
    return nil;
}
...
- (void)manager:(NIMMessageModel *)data
{
...
- (void)cover:(NIMMessageModel *)data
{
...
- (void)user:(NIMKitRobotTemplateItem *)item
           //: inView:(UIView *)view
           display:(UIView *)view
{
    ...
    switch (item.itemType) {
        ...
        case NIMKitRobotTemplateItemTypeText:
        {
            ...
            QuickNameView *label = [self genLabel];
        ...
        case NIMKitRobotTemplateItemTypeLinkBlock:
        {
            ...
            ButtonCoverView *button = [self info];
...
- (CGSize)beforeName:(CGSize)size
{
    //: CGFloat height = [NTESSessionRobotContentView itemSpacing];
    CGFloat height = [SoapBubbleView itemSpacing];
    //: CGFloat width = 0;;
    CGFloat width = 0;;
    ...
    for (UIView *view in self.subviews) {
        ...
        if (view == self.continueButton)
        {
            ...
            height += [SoapBubbleView continueItemSpacing];
        }
        //: else
        else
        {
            //: height += [NTESSessionRobotContentView itemSpacing];
            height += [SoapBubbleView itemSpacing];
        }
    }
...
- (void)alongClearHoldfast:(ButtonCoverView *)button
{
    //: NIMKitEvent *event = [[NIMKitEvent alloc] init];
    NIMKitEvent *event = [[NIMKitEvent alloc] init];
    ...
    else
    {
        ...
        for (QuickNameView *label in button.subviews)
        {
            //: if ([label isKindOfClass:[M80AttributedLabel class]])
            if ([label isKindOfClass:[QuickNameView class]])
            {
...
- (void)completeRecord
{
    ...
    [self max:self picture:self.model.contentViewInsets];

    //: UIEdgeInsets contentInsets = self.model.contentViewInsets;
    UIEdgeInsets contentInsets = self.model.contentViewInsets;
    //: CGFloat top = [NTESSessionRobotContentView itemSpacing] + contentInsets.top;
    CGFloat top = [SoapBubbleView itemSpacing] + contentInsets.top;
    //: for (UIView *view in self.subviews)
    for (UIView *view in self.subviews)
    {
        //: if (view == self.bubbleImageView)
        ...
        top = view.bottom + [SoapBubbleView itemSpacing];
    }
}


//: - (void)resizeAllSubView:(UIView *)superView insets:(UIEdgeInsets)insets
- (void)max:(UIView *)superView picture:(UIEdgeInsets)insets
{
    ...
    for (UIView *subView in superView.subviews)
    {
        ...
        if (subView.height == 0)
        {
            ...
            if ([subView isKindOfClass:[QuickNameView class]])
            {
                //: M80AttributedLabel *label = (M80AttributedLabel *)subView;
                QuickNameView *label = (QuickNameView *)subView;
            ...
            else if ([subView isKindOfClass:[ButtonCoverView class]])
            {
                //: NTESSessionRobotButton *button = (NTESSessionRobotButton *)subView;
                ButtonCoverView *button = (ButtonCoverView *)subView;
                //: button.width = width;
                button.width = width;
...
- (void)item:(UIView *)view
{
    ...
    for (UIView *subView in view.subviews)
    {
        ...
        if ([subView isKindOfClass:[ButtonCoverView class]])
        {
            //: NTESSessionRobotButton *btn = (NTESSessionRobotButton *)subView;
            ButtonCoverView *btn = (ButtonCoverView *)subView;
            //: btn.target = nil;
            btn.target = nil;
            //: btn.url = nil;
            btn.url = nil;
...
- (QuickNameView *)meteor
{
    //: M80AttributedLabel *label = nil;
    QuickNameView *label = nil;
    ...
    else
    {
        ...
        label = [[QuickNameView alloc] initWithFrame:CGRectZero];
...
- (ButtonCoverView *)info
{
    //: NTESSessionRobotButton *button = nil;
    ButtonCoverView *button = nil;
    ...
    else
    {
        ...
        button = [[ButtonCoverView alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
...
@implementation ButtonCoverView
