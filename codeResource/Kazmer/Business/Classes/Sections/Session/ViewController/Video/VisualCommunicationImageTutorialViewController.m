...
NSString *StringFromMagnituderusionData(Byte *data);        


//: 00:00
Byte k_sendId[] = {18, 5, 20, 13, 155, 28, 155, 189, 128, 195, 72, 181, 235, 28, 28, 38, 28, 28, 199};
...
Byte main_markData[] = {83, 33, 22, 6, 80, 64, 81, 92, 89, 95, 90, 73, 83, 88, 80, 89, 73, 75, 77, 94, 83, 96, 83, 94, 99, 73, 95, 90, 78, 75, 94, 79, 73, 80, 75, 83, 86, 79, 78, 150};

//: ic_pic_save
Byte dreamEstablishmentTitle[] = {42, 11, 33, 12, 225, 181, 92, 156, 236, 233, 122, 173, 72, 66, 62, 79, 72, 66, 62, 82, 64, 85, 68, 25};

//: 1.5x
Byte notiDominatePath[] = {49, 4, 50, 12, 118, 11, 146, 3, 245, 129, 179, 43, 255, 252, 3, 70, 60};
...
Byte dream_survivalContent[] = {23, 32, 39, 11, 9, 97, 11, 19, 111, 170, 121, 63, 75, 66, 62, 71, 61, 56, 79, 62, 75, 66, 63, 82, 56, 58, 79, 77, 66, 79, 66, 77, 82, 56, 71, 62, 77, 56, 62, 75, 75, 72, 75, 80};

//: error
Byte k_envelopeUrl[] = {75, 5, 96, 6, 142, 140, 5, 18, 18, 15, 18, 97};

//: /  00:00
Byte main_successStr[] = {29, 8, 36, 8, 150, 33, 186, 180, 11, 252, 252, 12, 12, 22, 12, 12, 182};

//: 0.5x
Byte kUglyContent[] = {28, 4, 69, 4, 235, 233, 240, 51, 21};

//: player_push
Byte noti_distinctiveValue[] = {32, 11, 33, 4, 79, 75, 64, 88, 68, 81, 62, 79, 84, 82, 71, 157};

//: ic_pic_share
Byte appBidKey[] = {1, 12, 47, 7, 127, 224, 67, 58, 52, 48, 65, 58, 52, 48, 68, 57, 50, 67, 54, 40};

//: group_info_activity_update_success
Byte main_mediaMsg[] = {54, 34, 54, 12, 193, 214, 1, 210, 66, 112, 248, 249, 49, 60, 57, 63, 58, 41, 51, 56, 48, 57, 41, 43, 45, 62, 51, 64, 51, 62, 67, 41, 63, 58, 46, 43, 62, 47, 41, 61, 63, 45, 45, 47, 61, 61, 156};

//: 播放失败
Byte main_deliverNecessaryPath[] = {66, 12, 35, 9, 155, 232, 128, 127, 212, 195, 111, 138, 195, 113, 155, 194, 129, 142, 197, 145, 130, 110};

// __DEBUG__
// __CLOSE_PRINT__
//
//  VisualCommunicationImageTutorialViewController.m
//  NIM
//
...
@interface VisualCommunicationImageTutorialViewController ()<SubOldDelegate>
...
@property (nonatomic, strong) OptionView *videoSlider; //滑动条
...
@implementation VisualCommunicationImageTutorialViewController
{
    //: CGPoint _startPoint; 
...
- (void)viewDidLoad {
    ...
    [backButton setImage:[UIImage imageNamed:StringFromMagnituderusionData(dreamRepresentativeId)] forState:(UIControlStateNormal)];
    ...
    _btnShare.frame = CGRectMake([[UIScreen mainScreen] bounds].size.width-15-24, [UIDevice tingHeight]+10, 24, 24);
    //: [_btnShare setImage:[UIImage imageNamed:@"ic_pic_share"] forState:UIControlStateNormal];
    [_btnShare setImage:[UIImage imageNamed:StringFromMagnituderusionData(appBidKey)] forState:UIControlStateNormal];
    //: [_btnShare addTarget:self action:@selector(onTouchshare) forControlEvents:UIControlEventTouchUpInside];
    [_btnShare addTarget:self action:@selector(childComment) forControlEvents:UIControlEventTouchUpInside];
    ...
    [_btnSave setImage:[UIImage imageNamed:StringFromMagnituderusionData(dreamEstablishmentTitle)] forState:UIControlStateNormal];
    ...
    [_btnPlay setImage:[UIImage imageNamed:StringFromMagnituderusionData(app_stateValue)] forState:UIControlStateNormal];
    //: [_btnPlay setImage:[UIImage imageNamed:@"player_push"] forState:UIControlStateSelected];
    [_btnPlay setImage:[UIImage imageNamed:StringFromMagnituderusionData(noti_distinctiveValue)] forState:UIControlStateSelected];
    //: [_btnPlay addTarget:self action:@selector(onTap) forControlEvents:UIControlEventTouchUpInside];
    [_btnPlay addTarget:self action:@selector(cutText) forControlEvents:UIControlEventTouchUpInside];
...
//    self.progressSlider.thumbTintColor = ThemeColor;
    ...
    [self.progressSlider addTarget:self action:@selector(workflows:) forControlEvents:UIControlEventValueChanged];
...
- (void)keyDomain:(UIButton *)sender
{
    @
    ...
    .wDataSet(@[StringFromMagnituderusionData(kUglyContent),@"1x",StringFromMagnituderusionData(notiDominatePath),@"2x"])
    //: .wStart();
    .wStart();
}
...
- (void)ting:(void(^)(NSError *error))handler{
//    [SVProgressHUD show];
...
- (void)messageView:(NSInteger)totalTime
{
    //: self.totalTime = totalTime;
    self.totalTime = totalTime;
    //: self.totalLabel.text = [NSString stringWithFormat:@"/  %@",[self timeFormatted:totalTime]];
    self.totalLabel.text = [NSString stringWithFormat:@"/  %@",[self app:totalTime]];
}

//: - (void)videoPlayercurrentTime:(NSInteger)currentTime
- (void)areaPlayercurrent:(NSInteger)currentTime
{
    //: self.currentTime = currentTime;
    self.currentTime = currentTime;
    //: self.currentLabel.text = [self timeFormatted:currentTime];
    self.currentLabel.text = [self app:currentTime];
...
- (NSString *)app:(NSInteger)totalSeconds
{
...
- (void)backgroundTop{
    ...
    self.avPlayer.view.frame = CGRectMake(0, (44.0f + [UIDevice tingHeight]), [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice tingHeight])-(({ BOOL isPhoneX = NO; if (@available(iOS 11.0, *)) { isPhoneX = [[UIApplication sharedApplication] delegate].window.safeAreaInsets.bottom > 0.0; } isPhoneX;}) ? 49 + 35 : 49));
//    self.avPlayer.view.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             ...
                                             selector:@selector(silentsMessage:)
...
- (void)progressBy: (NSNotification *)aNotification
{
    //: if (self.avPlayer == aNotification.object)
    if (self.avPlayer == aNotification.object)
    {
        //: [self topStatusUIHidden:NO];
        [self keyForHidden:NO];
        ...
        NSNumber *resultValue = [notificationUserInfo objectForKey:user_pointId];
        ...
        if (reason == NTESAVMovieFinishReasonPlaybackError)
        {
            ...
            NSError *mediaPlayerError = [notificationUserInfo objectForKey:StringFromMagnituderusionData(k_envelopeUrl)];
            //: NSString *errorTip = [NSString stringWithFormat:@"%@: %@", @"播放失败".ntes_localized, [mediaPlayerError localizedDescription]];
            NSString *errorTip = [NSString stringWithFormat:@"%@: %@", StringFromMagnituderusionData(main_deliverNecessaryPath).textIndex, [mediaPlayerError localizedDescription]];
...
- (void)silentsMessage: (NSNotification *)aNotification
{
    //: if (self.avPlayer == aNotification.object)
    if (self.avPlayer == aNotification.object)
    {
...
- (void)keyForHidden:(BOOL)isHidden
{
...
- (RecentController *)avPlayer {
    ...
    if (!_avPlayer) {
        ...
        _avPlayer = [[RecentController alloc] initWithUser:[NSURL fileURLWithPath:self.item.path]];
...
//        _imgPlay.center = self.view.center;
        ...
        _imgPlay.image = [UIImage imageNamed:StringFromMagnituderusionData(noti_distinctiveValue)];
        //: _imgPlay.hidden = YES;
        _imgPlay.hidden = YES;
    }
    //: return _imgPlay;
...
- (UILabel *)currentLabel{
    ...
    if (!_currentLabel) {
        ...
        _currentLabel.text = StringFromMagnituderusionData(k_sendId);
...
- (UILabel *)totalLabel{
    ...
    if (!_totalLabel) {
        ...
        _totalLabel.text = StringFromMagnituderusionData(main_successStr);
...
- (OptionView *)videoSlider{
    //: if (!_videoSlider) {
    if (!_videoSlider) {
        //: _videoSlider = [[LYSlider alloc] initWithFrame:CGRectMake(0, 0, 3*([[UIScreen mainScreen] bounds].size.width/4), 20)];
        _videoSlider = [[OptionView alloc] initWithFrame:CGRectMake(0, 0, 3*([[UIScreen mainScreen] bounds].size.width/4), 20)];
...
//        UIImage *highlightImage = [UIImage creatImageWithView:highlightView];
        ...
        [_videoSlider key:normalImage domain:UIControlStateNormal];
...
- (void)eigenvalue:(OptionView *)slider{
...
- (void)terminateBottom:(OptionView *)slider{
...
//    [self startHideControlTimer];
    ...
    [self.avPlayer message: slider.value];
}
...
- (UIPanGestureRecognizer *)panGesture{
    ...
    if (!_panGesture) {
        ...
        _panGesture = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(tools:)];
    }
    //: return _panGesture;
    return _panGesture;
}
//: - (void)panGestureTouch:(UIPanGestureRecognizer *)panGestureTouch{
- (void)tools:(UIPanGestureRecognizer *)panGestureTouch{
...
//            }
            ...
            [self.avPlayer message:_fastCurrentTime];
        }
//        [self startHideControlTimer];
    }
}
...
Byte * MagnituderusionDataToCache(Byte *data) {
    int countersection = data[0];
    int changeSeize = data[1];
    Byte hero = data[2];
    int cow = data[3];
    if (!countersection) return data + cow;
    for (int i = cow; i < cow + changeSeize; i++) {
        int value = data[i] + hero;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[cow + changeSeize] = 0;
    return data + cow;
}

NSString *StringFromMagnituderusionData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)MagnituderusionDataToCache(data)];
}
