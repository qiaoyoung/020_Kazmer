...
NSString *StringFromFinePremiseData(Byte *data);


//: red_packet
Byte dreamUntilUrl[] = {92, 10, 27, 11, 106, 151, 160, 240, 238, 58, 211, 141, 128, 127, 122, 139, 124, 126, 134, 128, 143, 195};

//: change
Byte app_moderateStr[] = {6, 6, 90, 8, 76, 240, 111, 82, 189, 194, 187, 200, 193, 191, 79};

//: icon_redpackage_close
Byte k_shirtFormat[] = {40, 21, 66, 9, 12, 174, 148, 141, 125, 171, 165, 177, 176, 161, 180, 167, 166, 178, 163, 165, 173, 163, 169, 167, 161, 165, 174, 177, 181, 167, 120};

//: icon_redpackage_money
Byte userCapitalId[] = {24, 21, 58, 11, 159, 150, 93, 177, 118, 163, 41, 163, 157, 169, 168, 153, 172, 159, 158, 170, 155, 157, 165, 155, 161, 159, 153, 167, 169, 168, 159, 179, 94};

//: #D6D6D6
Byte dreamDrinkFormat[] = {88, 7, 39, 4, 74, 107, 93, 107, 93, 107, 93, 107};

//: activity_pay_psw_input
Byte noti_trainingKey[] = {41, 22, 31, 8, 98, 161, 160, 3, 128, 130, 147, 136, 149, 136, 147, 152, 126, 143, 128, 152, 126, 143, 146, 150, 126, 136, 141, 143, 148, 147, 108};

//: #C7E8CA
Byte m_duringId[] = {83, 7, 60, 12, 159, 170, 174, 201, 154, 134, 116, 125, 95, 127, 115, 129, 116, 127, 125, 23};

//: #333333
Byte appEvolveValue[] = {65, 7, 56, 9, 68, 94, 242, 111, 87, 91, 107, 107, 107, 107, 107, 107, 107};

//: #888888
Byte userPatrolWorkplaceFormat[] = {41, 7, 90, 5, 220, 125, 146, 146, 146, 146, 146, 146, 113};

//: amount
Byte main_barMsg[] = {8, 6, 19, 7, 83, 51, 125, 116, 128, 130, 136, 129, 135, 190};
...
-(instancetype)initWithNew:(CGRect)frame object:(NSDictionary *)dictionary{
    ...
    if (self) {
        ...
        [closeButton setImage:[UIImage imageNamed:StringFromFinePremiseData(k_shirtFormat)] forState:UIControlStateNormal];
        ...
        line.backgroundColor = [UIColor min:StringFromFinePremiseData(m_duringId)];
        ...
        amountLabel.text = [NSString stringWithFormat:@"¥%@",[dictionary record:StringFromFinePremiseData(main_barMsg)]];
        ...
        blackLine.backgroundColor = [UIColor min:StringFromFinePremiseData(dreamDrinkFormat)];
        ...
        iconImage.image = [UIImage imageNamed:StringFromFinePremiseData(userCapitalId)];
        ...
        lingq.textColor = [UIColor min:StringFromFinePremiseData(userPatrolWorkplaceFormat)];
        //: lingq.font = [UIFont systemFontOfSize:15];
        lingq.font = [UIFont systemFontOfSize:15];
        //: lingq.text = @"change";
        lingq.text = StringFromFinePremiseData(app_moderateStr);
        ...
        blackLine_2.backgroundColor = [UIColor min:StringFromFinePremiseData(dreamDrinkFormat)];
        ...
        SoleView *view1 = [[SoleView alloc] initWithFrame:rect];
...
- (void)towardStart{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        [self->_backView setModelSearchion:([[UIScreen mainScreen] bounds].size.height - (44.0f + [UIDevice tingHeight]) - (({UIEdgeInsets i; if(@available(iOS 11.0, *)) {i = [[[UIApplication sharedApplication] delegate] window].safeAreaInsets;} else {i = UIEdgeInsetsZero;} i;}).bottom))/2- 270/2];
    //: }];
    }];
}
...
Byte * FinePremiseDataToCache(Byte *data) {
    int threshTowards = data[0];
    int successEater = data[1];
    Byte sendRepair = data[2];
    int ligature = data[3];
    if (!threshTowards) return data + ligature;
    for (int i = ligature; i < ligature + successEater; i++) {
        int value = data[i] - sendRepair;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[ligature + successEater] = 0;
    return data + ligature;
}

NSString *StringFromFinePremiseData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)FinePremiseDataToCache(data)];
}
