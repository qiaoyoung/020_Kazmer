...
@property (nonatomic,strong) BubbleImage *object;

//: @end
@end

//: @implementation NTESSearchMessageContentCell
@implementation SaloonViewCell
...
- (void)can:(BubbleImage *)object{
    ...
    [self.avatar message:avatarURL state:info.avatarImage];
    //: self.titleLabel.text = info.showName;
    self.titleLabel.text = info.showName;
    //: self.contentLabel.text = message.text;
    self.contentLabel.text = message.text;
    //: self.timeLabel.text = [NTESSessionUtil showTime:message.timestamp showDetail:YES];
    self.timeLabel.text = [OldUtil time:message.timestamp phaseOfTheMoon:YES];
    ...
    self.contentLabel.size = [self.contentLabel sizeThatFits:CGSizeMake(noti_sizeStr * [UIScreen mainScreen].bounds.size.width / 320, 1.7976931348623157e+308)];
    ...
    self.contentLabel.height = ((app_halfUrl) > (self.contentLabel.height) ? (app_halfUrl) : (self.contentLabel.height));
