...
NSString *const show_inputId = @"online_state";
...
@interface RedCover()<NIMEventSubscribeManagerDelegate,NIMConversationManagerDelegate,NIMLoginManagerDelegate,NIMUserManagerDelegate>
...
- (void)gray:(NSString *)userId
{
    ...
    NIMSubscribeRequest *request = [self arrow];
...
- (void)reply
{
    ...
    NSDictionary *ext = @{
                            ...
                            showCellMessage : @([ValueTimeHeightDevice language].title),
                            //: NTESSubscribeOnlineState : @(NTESOnlineStateNormal), 
                            show_inputId : @(NTESOnlineStateNormal), //移动端永远在线
                          //: };
                          };
    //: [event setExt:[ext nimkit_jsonString]];
    [event setExt:[ext picture]];
...
- (void)side
{
    ...
    [_subscribeIds addObjectsFromArray:self.indexBar.allObjects];
    //: [_subscribeIds addObjectsFromArray:self.contactUserIds.allObjects];
    [_subscribeIds addObjectsFromArray:self.examineIds.allObjects];

    //: [self subscribeNextMembers:_subscribeIds.allObjects];
    [self sizeTo:_subscribeIds.allObjects];
}

//: - (void)subscribeNextMembers:(NSArray *)ids
- (void)sizeTo:(NSArray *)ids
{
    //: if (!ids.count) {
    if (!ids.count) {
        //: return;
...
- (void)onFriendChanged:(NIMUser *)user
{
    ...
    if (!isMyFriend && ![self.indexBar containsObject:user.userId]) {
              ...
              totalUnreadCount:(NSInteger)totalUnreadCount
{
    ...
    if (recentSession.session.sessionType == NIMSessionTypeP2P && ![self.examineIds containsObject:recentSession.session.sessionId]) {
