...
static CGFloat const k_attributeMsg = 0.1;
//: static CGFloat const MIN_PRESS_TO_BEGIN_EDITING_DURATION = 0.6;
static CGFloat const main_lengthUserKey = 0.6;

//: static inline CGPoint CGPointOffset(CGPoint point, CGFloat dx, CGFloat dy)
static inline CGPoint wordMargin(CGPoint point, CGFloat dx, CGFloat dy)
{
...
@interface RecentFlowLayout () <UIGestureRecognizerDelegate>

//: @property (nonatomic,readonly) id<LxGridViewDataSource> dataSource;
@property (nonatomic,readonly) id<NameObject> dataSource;
//: @property (nonatomic,readonly) id<LxGridViewDelegateFlowLayout> delegate;
@property (nonatomic,readonly) id<RedBubble> delegate;

//: @end
@end

//: @implementation LxGridViewFlowLayout
@implementation RecentFlowLayout
{
...
- (void)thrust
{
    ...
    _longPressGestureRecognizer = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(eightDayTriggerd:)];
    //: _longPressGestureRecognizer.cancelsTouchesInView = NO;
    _longPressGestureRecognizer.cancelsTouchesInView = NO;
    //: _longPressGestureRecognizer.minimumPressDuration = PRESS_TO_MOVE_MIN_DURATION;
    _longPressGestureRecognizer.minimumPressDuration = k_attributeMsg;
    ...
    _panGestureRecognizer = [[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(enabled:)];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(valueColor:) name:UIApplicationWillResignActiveNotification object:nil];
}
...
- (void)eightDayTriggerd:(UILongPressGestureRecognizer *)longPress
{
    ...
    switch (longPress.state) {
        ...
        case UIGestureRecognizerStateBegan:
        {
            ...
            if (_displayLink == nil) {
                ...
                _displayLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(tingShow:)];
                ...
                _remainSecondsToBeginEditing = main_lengthUserKey;
            }
            ...
            if ([self.delegate respondsToSelector:@selector(infoPath:view:find:)]) {
                //: [self.delegate collectionView:self.collectionView layout:self willBeginDraggingItemAtIndexPath:_movingItemIndexPath];
                [self.delegate infoPath:self.collectionView view:self find:_movingItemIndexPath];
            }
            ...
            EventReusableView *sourceCell = (EventReusableView *)sourceCollectionViewCell;
            ...
            UIView * highlightedSnapshotView = [sourceCell line];
            ...
            UIView * snapshotView = [sourceCell line];
            ...
            [UIView animateWithDuration:0
                             ...
                             completion:^(BOOL finished) {
                                 ...
                                 if (strongSelf) {
                                     ...
                                     if ([strongSelf.delegate respondsToSelector:@selector(loadRemove:dragging:line:)]) {
                                         ...
                                         [strongSelf.delegate loadRemove:strongSelf.collectionView dragging:strongSelf line:self->_movingItemIndexPath];
                                     }
                                 }
                             //: }];
                             }];
        ...
        case UIGestureRecognizerStateCancelled:
        {
            ...
            if (movingItemIndexPath) {
                ...
                if ([self.delegate respondsToSelector:@selector(scale:itemColor:middle:)]) {
                    //: [self.delegate collectionView:self.collectionView layout:self willEndDraggingItemAtIndexPath:movingItemIndexPath];
                    [self.delegate scale:self.collectionView itemColor:self middle:movingItemIndexPath];
                }
                ...
                [UIView animateWithDuration:0.2
                                 ...
                                 completion:^(BOOL finished) {
                                     ...
                                     if (strongSelf) {
                                         ...
                                         if ([strongSelf.delegate respondsToSelector:@selector(alongManager:player:ofTitle:)]) {
                                             ...
                                             [strongSelf.delegate alongManager:strongSelf.collectionView player:strongSelf ofTitle:movingItemIndexPath];
                                         }
                                     }
                                 //: }];
                                 }];
...
- (void)enabled:(UIPanGestureRecognizer *)pan
{
    //: switch (pan.state) {
    switch (pan.state) {
        //: case UIGestureRecognizerStatePossible:
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            _beingMovedPromptView.center = wordMargin(_sourceItemCollectionViewCellCenter, panTranslation.x, panTranslation.y);
            ...
            if ([self.dataSource respondsToSelector:@selector(data:keyIn:text:)] && [self.dataSource data:self.collectionView keyIn:sourceIndexPath text:destinationIndexPath] == NO) {
                //: return;
                return;
            }

            //: if ([self.dataSource respondsToSelector:@selector(collectionView:itemAtIndexPath:willMoveToIndexPath:)]) {
            if ([self.dataSource respondsToSelector:@selector(holder:largeness:user:)]) {
                ...
                [self.dataSource holder:self.collectionView largeness:sourceIndexPath user:destinationIndexPath];
            }
            ...
            //: } completion:^(BOOL finished) {
                ...
                if ([strongSelf.dataSource respondsToSelector:@selector(flipWith:cleanQuick:actionTo:)]) {
                    ...
                    [strongSelf.dataSource flipWith:strongSelf.collectionView cleanQuick:sourceIndexPath actionTo:destinationIndexPath];
                }
            //: }];
            }];
        }
...
- (void)tingShow:(CADisplayLink *)displayLink
{
...
- (void)valueColor:(NSNotification *)notificaiton
{
