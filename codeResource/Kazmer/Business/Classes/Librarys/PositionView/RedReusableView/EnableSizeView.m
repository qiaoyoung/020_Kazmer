...
- (void)modifyWith:(BOOL)overrideExistingFrame
{
    //: CGPoint center = self.center;
    CGPoint center = self.center;
    //: CGSize requiredSize = [self sizeForNumberOfPages:self.numberOfPages];
    CGSize requiredSize = [self net:self.numberOfPages];
...
- (CGSize)net:(NSInteger)pageCount
{
...
- (void)back
{
    ...
    for (NSInteger i = 0; i <= self.numberOfPages; i++) {
        ...
        [self largeDetail:dot disk:i];
    }

    //: [self changeIndicatorViewLayout:self.currentPage];
    [self menuDown:self.currentPage];
...
- (void)largeDetail:(UIView *)dot disk:(NSInteger)index
{
    // Dots are always centered within view
    //: CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self sizeForNumberOfPages:self.numberOfPages].width) / 2);
    CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self net:self.numberOfPages].width) / 2);
