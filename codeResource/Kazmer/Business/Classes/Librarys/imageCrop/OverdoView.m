...
static CGFloat const appStyleMessageKitFormat = 0.75;
//: static BOOL const square = NO;
static BOOL const mainSizeBottomFormat = NO;
//: float IMAGE_MIN_HEIGHT = 400;
float kLoadStr = 400;
//: float IMAGE_MIN_WIDTH = 400;
float dream_blueId = 400;

//: @interface ImageCropViewController(){
@interface LengthCropViewController(){
    //: CGRect _cropArea;
    CGRect _cropArea;
}
...
#pragma mark LengthCropViewController implementation
//: @implementation ImageCropViewController
@implementation LengthCropViewController
...
- (void)viewDidLoad
{
    ...
    if (self){
        ...
        self.cropView = [[OverdoView alloc] initWithStateEnd:view fasteningMax:self.blurredBackground];
...
- (void)videoed:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(filter:blue:)])
    {
        //: UIImage *cropped;
        UIImage *cropped;
        //: if (self.image != nil){
        ...
        [self.delegate filter:self blue:cropped];
    }
...
#pragma mark CountersectionView implementation

//: @implementation ControlPointView
@implementation CountersectionView
...
static CGFloat const show_borderValue = 5;
...
- (void)initOverSearchedEnable {
    ...
    controlPointSize = show_borderValue;
    ...
    topLeftPoint = [self color:resultRoot(centerInView.x - initialCropAreaSize,
    ...
    bottomLeftPoint = [self color:resultRoot(centerInView.x - initialCropAreaSize,
                                                                      //: centerInView.y + initialCropAreaSize,
                                                                      centerInView.y + initialCropAreaSize,
                                                                      //: controlPointSize)];
                                                                      controlPointSize)];
    ...
    bottomRightPoint = [self color:resultRoot(centerInView.x + initialCropAreaSize,
    ...
    topRightPoint = [self color:resultRoot(centerInView.x + initialCropAreaSize,
                                                                    //: centerInView.y - initialCropAreaSize, controlPointSize)];
                                                                    centerInView.y - initialCropAreaSize, controlPointSize)];

    //the "hole"
    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self teamKey];
    ...
    self.maskAlpha = appStyleMessageKitFormat;
...
- (CountersectionView*)color:(CGRect)frame {
    //: ControlPointView* point = [[ControlPointView alloc] initWithFrame:frame];
    CountersectionView* point = [[CountersectionView alloc] initWithFrame:frame];
    //: return point;
    return point;
}
...
- (void)top:(CGPoint)topLeft possibility:(CGPoint)bottomLeft text:(CGPoint)bottomRight fromPoint:(CGPoint)topRight {
...
- (UIView*)outside:(CGPoint)point {
    //: UIView* view = cropAreaView;
    UIView* view = cropAreaView;
    //: for (int i = 0; i < PointsArray.count; i++) {
    for (int i = 0; i < PointsArray.count; i++) {
        //: CGFloat x = [(ControlPointView *)PointsArray[i] center].x;
        CGFloat x = [(CountersectionView *)PointsArray[i] center].x;
        //: CGFloat y = [(ControlPointView *)PointsArray[i] center].y;
        CGFloat y = [(CountersectionView *)PointsArray[i] center].y;
...
- (void)handleDrag:(UIPanGestureRecognizer*)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan || multiDragPoint.lastCount != count) {
        ...
        if (count > 1)
            ...
            [self disable:recognizer panQuantity:count];
        //: else
        else
    ...
    if (count > 1) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            [self whole:[recognizer locationOfTouch:i inView:self] shadow:(i == 0 ? dragViewOne : dragViewTwo)];
    ...
    //: } else {
        ...
        [self whole:[recognizer locationInView:self] shadow:dragViewOne];
    }
...
- (void)disable:(UIPanGestureRecognizer*)recognizer panQuantity:(NSUInteger)count
{
    //: for (int i = 0; i < count; i++) {
    for (int i = 0; i < count; i++) {
        //: if (i == 0) {
        if (i == 0) {
            ...
            dragViewOne = [self outside:[recognizer locationOfTouch:i inView:self]];
        ...
        //: } else {
            ...
            dragViewTwo = [self outside:[recognizer locationOfTouch:i inView:self]];
...
- (void)create:(UIPanGestureRecognizer*)recognizer
{
    //: dragViewOne = [self checkHit:[recognizer locationInView:self]];
    dragViewOne = [self outside:[recognizer locationInView:self]];
...
- (void)setBubble:(CGRect)cropArea
{
...
- (CGSize)record:(CGPoint)dragLocation itemGray:(CGPoint)draggedPoint derive:(CGPoint)oppositePoint {
...
- (void)larboardLeft:(CGPoint)dragLocation {
    ...
    CGSize disp = [self record:dragLocation itemGray:dragPoint.topLeftCenter derive:dragPoint.bottomRightCenter];
    ...
    if (width >= dream_blueId && height < kLoadStr) {
        //: topLeft.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - IMAGE_MIN_HEIGHT) / self.imageScale);
        topLeft.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - kLoadStr) / self.imageScale);
        //: topRight.y = topLeft.y;
        topRight.y = topLeft.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < dream_blueId && height >= kLoadStr) {
        //: topLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - IMAGE_MIN_WIDTH) / self.imageScale);
        topLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - dream_blueId) / self.imageScale);
    ...
    [self top:topLeft possibility:bottomLeft text:dragPoint.bottomRightCenter fromPoint:topRight];
}
//: - (void)handleDragBottomLeft:(CGPoint)dragLocation {
- (void)bringHome:(CGPoint)dragLocation {
    ...
    CGSize disp = [self record:dragLocation itemGray:dragPoint.bottomLeftCenter derive:dragPoint.topRightCenter];
    ...
    if (width >= dream_blueId && height < kLoadStr) {
        ...
        bottomLeft.y = cropArea.origin.y + (kLoadStr / self.imageScale);
        //: bottomRight.y = bottomLeft.y;
        bottomRight.y = bottomLeft.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < dream_blueId && height >= kLoadStr) {
        //: bottomLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - IMAGE_MIN_WIDTH) / self.imageScale);
        bottomLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - dream_blueId) / self.imageScale);
    ...
    [self top:topLeft possibility:bottomLeft text:bottomRight fromPoint:dragPoint.topRightCenter];
}

//: - (void)handleDragBottomRight:(CGPoint)dragLocation {
- (void)imageIcon:(CGPoint)dragLocation {
    ...
    CGSize disp = [self record:dragLocation itemGray:dragPoint.bottomRightCenter derive:dragPoint.topLeftCenter];
    ...
    if (width >= dream_blueId && height < kLoadStr) {
        ...
        bottomRight.y = cropArea.origin.y + (kLoadStr / self.imageScale);
        //: bottomLeft.y = bottomRight.y;
        bottomLeft.y = bottomRight.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < dream_blueId && height >= kLoadStr) {
        //: bottomRight.x = cropArea.origin.x + (IMAGE_MIN_WIDTH / self.imageScale);
        bottomRight.x = cropArea.origin.x + (dream_blueId / self.imageScale);
    ...
    [self top:dragPoint.topLeftCenter possibility:bottomLeft text:bottomRight fromPoint:topRight];
}

//: - (void)handleDragTopRight:(CGPoint)dragLocation {
- (void)shared:(CGPoint)dragLocation {
    ...
    CGSize disp = [self record:dragLocation itemGray:dragPoint.topRightCenter derive:dragPoint.bottomLeftCenter];
    ...
    if (width >= dream_blueId && height < kLoadStr) {
        ...
        topRight.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - kLoadStr) / self.imageScale);
        //: topLeft.y = topRight.y;
        topLeft.y = topRight.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < dream_blueId && height >= kLoadStr) {
        //: topRight.x = cropArea.origin.x + (IMAGE_MIN_WIDTH / self.imageScale);
        topRight.x = cropArea.origin.x + (dream_blueId / self.imageScale);
    ...
    [self top:topLeft possibility:dragPoint.bottomLeftCenter text:bottomRight fromPoint:topRight];
}

//: - (void)handleDragCropArea:(CGPoint)dragLocation {
- (void)tap:(CGPoint)dragLocation {
    //: CGFloat dX = dragLocation.x - dragPoint.dragStart.x;
    CGFloat dX = dragLocation.x - dragPoint.dragStart.x;
    //: CGFloat dY = dragLocation.y - dragPoint.dragStart.y;
    CGFloat dY = dragLocation.y - dragPoint.dragStart.y;
...
- (void)setImage:(UIImage *)image {
    ...
    if (self.blurred) {
        ...
        blur = [image tool:30 highGround:1 construe_strong:[UIColor blackColor]];
    //: } else {
    } else {
        //: blur = [image blurredImageWithRadius:0 iterations:1 tintColor:[UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1.0]];
        blur = [image tool:0 highGround:1 construe_strong:[UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1.0]];
    }
    ...
    [self top:topLeftPoint.center possibility:bottomLeftPoint.center text:bottomRightPoint.center fromPoint:topRightPoint.center];
    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self teamKey];
    //: cropAreaView.frame = cropArea;
    cropAreaView.frame = cropArea;
...
@implementation UIImage (MaxOrientation)
