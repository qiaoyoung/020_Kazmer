...
// __M_A_C_R_O__
//: #import "SGScanView.h"
#import "MentalRepresentationView.h"
...
@interface MentalRepresentationView ()
//: @property (nonatomic, strong) SGScanViewConfigure *configure;
@property (nonatomic, strong) RepresentationOld *configure;
...
@implementation MentalRepresentationView

//: - (void)dealloc {
- (void)dealloc {
    //: if ([SGQRCodeLog sharedQRCodeLog].log) {
    if ([MessageLog bite].log) {
        //: NSLog(@"SGScanView - - dealloc");
    }
}

//: - (instancetype)initWithFrame:(CGRect)frame configure:(SGScanViewConfigure *)configure {
- (instancetype)initWithInputLight:(CGRect)frame convert:(RepresentationOld *)configure {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(stepRed)];
...
+ (instancetype)soapBubbleColor:(CGRect)frame messageWith:(RepresentationOld *)configure {
    //: return [[SGScanView alloc] initWithFrame:frame configure:configure];
    return [[MentalRepresentationView alloc] initWithInputLight:frame convert:configure];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    [self image:borderX recent:borderY session:cornerLength glazed:insideExcess excessUnder:outsideExcess];
    ...
    [self ergodic:borderX value:borderY maxFloat:borderH changeExcess:cornerLength key:insideExcess session:outsideExcess];
    ...
    [self my:borderX outdoorsDoing:borderY namesExcess:borderW rightBy:cornerLength userFloat:insideExcess display:outsideExcess];
    ...
    [self bestirSwitcheself:borderX advowsonTextOvermuchExcess:borderY show:borderW bar:borderH max:cornerLength boundary:insideExcess view:outsideExcess];
}
...
- (void)image:(CGFloat)borderX recent:(CGFloat)borderY session:(CGFloat)cornerLength glazed:(CGFloat) insideExcess excessUnder:(CGFloat)outsideExcess {
...
- (void)my:(CGFloat)borderX outdoorsDoing:(CGFloat)borderY namesExcess:(CGFloat)borderW rightBy:(CGFloat)cornerLength userFloat:(CGFloat) insideExcess display:(CGFloat)outsideExcess {
...
- (void)ergodic:(CGFloat)borderX value:(CGFloat)borderY maxFloat:(CGFloat)borderH changeExcess:(CGFloat)cornerLength key:(CGFloat) insideExcess session:(CGFloat)outsideExcess {
...
- (void)bestirSwitcheself:(CGFloat)borderX advowsonTextOvermuchExcess:(CGFloat)borderY show:(CGFloat)borderW bar:(CGFloat)borderH max:(CGFloat)cornerLength boundary:(CGFloat) insideExcess view:(CGFloat)outsideExcess {
...
- (void)language {
    ...
    if (self.link == nil) {
        ...
        self.link = [CADisplayLink displayLinkWithTarget:[InfoButtonProxy tip:self] selector:@selector(managerUi)];
