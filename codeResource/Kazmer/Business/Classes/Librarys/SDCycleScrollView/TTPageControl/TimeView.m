...
- (void)eyeglasses:(BOOL)overrideExistingFrame
{
    //: CGPoint center = self.center;
    CGPoint center = self.center;
    //: CGSize requiredSize = [self sizeForNumberOfPages:self.numberOfPages];
    CGSize requiredSize = [self individual:self.numberOfPages];
...
- (CGSize)individual:(NSInteger)pageCount
{
...
- (void)date
{
    ...
    for (NSInteger i = 0; i <= self.numberOfPages; i++) {
        ...
        //: } else {
            ...
            dot = [self weltanschauungIn];
        }

        //: [self updateDotFrame:dot atIndex:i];
        [self modify:dot messagePoint:i];
    }

    //: [self changeIndicatorViewLayout:self.currentPage];
    [self distance:self.currentPage];
...
- (void)modify:(UIView *)dot messagePoint:(NSInteger)index
{
    // Dots are always centered within view
    //: CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self sizeForNumberOfPages:self.numberOfPages].width) / 2);
    CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self individual:self.numberOfPages].width) / 2);
