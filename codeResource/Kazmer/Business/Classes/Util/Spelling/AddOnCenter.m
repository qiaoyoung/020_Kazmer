...
@interface AddOnCenter ()
//: - (SpellingUnit *)calcSpellingOfString: (NSString *)source;
- (IndexCrop *)quickViewTeam: (NSString *)source;
//: @end
@end


//: @implementation USERSpellingCenter
@implementation AddOnCenter
...
+ (AddOnCenter *)representation
{
    ...
    static AddOnCenter *instance = nil;
    //: static dispatch_once_t onceToken;
    static dispatch_once_t onceToken;
    //: _dispatch_once(&onceToken, ^{
    _dispatch_once(&onceToken, ^{
        //: instance = [[USERSpellingCenter alloc]init];
        instance = [[AddOnCenter alloc]init];
    //: });
    });
    //: return instance;
    return instance;
...
- (IndexCrop *)quickViewTeam:(NSString *)source
{
    ...
    for (NSInteger i = 0; i < [source length]; i++)
    {
        ...
        NSString *pinyin = [[MessageColor someone] doing:word];
    ...
    IndexCrop *unit = [[IndexCrop alloc]init];
...
- (IndexCrop *)formatTitle:(NSString *)source
{
    //: if ([source length] == 0)
    if ([source length] == 0)
    {
    ...
    IndexCrop *spellingUnit = nil;
    //: @synchronized(self)
    @synchronized(self)
    {
        //: SpellingUnit *unit = [_spellingCache objectForKey:source];
        IndexCrop *unit = [_spellingCache objectForKey:source];
        //: if (!unit)
        if (!unit)
        {
            //: unit = [self calcSpellingOfString:source];
            unit = [self quickViewTeam:source];
...
- (NSString *)filterLetter:(NSString *)input
{
    //: SpellingUnit *unit = [self spellingForString:input];
    IndexCrop *unit = [self formatTitle:input];
