...
#import "HolderCheck.h"

//: @implementation NTESCADisplayLinkHolder
@implementation HolderCheck
{
...
- (void)draw:(id<QuickTo>)delegate
{
    ...
    _displayLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(broadcasting:)];
...
- (void)broadcasting: (CADisplayLink *) displayLink
{
    //: if (_delegate && [_delegate respondsToSelector:@selector(onDisplayLinkFire:duration:displayLink:)]){
    if (_delegate && [_delegate respondsToSelector:@selector(imageLink:messageBegin:visible:)]){
        //: [_delegate onDisplayLinkFire:self
        [_delegate imageLink:self
                            //: duration:displayLink.duration
                            messageBegin:displayLink.duration
