...
@implementation UIImage (Turn)
//: - (UIImage *)drawImageWithSize: (CGSize)size
- (UIImage *)green: (CGSize)size
{
...
- (UIImage *)roundAdd
{
    ...
    CGFloat pixels = [[SizePage lastDevice] house];
    //: UIImage * image = [self imageForUpload:pixels];
    UIImage * image = [self should:pixels];
...
- (UIImage *)enumerate:(CGSize)newSize
{
...
- (UIImage *)color: (CGFloat)maxPixels
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self enumerate:CGSizeMake(newSizeWidth, newSizeHeight)];
}
...
- (UIImage *)should: (CGFloat)suggestPixels
{
        ...
        (width / height > kMaxRatio || height / width > kMaxRatio))
    {
        ...
        return [self color:kMaxPixels];
    }
    //: else
    else
    {
        //: return [self scaleWithMaxPixels:suggestPixels];
        return [self color:suggestPixels];
    }
}
