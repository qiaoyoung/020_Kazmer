...
#import "UIDevice+ShowAddition.h"

//: @implementation UIDevice (VGAddition)
@implementation UIDevice (ShowAddition)
...
+ (CGFloat)tingHeight {
    ...
    if (statusBarHeight <= 0) {
        ...
        CGFloat topPadding = [UIDevice notMedia];
...
+ (CGFloat)purview {
    ...
    return [UIDevice tingHeight] + [UIDevice fullness];
}

/// 底部导航栏高度
//: + (CGFloat)vg_tabBarHeight {
+ (CGFloat)containerVersion {
    //: return 49.0f;
    return 49.0f;
}
...
+ (CGFloat)tab {
    ...
    return [UIDevice tingHeight] + [UIDevice old];
}
