...
static char show_modelKey;


//: @implementation UIActionSheet (NTESBlock)
@implementation UIActionSheet (InputSub)


//: - (void)showInView: (UIView *)view completionHandler: (ActionSheetBlock)block
- (void)needGenerate: (UIView *)view popShould: (ActionSheetBlock)block
{
    //: self.delegate = self;
    self.delegate = self;
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,block,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&show_modelKey,block,OBJC_ASSOCIATION_COPY);
    ...
    else
    {
        ...
        else
        {
            ...
            [self performSelector:@selector(bied:)
                       //: withObject:view
                       withObject:view
                       //: afterDelay:1];
                       afterDelay:1];
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    ActionSheetBlock block = [objc_getAssociatedObject(self, &show_modelKey) copy];
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,nil,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&show_modelKey,nil,OBJC_ASSOCIATION_COPY);
