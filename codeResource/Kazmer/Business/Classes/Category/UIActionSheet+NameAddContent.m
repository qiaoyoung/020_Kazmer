...
// __CLOSE_PRINT__
//
//  UIActionSheet+NameAddContent.m
//  eim_iphone
//
...
static char kRangeFormat;


//: @implementation UIActionSheet (USERBlock)
@implementation UIActionSheet (NameAddContent)
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    ActionSheetBlock block = [objc_getAssociatedObject(self, &kRangeFormat) copy];
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,nil,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&kRangeFormat,nil,OBJC_ASSOCIATION_COPY);
...
- (void)maxExhibit: (UIView *)view boldDateDevice: (ActionSheetBlock)block
{
    ...
    objc_setAssociatedObject(self,&kRangeFormat,block,OBJC_ASSOCIATION_COPY);
    ...
    else
    {
        ...
        else
        {
            ...
            [self performSelector:@selector(bottoms:)
                       //: withObject:view
                       withObject:view
                       //: afterDelay:1];
                       afterDelay:1];
