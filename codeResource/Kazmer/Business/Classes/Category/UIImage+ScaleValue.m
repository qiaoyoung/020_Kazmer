...
@implementation UIImage (ScaleValue)
...
+ (void)load {
    ...
    _dispatch_once(&onceToken, ^{
        ...
        Method swizzledMethod = class_getClassMethod([UIImage class], @selector(imaginationImage:));
...
+ (UIImage *)imaginationImage:(NSString *)name {
    ...
    UIImage *image = [[PersonShould commonKey] imaginationImage:name];
    //: if (image) {
    if (image) {
        //: return image;
        return image;
    ...
    return [self imaginationImage:name]; // 这里会调用原始的 imageNamed: 方法，因为已经交换了实现
}
