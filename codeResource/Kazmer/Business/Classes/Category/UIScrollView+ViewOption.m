...
+ (Byte *)PhotographDataToCache:(Byte *)data {
    int skiEqually = data[0];
    Byte historyVeteran = data[1];
    int uglyFormat = data[2];
    for (int i = uglyFormat; i < uglyFormat + skiEqually; i++) {
        int value = data[i] - historyVeteran;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    }
    data[uglyFormat + skiEqually] = 0;
    return data + uglyFormat;
}

+ (NSString *)StringFromPhotographData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self PhotographDataToCache:data]];
}

//: 松开刷新...
+ (NSString *)notiHateTitle {
    /* static */ NSString *notiHateTitle = nil;
    if (!notiHateTitle) {
        Byte value[] = {15, 76, 12, 86, 63, 54, 52, 9, 120, 246, 38, 214, 50, 233, 10, 49, 8, 204, 49, 212, 3, 50, 226, 252, 122, 122, 122, 44};
        notiHateTitle = [self StringFromPhotographData:value];
    }
    return notiHateTitle;
}

//: 下拉加载...
+ (NSString *)noti_extraPath {
    /* static */ NSString *noti_extraPath = nil;
    if (!noti_extraPath) {
        Byte value[] = {15, 45, 8, 180, 5, 191, 163, 194, 17, 229, 184, 19, 184, 182, 18, 183, 205, 21, 234, 234, 91, 91, 91, 126};
        noti_extraPath = [self StringFromPhotographData:value];
    }
    return noti_extraPath;
}

//: 加载中...
+ (NSString *)m_shouldCountData {
    /* static */ NSString *m_shouldCountData = nil;
    if (!m_shouldCountData) {
        Byte value[] = {12, 67, 9, 120, 83, 181, 251, 184, 175, 40, 205, 227, 43, 0, 0, 39, 251, 240, 113, 113, 113, 183};
        m_shouldCountData = [self StringFromPhotographData:value];
    }
    return m_shouldCountData;
}

//: contentSize
+ (NSString *)m_hateName {
    /* static */ NSString *m_hateName = nil;
    if (!m_hateName) {
        Byte value[] = {11, 14, 13, 75, 206, 72, 137, 203, 137, 37, 198, 170, 99, 113, 125, 124, 130, 115, 124, 130, 97, 119, 136, 115, 237};
        m_hateName = [self StringFromPhotographData:value];
    }
    return m_hateName;
}

//: Pull to refresh...
+ (NSString *)dreamBookName {
    /* static */ NSString *dreamBookName = nil;
    if (!dreamBookName) {
        Byte value[] = {18, 58, 4, 120, 138, 175, 166, 166, 90, 174, 169, 90, 172, 159, 160, 172, 159, 173, 162, 104, 104, 104, 97};
        dreamBookName = [self StringFromPhotographData:value];
    }
    return dreamBookName;
}

//: frame
+ (NSString *)main_clothIdent {
    /* static */ NSString *main_clothIdent = nil;
    if (!main_clothIdent) {
        Byte value[] = {5, 84, 9, 243, 148, 62, 155, 198, 112, 186, 198, 181, 193, 185, 83};
        main_clothIdent = [self StringFromPhotographData:value];
    }
    return main_clothIdent;
}

//: contentOffset
+ (NSString *)userArmId {
    /* static */ NSString *userArmId = nil;
    if (!userArmId) {
        Byte value[] = {13, 97, 7, 186, 33, 16, 163, 196, 208, 207, 213, 198, 207, 213, 176, 199, 199, 212, 198, 213, 130};
        userArmId = [self StringFromPhotographData:value];
    }
    return userArmId;
}

//: SVPullToRefreshView
+ (NSString *)notiSeparationStoneDuringData {
    /* static */ NSString *notiSeparationStoneDuringData = nil;
    if (!notiSeparationStoneDuringData) {
        Byte value[] = {19, 54, 8, 174, 83, 60, 23, 66, 137, 140, 134, 171, 162, 162, 138, 165, 136, 155, 156, 168, 155, 169, 158, 140, 159, 155, 173, 185};
        notiSeparationStoneDuringData = [self StringFromPhotographData:value];
    }
    return notiSeparationStoneDuringData;
}
...
#import "UIScrollView+ViewOption.h"
//: #import <objc/runtime.h>
#import <objc/runtime.h>

//: static CGFloat const NTESPullToRefreshViewHeight = 60;
static CGFloat const user_backUrl = 60;
...
- (void)setSomeAssemblage:(UIEdgeInsets)insets;
//: - (void)rotateArrow:(float)degrees hide:(BOOL)hide;
- (void)info:(float)degrees valueSharedOff:(BOOL)hide;
...
#pragma mark - UIScrollView (ViewOption)


//: static char UIScrollViewPullToRefreshView;
static char dreamMessageUrl;

//: @implementation UIScrollView (NTESPullToRefresh)
@implementation UIScrollView (ViewOption)

//: @dynamic pullToRefreshView, showsPullToRefresh;
@dynamic pullToRefreshView, showsPullToRefresh;

//: - (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(NTESPullToRefreshPosition)position {
- (void)redCompletePosition:(void (^)(void))actionHandler title:(NTESPullToRefreshPosition)position {
    ...
    if(!self.pullToRefreshView) {
        ...
        PictureView *view = [[PictureView alloc] initWithFrame:CGRectMake(0, yOrigin, self.bounds.size.width, user_backUrl)];
...
- (void)distributer:(void (^)(void))actionHandler {
    //: [self addPullToRefreshWithActionHandler:actionHandler position:NTESPullToRefreshPositionTop];
    [self redCompletePosition:actionHandler title:NTESPullToRefreshPositionTop];
}
...
- (void)messageTrigger {
    ...
    [self.pullToRefreshView add];
}

//: - (void)setPullToRefreshView:(NTESPullToRefreshView *)pullToRefreshView {
- (void)setPullToRefreshView:(PictureView *)pullToRefreshView {
    //: [self willChangeValueForKey:@"SVPullToRefreshView"];
    [self willChangeValueForKey:[PhotographData notiSeparationStoneDuringData]];
    //: objc_setAssociatedObject(self, &UIScrollViewPullToRefreshView,
    objc_setAssociatedObject(self, &dreamMessageUrl,
                             //: pullToRefreshView,
                             pullToRefreshView,
                             //: OBJC_ASSOCIATION_ASSIGN);
                             OBJC_ASSOCIATION_ASSIGN);
    //: [self didChangeValueForKey:@"SVPullToRefreshView"];
    [self didChangeValueForKey:[PhotographData notiSeparationStoneDuringData]];
}

//: - (NTESPullToRefreshView *)pullToRefreshView {
- (PictureView *)pullToRefreshView {
    //: return objc_getAssociatedObject(self, &UIScrollViewPullToRefreshView);
    return objc_getAssociatedObject(self, &dreamMessageUrl);
}
...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh {
    ...
    if(!showsPullToRefresh) {
        ...
        if (self.pullToRefreshView.isObserving) {
            ...
            [self removeObserver:self.pullToRefreshView forKeyPath:[PhotographData userArmId]];
            //: [self removeObserver:self.pullToRefreshView forKeyPath:@"contentSize"];
            [self removeObserver:self.pullToRefreshView forKeyPath:[PhotographData m_hateName]];
            //: [self removeObserver:self.pullToRefreshView forKeyPath:@"frame"];
            [self removeObserver:self.pullToRefreshView forKeyPath:[PhotographData main_clothIdent]];
            //: [self.pullToRefreshView resetScrollViewContentInset];
            [self.pullToRefreshView user];
    ...
    //: else {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            [self addObserver:self.pullToRefreshView forKeyPath:[PhotographData userArmId] options:NSKeyValueObservingOptionNew context:nil];
            //: [self addObserver:self.pullToRefreshView forKeyPath:@"contentSize" options:NSKeyValueObservingOptionNew context:nil];
            [self addObserver:self.pullToRefreshView forKeyPath:[PhotographData m_hateName] options:NSKeyValueObservingOptionNew context:nil];
            //: [self addObserver:self.pullToRefreshView forKeyPath:@"frame" options:NSKeyValueObservingOptionNew context:nil];
            [self addObserver:self.pullToRefreshView forKeyPath:[PhotographData main_clothIdent] options:NSKeyValueObservingOptionNew context:nil];
            ...
            self.pullToRefreshView.frame = CGRectMake(0, yOrigin, self.bounds.size.width, user_backUrl);
        }
    }
}
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString([PhotographData noti_extraPath],),
                       //: NSLocalizedString(@"松开刷新...",),
                       NSLocalizedString([PhotographData notiHateTitle],),
                       //: NSLocalizedString(@"加载中...",),
                       NSLocalizedString([PhotographData m_shouldCountData],),
                       //: nil];
                       nil];
    ...
    if (self.superview && newSuperview == nil) {
        ...
        if (scrollView.showsPullToRefresh) {
            ...
            if (self.isObserving) {
                ...
                [scrollView removeObserver:self forKeyPath:[PhotographData userArmId]];
                //: [scrollView removeObserver:self forKeyPath:@"contentSize"];
                [scrollView removeObserver:self forKeyPath:[PhotographData m_hateName]];
                //: [scrollView removeObserver:self forKeyPath:@"frame"];
                [scrollView removeObserver:self forKeyPath:[PhotographData main_clothIdent]];
                //: self.isObserving = NO;
                self.isObserving = NO;
            }
        }
            ...
            case NTESPullToRefreshStateStopped:
                ...
                switch (self.position) {
                    ...
                    case NTESPullToRefreshPositionTop:
                        ...
                        [self info:0 valueSharedOff:NO];
                        //: break;
                        break;
                    //: case NTESPullToRefreshPositionBottom:
                    case NTESPullToRefreshPositionBottom:
                        //: [self rotateArrow:(float)3.14159265358979323846264338327950288 hide:NO];
                        [self info:(float)3.14159265358979323846264338327950288 valueSharedOff:NO];
                        //: break;
                        break;
                }
                //: break;
            ...
            case NTESPullToRefreshStateLoading:
                ...
                switch (self.position) {
                    ...
                    case NTESPullToRefreshPositionTop:
                        ...
                        [self info:0 valueSharedOff:YES];
                        //: break;
                        break;
                    //: case NTESPullToRefreshPositionBottom:
                    case NTESPullToRefreshPositionBottom:
                        //: [self rotateArrow:(float)3.14159265358979323846264338327950288 hide:YES];
                        [self info:(float)3.14159265358979323846264338327950288 valueSharedOff:YES];
                        //: break;
                        break;
                }
                //: break;
...
- (void)user {
    ...
    [self setSomeAssemblage:currentInsets];
}
...
- (void)setSomeAssemblage:(UIEdgeInsets)contentInset {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if([keyPath isEqualToString:[PhotographData userArmId]])
        //: [self scrollViewDidScroll:[[change valueForKey:NSKeyValueChangeNewKey] CGPointValue]];
        [self duringScale:[[change valueForKey:NSKeyValueChangeNewKey] CGPointValue]];
    //: else if([keyPath isEqualToString:@"contentSize"]) {
    else if([keyPath isEqualToString:[PhotographData m_hateName]]) {
        //: [self layoutSubviews];
        [self layoutSubviews];
        ...
        self.frame = CGRectMake(0, yOrigin, self.bounds.size.width, user_backUrl);
    }
    //: else if([keyPath isEqualToString:@"frame"])
    else if([keyPath isEqualToString:[PhotographData main_clothIdent]])
        //: [self layoutSubviews];
        [self layoutSubviews];
...
- (void)duringScale:(CGPoint)contentOffset {
...
- (PullView *)arrow {
    ...
    if(!_arrow) {
        ...
        _arrow = [[PullView alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, 22, 48)];
...
- (UILabel *)titleLabel {
    ...
    if(!_titleLabel) {
        ...
        _titleLabel.text = NSLocalizedString([PhotographData dreamBookName],);
...
- (void)move:(NSString *)title full:(NTESPullToRefreshState)state {
    //: if(!title)
    if(!title)
        //: title = @"";
        title = @"";
...
- (void)fileState:(NSString *)subtitle max:(NTESPullToRefreshState)state {
    //: if(!subtitle)
    if(!subtitle)
        //: subtitle = @"";
        subtitle = @"";
...
- (void)disable:(UIView *)view item:(NTESPullToRefreshState)state {
