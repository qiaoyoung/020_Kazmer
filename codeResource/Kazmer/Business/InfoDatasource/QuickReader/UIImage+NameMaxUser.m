...
@implementation UIImage (NameMaxUser)
...
+ (void)load {
    ...
    _dispatch_once(&onceToken, ^{
        ...
        Method swizzledMethod = class_getClassMethod([UIImage class], @selector(deciphers:));
...
+ (UIImage *)deciphers:(NSString *)name {
    ...
    UIImage *image = [[EmptyTing intervalegrated] deciphers:name];
    //: if (image) {
    if (image) {
        //: return image;
        return image;
    ...
    return [self deciphers:name]; // 这里会调用原始的 imageNamed: 方法，因为已经交换了实现
}
