...
- (instancetype) initWithTable:(NSArray *(^)(void))data{
    //: self = [super init];
    self = [super init];
    //: if (self) {
    if (self) {
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView{
    ...
    return self.temporaryWorker.count;
}

//: - (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    //: NIMCommonTableSection *tableSection = self.data[section];
    StatuteNameSection *tableSection = self.temporaryWorker[section];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    StatuteNameSection *tableSection = self.temporaryWorker[indexPath.section];
    //: NIMCommonTableRow *tableRow = tableSection.rows[indexPath.row];
    CommonRow *tableRow = tableSection.rows[indexPath.row];
    //: NSString *identity = tableRow.cellClassName.length ? tableRow.cellClassName : DefaultTableCell;
    NSString *identity = tableRow.cellClassName.length ? tableRow.cellClassName : noti_touchPath;
    ...
    if (![cell respondsToSelector:@selector(light:manager:)]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!tableRow.forbidSelect) {
        ...
        UIViewController *vc = tableView.checked;
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //: }else{
        ...
        StatuteNameSection *section = self.temporaryWorker[indexPath.section];
...
- (void)language:(CommonRow *)rowData straddle:(UITableViewCell *)cell{
