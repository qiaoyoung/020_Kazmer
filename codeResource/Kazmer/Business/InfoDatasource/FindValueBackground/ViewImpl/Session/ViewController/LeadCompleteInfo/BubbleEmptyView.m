...
@interface BubbleEmptyView () <DigitizerOld>
...
@property (nonatomic, strong) NSMutableArray<ValueSubView*> *items;
...
@property (nonatomic, strong) TouchButtonView *touchReceiver;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self = [super initWithCoder:aDecoder]) {
        ...
        _timeIntervalPerScroll = m_objectKey;
        //: _timeDurationPerScroll = DEFAULT_TIME_DURATION;
        _timeDurationPerScroll = dream_pathFormat;
        //: _scrollSpeed = DEFAULT_SCROLL_SPEED;
        _scrollSpeed = dreamResId;
        //: _itemSpacing = DEFAULT_ITEM_SPACING;
        _itemSpacing = showControlMessage;
        //: _touchEnabled = NO;
        _touchEnabled = NO;
...
- (void)cipherred:(NSNotification*)notification {
...
- (void)making:(NSNotification*)notification {
    //: if (!_isPausingBeforeResignActive) {
    if (!_isPausingBeforeResignActive) {
        //: [self start];
        [self detectShouldState];
    }
}
...
- (void)scale {
    ...
    //: } else {
        ...
        if ([_delegate respondsToSelector:@selector(nameView:)]) {
            //: self.visibleItemCount = [_delegate numberOfVisibleItemsForMarqueeView:self];
            self.visibleItemCount = [_delegate nameView:self];
        ...
        //: } else {
            ...
            self.visibleItemCount = mEventIdent;
        }
    }
    ...
    for (int i = 0; i < _visibleItemCount + 2; i++) {
        ...
        ValueSubView *itemView = [[ValueSubView alloc] init];
    ...
    if (_direction == UUMarqueeViewDirectionLeftward) {
        ...
        for (int i = 0; i < _items.count; i++) {
            ...
            //: } else {
                ...
                _items[index].width = [self sub:_items[index].tag];
                ...
                [self system:_items[index] headwater:_items[index].tag];
            }
        }
    //: } else {
    } else {
        ...
        if (_useDynamicHeight) {
            ...
            for (int i = 0; i < _items.count; i++) {
                ...
                if (i == _items.count - 1) {
                    ...
                    _items[index].height = [self listChangeMarginOfSafety:_items[index].tag];
        ...
        //: } else {
            ...
            if ([_delegate respondsToSelector:@selector(covering:)]) {
                //: dataCount = [_delegate numberOfDataForMarqueeView:self];
                dataCount = [_delegate covering:self];
            }
...
- (void)system:(ValueSubView*)itemView headwater:(NSInteger)index {
    //: if (index < 0) {
    if (index < 0) {
        //: [itemView clear];
        [itemView valueClear];
    ...
    if (index >= 0) {
        ...
        if ([_delegate respondsToSelector:@selector(unwanted:stratumWeltanschauungFrom:success:)]) {
            //: [_delegate updateItemView:itemView atIndex:index forMarqueeView:self];
            [_delegate unwanted:itemView stratumWeltanschauungFrom:index success:self];
        }
    }
}
...
- (void)timerInfo:(NSTimer *)timer {
    ...
    dispatch_async(dispatch_get_main_queue(), ^() {
        ...
        if (_direction == UUMarqueeViewDirectionLeftward) {
            ...
            _items[_firstItemIndex].width = [self sub:_items[_firstItemIndex].tag];
            ...
            if (firstItemWidth != nextItemWidth) {
                ...
                [_items[_firstItemIndex] valueClear];
            }
            //: [self updateItemView:_items[_firstItemIndex] atIndex:_items[_firstItemIndex].tag];
            [self system:_items[_firstItemIndex] headwater:_items[_firstItemIndex].tag];
        ...
        //: } else {
            ...
            if (_useDynamicHeight) {
                ...
                _items[_firstItemIndex].height = [self listChangeMarginOfSafety:_items[_firstItemIndex].tag];
...
- (void)tipItem {
    ...
    if (_touchEnabled) {
        ...
        if (!_touchReceiver) {
            ...
            self.touchReceiver = [[TouchButtonView alloc] init];
...
- (void)undersurface:(CGPoint)point {
    ...
    for (ValueSubView *itemView in _items) {
        ...
        if ([itemView.layer.presentationLayer hitTest:point]) {
            ...
            if (dataCount > 0 && itemView.tag >= 0 && itemView.tag < dataCount) {
                ...
                if ([self.delegate respondsToSelector:@selector(should:terminatedLikeCuculiformBirdPalpebraLight:)]) {
                    //: [self.delegate didTouchItemViewAtIndex:itemView.tag forMarqueeView:self];
                    [self.delegate should:itemView.tag terminatedLikeCuculiformBirdPalpebraLight:self];
                }
            }
            //: break;
            break;
...
@implementation TouchButtonView
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (_touchDelegate) {
        ...
        [_touchDelegate undersurface:touchLocation];
    }
}
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (_touchDelegate) {
        ...
        [_touchDelegate headingContainer];
    }
}
