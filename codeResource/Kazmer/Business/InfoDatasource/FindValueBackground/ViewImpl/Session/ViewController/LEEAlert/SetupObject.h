 ...
 *************************简要说明************************
 ...
 [SetupObject configMainWindow:self.window];
 
 
 NoticeOld 使用方法
 
 [SetupObject alert].cofing.XXXXX.XXXXX.LeeShow();
 
 ReplacementTo 使用方法
 
 [SetupObject actionSheet].cofing.XXXXX.XXXXX.LeeShow();
...
@interface SetupObject: NSObject

/** 初始化 */

//: + (nonnull LEEAlertConfig *)alert;
+ (nonnull ViewPathBaseConfig *)range;

//: + (nonnull LEEActionSheetConfig *)actionsheet;
+ (nonnull UserMaxBaseConfig *)bubble;

/** 获取Alert窗口 */
//: + (nonnull LEEAlertWindow *)getAlertWindow;
+ (nonnull TingImageView *)to;
...
+ (void)value:(NSString *)identifier read:(BOOL)force toss:(void (^ _Nullable)(void))completionBlock;
...
+ (void)briquetWith:(void (^ _Nullable)(void))completionBlock;

//: @end
@end

//: @interface LEEBaseConfigModel: NSObject
@interface ImageBar: NSObject
...
/** 设置 添加自定义视图 -> 格式: .LeeAddCustomView(^(ViewUsageTing *){ //code.. }) */
//: @property (nonatomic, copy, readonly) LEEConfigToCustomView LeeAddCustomView;
@property (nonatomic, copy, readonly) LEEConfigToCustomView LeeAddCustomView;

/** 设置 添加一项 -> 格式: .LeeAddItem(^(ShowSizeItem *){ //code.. }) */
//: @property (nonatomic, copy, readonly) LEEConfigToItem LeeAddItem;
@property (nonatomic, copy, readonly) LEEConfigToItem LeeAddItem;

/** 设置 添加动作 -> 格式: .LeeAddAction(^(ModelButtonAction *){ //code.. }) */
...
@interface ImageBar(Presentation)

/** 设置 层级 -> 格式: .LeePresentation([WorkflowClose windowLevel:1000] / [WorkflowClose viewController:xxxx]) */
...
@interface ImageBar(ReplacementTo)
...
@interface ShowSizeItem: NSObject
...
@interface ModelButtonAction: NSObject
...
@interface ViewUsageTing: NSObject
...
@interface WorkflowClose: NSObject

//: + (LEEPresentation *)windowLevel: (UIWindowLevel)level;
+ (WorkflowClose *)windowGlass: (UIWindowLevel)level;

//: + (LEEPresentation *)viewController: (UIViewController *)controller;
+ (WorkflowClose *)untilControl: (UIViewController *)controller;

//: @end
@end

//: @interface LEEBaseConfig: NSObject
@interface InfoCheckConfig: NSObject

//: @property (nonatomic, strong, nonnull) LEEBaseConfigModel *config;
@property (nonatomic, strong, nonnull) ImageBar *config;

//: @end
@end

//: @interface LEEAlertConfig: LEEBaseConfig
@interface ViewPathBaseConfig: InfoCheckConfig

//: @end
@end

//: @interface LEEActionSheetConfig: LEEBaseConfig
@interface UserMaxBaseConfig: InfoCheckConfig

//: @end
@end

//: @interface LEEAlertWindow: UIWindow @end
@interface TingImageView: UIWindow @end

//: @interface LEEBaseViewController: UIViewController @end
@interface PinViewController: UIViewController @end

//: @interface LEEAlertViewController: LEEBaseViewController @end
@interface NoticeFindViewController: PinViewController @end

//: @interface LEEActionSheetViewController: LEEBaseViewController @end
@interface EdgeViewController: PinViewController @end


//: @interface UIView (CornerRadii)
@interface UIView (AreaRadii)
...
AreaRadii viewInvite(void);

//: CornerRadii CornerRadiiNull(void);
AreaRadii makeTable(void);
