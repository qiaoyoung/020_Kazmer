...
NSString *StringFromNineData(Byte *data);


//: gif
Byte notiStyleMessage[] = {40, 3, 5, 98, 19, 102, 105, 103, 214};

//: png
Byte dream_atMsg[] = {3, 3, 8, 253, 159, 52, 128, 164, 103, 110, 112, 76};

//: tiff
Byte appGuidanceAllIdent[] = {12, 4, 3, 102, 102, 105, 116, 151};

//: jpeg
Byte notiSurgeryElsewhereStr[] = {83, 4, 7, 131, 157, 146, 97, 103, 101, 112, 106, 60};

//: yyyy-MM-dd HH:mm:ss
Byte appScribeKey[] = {21, 19, 13, 38, 189, 9, 179, 209, 93, 204, 40, 34, 108, 115, 115, 58, 109, 109, 58, 72, 72, 32, 100, 100, 45, 77, 77, 45, 121, 121, 121, 121, 184};
...
@implementation UIImage (Page)

//: - (UIImage *)flip:(BOOL)isHorizontal {
- (UIImage *)curriculum:(BOOL)isHorizontal {
...
- (UIImage *)scale:(CGFloat)width {
    ...
    return [self safetyMargin:width nearDate:height];
}
...
- (UIImage *)textHeight:(CGFloat)heigh {
    ...
    return [self safetyMargin:width nearDate:heigh];
}
...
- (UIImage *)nameData {
    ...
    [df setDateFormat:StringFromNineData(appScribeKey)];
    ...
    return [self value:dateString
               //: textColor:[UIColor blackColor]
               add_strong:[UIColor blackColor]
                    //: font:[UIFont boldSystemFontOfSize:16.0f]
                    quantity:[UIFont boldSystemFontOfSize:16.0f]
                   //: point:CGPointMake(self.size.width-size.width-10, self.size.height-size.height-10)];
                   of:CGPointMake(self.size.width-size.width-10, self.size.height-size.height-10)];
...
+ (NSString *) endSize:(NSData *)data{
    ...
    switch (c) {
        ...
        case 0xFF:
            //: return @"jpeg";
            return StringFromNineData(notiSurgeryElsewhereStr);
        //: case 0x89:
        case 0x89:
            //: return @"png";
            return StringFromNineData(dream_atMsg);
        //: case 0x47:
        case 0x47:
            //: return @"gif";
            return StringFromNineData(notiStyleMessage);
        //: case 0x49:
        case 0x49:
        //: case 0x4D:
        case 0x4D:
            //: return @"tiff";
            return StringFromNineData(appGuidanceAllIdent);
    }
    //: return @"";
    return @"";
}
...
CGFloat photoRadians(CGFloat degrees) {return degrees * 3.14159265358979323846264338327950288 / 180;};
//: CGFloat KKRadiansToDegrees(CGFloat radians) {return radians * 180/3.14159265358979323846264338327950288;};
CGFloat recentByDegrees(CGFloat radians) {return radians * 180/3.14159265358979323846264338327950288;};
...
- (UIImage *)untilWith:(CGFloat)degrees{
    ...
    CGAffineTransform t = CGAffineTransformMakeRotation(photoRadians(degrees));
    ...
    CGContextRotateCTM(bitmap, photoRadians(degrees));
...
- (UIImage*)doing:(CGSize)targetSize
{
    //: UIImage *sourceImage = self;
    UIImage *sourceImage = self;
    //: UIImage *newImage = nil;
...
- (UIImage *)checked:(NSUInteger)maxLength{
    ...
    NSData *compressData = [self centerWith:maxLength*1024];
...
-(NSData *)centerWith:(double)maxLength{
    // Compress by quality
...
Byte * NineDataToCache(Byte *data) {
    int have = data[0];
    int pipeCombination = data[1];
    int concede = data[2];
    if (!have) return data + concede;
    for (int i = 0; i < pipeCombination / 2; i++) {
        int begin = concede + i;
        int end = concede + pipeCombination - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[0] = 0;
    data[concede + pipeCombination] = 0;
    return data + concede;
}

NSString *StringFromNineData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)NineDataToCache(data)];
}  
