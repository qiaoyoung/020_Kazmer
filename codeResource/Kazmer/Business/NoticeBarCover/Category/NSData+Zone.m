...
void errorFrame(const char *key, int keylen, char* output, const char* input, int len)
{
    // 设置密钥
    //: char key_[256];
    char key_[256];
...
- (NSData *)status:(NSString *)key {
    ...
    errorFrame(pwd, (int)key.length, outdata, self.bytes, (int)self.length);
