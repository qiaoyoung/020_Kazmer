...
- (UIImage *)quick
{
    ...
    CGFloat pixels = [[ValueTimeHeightDevice language] bubble];
    //: UIImage * image = [self imageForUpload:pixels];
    UIImage * image = [self referenceBook:pixels];
...
- (UIImage *)referenceBook: (CGFloat)suggestPixels
{
        ...
        (width / height > kMaxRatio || height / width > kMaxRatio))
    {
        ...
        return [self messageMode:kMaxPixels];
    }
    //: else
    else
    {
        //: return [self scaleWithMaxPixels:suggestPixels];
        return [self messageMode:suggestPixels];
    }
}

//: - (UIImage *)scaleWithMaxPixels: (CGFloat)maxPixels
- (UIImage *)messageMode: (CGFloat)maxPixels
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self colorRecording:CGSizeMake(newSizeWidth, newSizeHeight)];
}

//内缩放，一条变等于最长边，另外一条小于等于最长边
//: - (UIImage *)scaleToSize:(CGSize)newSize
- (UIImage *)colorRecording:(CGSize)newSize
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
