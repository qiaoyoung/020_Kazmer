...
@interface BubbleContentScrollView : UIScrollView

//: @end
@end

//: @interface NTESPageView ()
@interface LootedView ()
{
...
- (void)layoutSubviews
{
    ...
    if (self.pageViewDelegate && [self.pageViewDelegate respondsToSelector:@selector(videoTime)])
    {
        //: animation = [self.pageViewDelegate needScrollAnimation];
        animation = [self.pageViewDelegate videoTime];
    }
...
- (void)ofMessage
{
    ...
    if (_scrollView == nil)
    {
        ...
        _scrollView = [[BubbleContentScrollView alloc]initWithFrame:self.bounds];
...
- (NSInteger)alongSecMobile:(NSInteger)value add:(NSInteger)min liquidEcstasyView:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
...
- (void)information:(NSInteger)currentPage
{
    ...
    NSInteger first = [self alongSecMobile:currentPage - 1 add:0 liquidEcstasyView:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self alongSecMobile:currentPage + 1 add:0 liquidEcstasyView:count - 1];
    ...
    for (NSUInteger index = 0; index < count; index++)
    {
        ...
        if (NSLocationInRange(index, range))
        {
            ...
            if (![obj isKindOfClass:[UIView class]])
            {
                ...
                if (_dataSource && [_dataSource respondsToSelector:@selector(colorPage:bubble:)])
                {
                    //: UIView *view = [_dataSource pageView:self viewInPage:index];
                    UIView *view = [_dataSource colorPage:self bubble:index];
...
- (void)select
{
    ...
    if(_dataSource && [_dataSource respondsToSelector:@selector(mImage:)])
    {
        //: numberOfPages = [_dataSource numberOfPages:self];
        numberOfPages = [_dataSource mImage:self];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(valueView:)])
    {
        //: [_pageViewDelegate pageViewDidScroll:self];
        [_pageViewDelegate valueView:self];
    }
}
...
- (void)back
{
    ...
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(clean:endWith:end:)])
    {
        //: [_pageViewDelegate pageViewScrollEnd:self
        [_pageViewDelegate clean:self
                                //: currentIndex:_currentPage
                                endWith:_currentPage
...
@implementation BubbleContentScrollView
//fix: swipe to delete in a UITableView which is embeded in a UIScrollView
//: - (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRequireFailureOfGestureRecognizer:(nonnull UIGestureRecognizer *)otherGestureRecognizer
- (BOOL)adpSystem:(UIGestureRecognizer *)gestureRecognizer pressMax:(nonnull UIGestureRecognizer *)otherGestureRecognizer
{
