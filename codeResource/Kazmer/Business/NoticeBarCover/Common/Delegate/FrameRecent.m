...
NSString *StringFromChampionData(Byte *data);


//: UITabBarButton
Byte appThoroughlyMessage[] = {83, 14, 4, 157, 110, 111, 116, 116, 117, 66, 114, 97, 66, 98, 97, 84, 73, 85, 2};
...
@implementation FrameRecent
//: static inline BOOL
static inline BOOL
//: OverrideImplementation(Class targetClass, SEL targetSelector, id (^implementationBlock)(Class originClass, SEL originCMD, IMP originIMP)) {
carryingOut(Class targetClass, SEL targetSelector, id (^implementationBlock)(Class originClass, SEL originCMD, IMP originIMP)) {
...
+ (void)load
{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        if (@available(iOS 12.1, *)) { // 解决12.1 的UITabbar 位置显示异常
            ...
            carryingOut(NSClassFromString(StringFromChampionData(appThoroughlyMessage)), @selector(setFrame:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP originIMP) {
...
- (void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext
{
    ...
    switch (self.currentOpearation) {
        ...
        case UINavigationControllerOperationPop:
            ...
            [self animationCell:transitionContext];
            //: break;
            break;
        //: case UINavigationControllerOperationPush:
        case UINavigationControllerOperationPush:
            //: [self pushAnimation:transitionContext];
            [self driveVertical:transitionContext];
            //: break;
            break;
        //: default:
        default:
...
- (void)driveVertical:(id<UIViewControllerContextTransitioning>)transitionContext
{
...
- (void)animationCell:(id<UIViewControllerContextTransitioning>)transitionContext
{
    //: UIViewController *toViewController = [transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    UIViewController *toViewController = [transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    //: UIViewController *fromViewController = [transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    ...
    CGFloat snapshootHeight = [UIDevice tingHeight] + fromViewController.navigationController.navigationBar.height;
    ...
    UITabBar *tabbar = [UserBarController scaleInstance].tabBar;
    //: UIView *containerView = [transitionContext containerView];
    UIView *containerView = [transitionContext containerView];
    ...
    //: } completion:^(BOOL finished) {
        ...
        [[UserBarController scaleInstance].view addSubview:tabbar];
...
- (void)superfluous
{
    ...
    if ([self.delegate respondsToSelector:@selector(blueGray:)]) {
        //: [self.delegate animationDidEnd:self];
        [self.delegate blueGray:self];
    }
}
...
Byte * ChampionDataToCache(Byte *data) {
    int representation = data[0];
    int cue = data[1];
    int teamTapFormat = data[2];
    if (!representation) return data + teamTapFormat;
    for (int i = 0; i < cue / 2; i++) {
        int begin = teamTapFormat + i;
        int end = teamTapFormat + cue - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[0] = 0;
    data[teamTapFormat + cue] = 0;
    return data + teamTapFormat;
}

NSString *StringFromChampionData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)ChampionDataToCache(data)];
}  
