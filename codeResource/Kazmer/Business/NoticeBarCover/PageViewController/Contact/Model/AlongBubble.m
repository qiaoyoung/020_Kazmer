...
- (instancetype)initWithExceptBy:(id)first green:(id)second {
    //: self = [super init];
    self = [super init];
    //: if(self) {
    if(self) {
...
- (void)setMembers:(NSArray *)members
{
    ...
    for (id<OldProgress>member in members) {
        ...
        NSString *groupTitle = [member background];
    ...
    [tmp enumerateKeysAndObjectsUsingBlock:^(NSString *groupTitle, NSMutableArray *groupedMembers, BOOL *stop) {
        ...
        if (groupTitle.length) {
            ...
            [_groups addObject:[[Pair alloc] initWithExceptBy:groupTitle green:groupedMembers]];
        }
    //: }];
    }];
    //: [self sort];
...
- (void)mix:(id<OldProgress>)member
{
    ...
    if(!pair) {
        ...
        pair = [[Pair alloc] initWithExceptBy:groupTitle green:members];
    }
...
- (void)tap:(NSString *)title valueMembers:(NSArray *)members {
    ...
    Pair *pair = [[Pair alloc] initWithExceptBy:title green:members];
...
- (id<OldProgress>)monthMemberId:(NSString *)uid{
    ...
    for (Pair *pair in _groups) {
        ...
        for (id<OldProgress> member in members) {
            ...
            if ([[member dataWith] isEqualToString:uid]) {
                //: return member;
                return member;
            }
        }
...
- (void)sort
{
    ...
    [_groups enumerateObjectsUsingBlock:^(Pair *obj, NSUInteger idx, BOOL *stop) {
        ...
        [groupedMembers sortUsingComparator:^NSComparisonResult(id<OldProgress> member1, id<OldProgress> member2) {
            //: return _groupMemberComparator([member1 sortKey], [member2 sortKey]);
            return _groupMemberComparator([member1 outsideScale], [member2 outsideScale]);
        //: }];
        }];
    //: }];
    }];
