...
@implementation MessageModeView
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    //: if (self) {
    if (self) {
        //: _avatarImageView = [[WatchAvatarImageView alloc] initWithFrame:CGRectMake(0, 0, 40, 40)];
        _avatarImageView = [[RecordControl alloc] initWithFrame:CGRectMake(0, 0, 40, 40)];
        //: [_avatarImageView addTarget:self action:@selector(onTouchAvatar:) forControlEvents:UIControlEventTouchUpInside];
        [_avatarImageView addTarget:self action:@selector(progressed:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)beyondTimeMember:(TimeMember *)member{
    ...
    self.textLabel.text = [OldUtil red:member.info.infoId dateSession:nil];
    ...
    [_avatarImageView press:url title:member.info.avatarImage bubbleImageOptions:SDWebImageRetryFailed];
}
...
- (void)progressed:(id)sender{
    ...
    if ([self.delegate respondsToSelector:@selector(progressAvatar:)]) {
        //: [self.delegate didTouchUserListAvatar:self.member.info.infoId];
        [self.delegate progressAvatar:self.member.info.infoId];
    }
}
