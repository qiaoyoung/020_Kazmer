...
- (void)showCan{
    ...
    self.thumb.center = [self pullInWithoutScaleValue:_value];
...
- (CGPoint)pullInWithoutScaleValue:(CGFloat)value{
...
- (void)setBufferProgress:(CGFloat)bufferProgress{
    ...
    bufferProgress = [self inside:bufferProgress];
...
- (void)key:(UIImage *)thumbImage domain:(UIControlState)state{
...
- (void)countChanged:(BOOL)isFullScreen{
    //: _frame = self.bounds;
    _frame = self.bounds;
    //: [self creatUI];
    [self showCan];
