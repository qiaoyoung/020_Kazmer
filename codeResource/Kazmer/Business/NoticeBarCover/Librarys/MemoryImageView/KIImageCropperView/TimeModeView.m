...
@interface RecentReplacementView : UIView {
//: @private
@private
    //: CGRect _maskRect;
    CGRect _maskRect;
...
- (void)setParadigmSize:(CGSize)size;
//: - (CGSize)maskSize;
- (CGSize)paradigmSize;
//: @end
@end
...
@interface TimeModeView() {
    //: KIImageCropperMaskView *_maskView;
    RecentReplacementView *_maskView;
...
- (RecentReplacementView *)maskView {
    //: if (_maskView == nil) {
    if (_maskView == nil) {
        //: _maskView = [[KIImageCropperMaskView alloc] initWithFrame:self.bounds];
        _maskView = [[RecentReplacementView alloc] initWithFrame:self.bounds];
...
- (UIImageView *)representationVisualImage {
    //: if (_imageView == nil) {
    if (_imageView == nil) {
        //: _imageView = [[UIImageView alloc] init];
        _imageView = [[UIImageView alloc] init];
        //: [[self scrollView] addSubview:_imageView];
        [[self when] addSubview:_imageView];
    }
    //: return _imageView;
    return _imageView;
}
...
- (UIView *)viewForZoomingInScrollView:(UIScrollView *)scrollView {
    ...
    return [self representationVisualImage];
}
...
- (void)keySession:(KIImageCropRatio)ratio {
...
- (void)setImage:(UIImage *)image {
    ...
    if (_image != image) {
        ...
        [[self representationVisualImage] setImage:_image];

        //: [[self imageView] setFrame:CGRectMake(0, 0, _image.size.width, _image.size.height)];
        [[self representationVisualImage] setFrame:CGRectMake(0, 0, _image.size.width, _image.size.height)];
...
- (void)bubble {
    ...
    [[self when] setMinimumZoomScale:min];
    //: [[self scrollView] setMaximumZoomScale:max+5.0f];
    [[self when] setMaximumZoomScale:max+5.0f];

    //: [[self scrollView] setZoomScale:min animated:YES];
    [[self when] setZoomScale:min animated:YES];

    //: [[self scrollView] setContentInset:_imageViewInsets];
    [[self when] setContentInset:_imageViewInsets];
    //: [[self scrollView] setContentOffset:CGPointMake(-_imageViewInsets.left, -_imageViewInsets.top) animated:YES];
    [[self when] setContentOffset:CGPointMake(-_imageViewInsets.left, -_imageViewInsets.top) animated:YES];
}
...
- (void)layoutSubviews {
    ...
    [(RecentReplacementView*)[self maskView] setParadigmSize:[self smallnessViewDissemble]];
    //: [[self scrollView] setFrame:self.bounds];
    [[self when] setFrame:self.bounds];
    //: [self updateZoomScale];
    [self bubble];
}

//: - (UIImage *)croppedImage {
- (UIImage *)defendingTeam {
    //: CGFloat zoomScale = [self scrollView].zoomScale;
    CGFloat zoomScale = [self when].zoomScale;

    //: CGFloat offsetX = [self scrollView].contentOffset.x;
    CGFloat offsetX = [self when].contentOffset.x;
    //: CGFloat offsetY = [self scrollView].contentOffset.y;
    CGFloat offsetY = [self when].contentOffset.y;
    ...
    UIImage *image = [_image modelHeight:aX display:aY width:aWidth value:aHeight];
    ...
    image = [self buttonObserver:image tool:imgSize image:YES];
...
- (UIImage*)buttonObserver:(UIImage*)image tool:(CGSize)newSize image:(BOOL)opaque
{
...
@implementation RecentReplacementView
