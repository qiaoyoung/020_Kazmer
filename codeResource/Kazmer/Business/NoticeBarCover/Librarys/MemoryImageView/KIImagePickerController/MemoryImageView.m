...
#import "MemoryImageView.h"

//: @interface KICropImageMaskView : UIView {
@interface DissembleView : UIView {
//: @private
@private
    //: CGRect _cropRect;
    CGRect _cropRect;
...
@implementation MemoryImageView
...
- (void)setFrame:(CGRect)frame {
    ...
    [[self anComplete] setFrame:self.bounds];
    ...
    if (__CGSizeEqualToSize(_cropSize, CGSizeZero)) {
        ...
        [self setSet:CGSizeMake(100, 100)];
    }
}

//: - (UIScrollView *)scrollView {
- (UIScrollView *)anComplete {
    //: if (_scrollView == nil) {
    if (_scrollView == nil) {
...
- (UIImageView *)data {
    ...
    if (_imageView == nil) {
        ...
        [[self anComplete] addSubview:_imageView];
    }
    //: return _imageView;
    return _imageView;
}
...
- (DissembleView *)maskView {
    //: if (_maskView == nil) {
    if (_maskView == nil) {
        //: _maskView = [[KICropImageMaskView alloc] init];
        _maskView = [[DissembleView alloc] init];
...
- (void)text {
    ...
    [[self data] setFrame:CGRectMake(0, 0, width, height)];
...
//    }
    ...
    [[self anComplete] setMinimumZoomScale:min];
    //: [[self scrollView] setMaximumZoomScale:max + 5.0f];
    [[self anComplete] setMaximumZoomScale:max + 5.0f];

    //: [[self scrollView] setZoomScale:min animated:YES];
    [[self anComplete] setZoomScale:min animated:YES];
}
...
- (void)setSet:(CGSize)size {
    ...
    [(DissembleView *)[self maskView] setTime:_cropSize];
    ...
    [[self anComplete] setContentInset:_imageInset];

    //: [[self scrollView] setContentOffset:CGPointMake(0, 0)];
    [[self anComplete] setContentOffset:CGPointMake(0, 0)];
}

//: - (UIImage *)cropImage {
- (UIImage *)lengthTeam {
    //: CGFloat zoomScale = [self scrollView].zoomScale;
    CGFloat zoomScale = [self anComplete].zoomScale;

    //: CGFloat offsetX = [self scrollView].contentOffset.x;
    CGFloat offsetX = [self anComplete].contentOffset.x;
    //: CGFloat offsetY = [self scrollView].contentOffset.y;
    CGFloat offsetY = [self anComplete].contentOffset.y;
    ...
    image = [image safetyMargin:resizeWidth nearDate:resizeheight];
