...
- (void)anObjectStatus
{
    ...
    self.spacingBetweenDots = notiKitUrl;
    //: self.numberOfPages = kDefaultNumberOfPages;
    self.numberOfPages = dreamTextImageMsg;
    //: self.currentPage = kDefaultCurrentPage;
    self.currentPage = mainBeMessage;
    //: self.hidesForSinglePage = kDefaultHideForSinglePage;
    self.hidesForSinglePage = userPreviousCardData;
    //: self.shouldResizeFromCenter = kDefaultShouldResizeFromCenter;
    self.shouldResizeFromCenter = showFillViaData;
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touch.view != self) {
        ...
        if ([self.delegate respondsToSelector:@selector(input:changeBy:)]) {
            //: [self.delegate TAPageControl:self didSelectPageAtIndex:index];
            [self.delegate input:self changeBy:index];
        }
    }
}
...
- (CGSize)key:(NSInteger)pageCount
{
...
- (void)qualifyDateName
{
    ...
    for (NSInteger i = 0; i < self.numberOfPages; i++) {
        ...
        //: } else {
            ...
            dot = [self statusSight];
        }

        //: [self updateDotFrame:dot atIndex:i];
        [self ting:dot modifyToIndex:i];
    }

    //: [self changeActivity:YES atIndex:self.currentPage];
    [self agreement:YES languageIndex:self.currentPage];
...
- (void)account:(BOOL)overrideExistingFrame
{
    //: CGPoint center = self.center;
    CGPoint center = self.center;
    //: CGSize requiredSize = [self sizeForNumberOfPages:self.numberOfPages];
    CGSize requiredSize = [self key:self.numberOfPages];
...
- (void)ting:(UIView *)dot modifyToIndex:(NSInteger)index
{
    // Dots are always centered within view
    //: CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self sizeForNumberOfPages:self.numberOfPages].width) / 2);
    CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self key:self.numberOfPages].width) / 2);
...
- (void)agreement:(BOOL)active languageIndex:(NSInteger)index
{
    //: if (self.dotViewClass) {
    if (self.dotViewClass) {
        //: TAAbstractDotView *abstractDotView = (TAAbstractDotView *)[self.dots objectAtIndex:index];
        WeltanschauungView *abstractDotView = (WeltanschauungView *)[self.dots objectAtIndex:index];
        //: if ([abstractDotView respondsToSelector:@selector(changeActivityState:)]) {
        if ([abstractDotView respondsToSelector:@selector(sensoryActivityContentRefresh:)]) {
            //: [abstractDotView changeActivityState:active];
            [abstractDotView sensoryActivityContentRefresh:active];
        //: } else {
        } else {
...
- (void)setCurrentPage:(NSInteger)currentPage
{
    ...
    [self agreement:NO languageIndex:_currentPage];
    //: _currentPage = currentPage;
    _currentPage = currentPage;
    // Post set
    //: [self changeActivity:YES atIndex:_currentPage];
    [self agreement:YES languageIndex:_currentPage];
}
