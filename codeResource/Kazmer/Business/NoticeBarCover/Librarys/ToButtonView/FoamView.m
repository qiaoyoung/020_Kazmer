...
NSString *StringFromPointeToData(Byte *data);


//: http
Byte show_armColorFormat[] = {44, 4, 4, 142, 112, 116, 116, 104, 157};
...
NSString * const app_taskText = @"cycleCell";

//: @interface SDCycleScrollView () <UICollectionViewDataSource, UICollectionViewDelegate>
@interface FoamView () <UICollectionViewDataSource, UICollectionViewDelegate>
...
+ (instancetype)option:(CGRect)frame media:(NSArray *)imageNamesGroup
{
    //: SDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
    FoamView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
+ (instancetype)fasteningGroup:(CGRect)frame smallBeer:(BOOL)infiniteLoop session:(NSArray *)imageNamesGroup
{
    //: SDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
    FoamView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
+ (instancetype)margin:(CGRect)frame afterNameUnder:(NSArray *)imageURLsGroup
{
    //: SDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
    FoamView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
+ (instancetype)maturity:(CGRect)frame infoViewDelegate:(id<SubUser>)delegate user:(UIImage *)placeholderImage
{
    //: SDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
    FoamView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
- (void)play
{
    ...
    [mainView registerClass:[ToButtonView class] forCellWithReuseIdentifier:app_taskText];
...
- (void)setPageControlDotSize:(CGSize)pageControlDotSize
{
    ...
    if ([self.pageControl isKindOfClass:[SubView class]]) {
        //: TAPageControl *pageContol = (TAPageControl *)_pageControl;
        SubView *pageContol = (SubView *)_pageControl;
...
- (void)setCurrentPageDotColor:(UIColor *)currentPageDotColor
{
    ...
    if ([self.pageControl isKindOfClass:[SubView class]]) {
        ...
        SubView *pageControl = (SubView *)_pageControl;
...
- (void)setCurrentPageDotImage:(UIImage *)currentPageDotImage
{
    ...
    [self text:currentPageDotImage than:YES];
}
...
- (void)setPageDotImage:(UIImage *)pageDotImage
{
    ...
    [self text:pageDotImage than:NO];
}

//: - (void)setCustomPageControlDotImage:(UIImage *)image isCurrentPageDot:(BOOL)isCurrentPageDot
- (void)text:(UIImage *)image than:(BOOL)isCurrentPageDot
{
...
- (void)dataColor
{
    ...
    NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:self.autoScrollTimeInterval target:self selector:@selector(marginShow) userInfo:nil repeats:YES];
    //: _timer = timer;
    _timer = timer;
...
- (void)sessionButton
{
    ...
    int indexOnPageControl = [self skim] % self.imagePathsGroup.count;
    ...
    switch (self.pageControlStyle) {
        ...
        case SDCycleScrollViewPageContolStyleStrip:
        {
            ...
            TimeView *pageControl = [[TimeView alloc] init];
        ...
        case SDCycleScrollViewPageContolStyleAnimated:
        {
            ...
            SubView *pageControl = [[SubView alloc] init];
            //: pageControl.numberOfPages = self.imagePathsGroup.count;
            pageControl.numberOfPages = self.imagePathsGroup.count;
            //: pageControl.dotColor = self.currentPageDotColor;
            pageControl.dotColor = self.currentPageDotColor;
...
- (void)layoutSubviews
{
    ...
    if ([self.pageControl isKindOfClass:[SubView class]]) {
        ...
        size = [pageControl key:self.imagePathsGroup.count];
    }
    //: else if ([self.pageControl isKindOfClass:[TTStripPageControl class]]){
    else if ([self.pageControl isKindOfClass:[TimeView class]]){
        //: TTStripPageControl *pageControl = (TTStripPageControl *)_pageControl;
        TimeView *pageControl = (TimeView *)_pageControl;
        //: size = [pageControl sizeForNumberOfPages:self.imagePathsGroup.count];
        size = [pageControl individual:self.imagePathsGroup.count];
    }
    //: else {
    else {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ToButtonView *cell = [collectionView dequeueReusableCellWithReuseIdentifier:app_taskText forIndexPath:indexPath];
    ...
    if ([imagePath isKindOfClass:[NSString class]]) {
        ...
        if ([imagePath hasPrefix:StringFromPointeToData(show_armColorFormat)]) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(shouldRow:info:)]) {
        //: [self.delegate cycleScrollView:self didSelectItemAtIndex:indexPath.item % self.imagePathsGroup.count];
        [self.delegate shouldRow:self info:indexPath.item % self.imagePathsGroup.count];
    }
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
    ...
    if ([self.delegate respondsToSelector:@selector(messageTitle:bubble:)]) {
        //: [self.delegate cycleScrollView:self didScrollToIndex:indexOnPageControl];
        [self.delegate messageTitle:self bubble:indexOnPageControl];
...
Byte * PointeToDataToCache(Byte *data) {
    int wideCut = data[0];
    int surely = data[1];
    int veteranImage = data[2];
    if (!wideCut) return data + veteranImage;
    for (int i = 0; i < surely / 2; i++) {
        int begin = veteranImage + i;
        int end = veteranImage + surely - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[0] = 0;
    data[veteranImage + surely] = 0;
    return data + veteranImage;
}

NSString *StringFromPointeToData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)PointeToDataToCache(data)];
}  
