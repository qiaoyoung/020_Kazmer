...
typedef void(^SGPermissionPhotoBlock)(PagePhoto *photos, SGPermissionStatus status);

//: @interface SGPermissionPhoto : NSObject
@interface PagePhoto : NSObject
//: + (void)photo:(SGPermissionPhotoBlock)block;
+ (void)belowSendPhoto:(SGPermissionPhotoBlock)block;
//: + (void)request:(void (^)(BOOL granted))handler;
+ (void)at:(void (^)(BOOL granted))handler;
//: @end
@end
