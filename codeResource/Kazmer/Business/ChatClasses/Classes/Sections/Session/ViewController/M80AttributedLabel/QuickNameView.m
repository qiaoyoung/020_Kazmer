...
static NSString* const show_pointIdent = @"\u2026";

//: @interface M80AttributedLabel ()
@interface QuickNameView ()
...
- (NSAttributedString *)team:(NSString *)text
{
    //: if ([text length])
    if ([text length])
    {
...
- (NSAttributedString *)model:(NSMutableAttributedString *)drawString
{
    //: if (drawString)
    if (drawString)
    {
...
+ (NSRegularExpression *)quickEmoticon {
...
- (NSMutableAttributedString *)scale:(NSAttributedString *)attributedString {
    // 匹配 [表情]
    ...
    if (attrM.length > 0) {
        ...
        NSArray<NSTextCheckingResult *> *emoticonResults = [[QuickNameView quickEmoticon] matchesInString:attrM.string options:kNilOptions range:attrM.yy_rangeOfAll];
...
- (NSMutableAttributedString*)show:(NSMutableAttributedString *)wholeText {
...
- (void)setText:(NSString *)text
{
    ...
    NSAttributedString *attributedText = [self team:text];
...
- (void)setAttributedText:(NSAttributedString *)attributedText
{
    ...
    NSAttributedString *attString = [self model:[[NSMutableAttributedString alloc] initWithAttributedString:attributedText]];
    //: _attributedString = [self needHightText:[self transformEmojiDescToEomjiImageWithAttributedString:attString]];
    _attributedString = [self show:[self scale:attString]];
...
- (void)format:(NSString *)text
{
    //: NSAttributedString *attributedText = [self attributedString:text];
    NSAttributedString *attributedText = [self team:text];
    //: [self appendAttributedText:attributedText];
    [self to:attributedText];
}

//: - (void)appendAttributedText:(NSAttributedString *)attributedText
- (void)to:(NSAttributedString *)attributedText
{
