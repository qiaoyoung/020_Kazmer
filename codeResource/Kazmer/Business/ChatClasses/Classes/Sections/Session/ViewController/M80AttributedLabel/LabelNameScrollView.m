...
static NSString* const app_eventFieldLocationKey = @"\u2026";

//: @interface M80AttributedLabel ()
@interface LabelNameScrollView ()
...
@implementation LabelNameScrollView
...
- (NSMutableAttributedString *)formRangeSnapline:(NSAttributedString *)attributedString {
    // 匹配 [表情]
    ...
    if (attrM.length > 0) {
        ...
        NSArray<NSTextCheckingResult *> *emoticonResults = [[LabelNameScrollView blockPin] matchesInString:attrM.string options:kNilOptions range:attrM.yy_rangeOfAll];
...
- (NSMutableAttributedString*)label:(NSMutableAttributedString *)wholeText {
...
- (void)setText:(NSString *)text
{
    ...
    NSAttributedString *attributedText = [self dateSystem:text];
...
- (void)setAttributedText:(NSAttributedString *)attributedText
{
    ...
    NSAttributedString *attString = [self tillContent:[[NSMutableAttributedString alloc] initWithAttributedString:attributedText]];
    //: _attributedString = [self needHightText:[self transformEmojiDescToEomjiImageWithAttributedString:attString]];
    _attributedString = [self label:[self formRangeSnapline:attString]];
...
- (NSAttributedString *)tillContent:(NSMutableAttributedString *)drawString
{
    //: if (drawString)
    if (drawString)
    {
...
- (NSAttributedString *)dateSystem:(NSString *)text
{
    //: if ([text length])
    if ([text length])
    {
