...
@interface CabinData : NSObject

@end

@implementation CabinData

+ (Byte *)CabinDataToCache:(Byte *)data {
    int courtAccentPlace = data[0];
    int technologic = data[1];
    for (int i = 0; i < courtAccentPlace / 2; i++) {
        int begin = technologic + i;
        int end = technologic + courtAccentPlace - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[technologic + courtAccentPlace] = 0;
    return data + technologic;
}

+ (NSString *)StringFromCabinData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self CabinDataToCache:data]];
}  

//: friend_circle_adapter_like
+ (NSString *)dream_patrolValue {
    /* static */ NSString *dream_patrolValue = nil;
    if (!dream_patrolValue) {
        Byte value[] = {26, 3, 5, 101, 107, 105, 108, 95, 114, 101, 116, 112, 97, 100, 97, 95, 101, 108, 99, 114, 105, 99, 95, 100, 110, 101, 105, 114, 102, 248};
        dream_patrolValue = [self StringFromCabinData:value];
    }
    return dream_patrolValue;
}

//: ic_fold
+ (NSString *)show_genetKey {
    /* static */ NSString *show_genetKey = nil;
    if (!show_genetKey) {
        Byte value[] = {7, 10, 209, 255, 185, 229, 237, 236, 124, 91, 100, 108, 111, 102, 95, 99, 105, 173};
        show_genetKey = [self StringFromCabinData:value];
    }
    return show_genetKey;
}

//: arrowDown
+ (NSString *)dream_despiteSuggestUrl {
    /* static */ NSString *dream_despiteSuggestUrl = nil;
    if (!dream_despiteSuggestUrl) {
        Byte value[] = {9, 12, 53, 15, 122, 230, 166, 70, 233, 180, 79, 163, 110, 119, 111, 68, 119, 111, 114, 114, 97, 24};
        dream_despiteSuggestUrl = [self StringFromCabinData:value];
    }
    return dream_despiteSuggestUrl;
}

//: WatchCF7
+ (NSString *)kWholeKey {
    /* static */ NSString *kWholeKey = nil;
    if (!kWholeKey) {
        Byte value[] = {6, 2, 55, 70, 67, 70, 70, 70, 118};
        kWholeKey = [self StringFromCabinData:value];
    }
    return kWholeKey;
}

//: arrowUp
+ (NSString *)mDynamicsValue {
    /* static */ NSString *mDynamicsValue = nil;
    if (!mDynamicsValue) {
        Byte value[] = {7, 3, 4, 112, 85, 119, 111, 114, 114, 97, 130};
        mDynamicsValue = [self StringFromCabinData:value];
    }
    return mDynamicsValue;
}

//: #333333
+ (NSString *)showMemoryExcessFeelValue {
    /* static */ NSString *showMemoryExcessFeelValue = nil;
    if (!showMemoryExcessFeelValue) {
        Byte value[] = {7, 5, 133, 69, 57, 51, 51, 51, 51, 51, 51, 35, 56};
        showMemoryExcessFeelValue = [self StringFromCabinData:value];
    }
    return showMemoryExcessFeelValue;
}

//: ic_praise%d
+ (NSString *)dreamSpecificImageMeaningName {
    /* static */ NSString *dreamSpecificImageMeaningName = nil;
    if (!dreamSpecificImageMeaningName) {
        Byte value[] = {11, 12, 23, 189, 119, 52, 202, 42, 16, 48, 63, 25, 100, 37, 101, 115, 105, 97, 114, 112, 95, 99, 105, 3};
        dreamSpecificImageMeaningName = [self StringFromCabinData:value];
    }
    return dreamSpecificImageMeaningName;
}
...
//: typedef NS_OPTIONS(NSUInteger, WatchBubbleMenuViewDirectionPriorityType) {
typedef NS_OPTIONS(NSUInteger, WatchBubbleMenuViewDirectionPriorityType) {
    //: WatchBubbleMenuViewDirectionPriorityType_Up,
    WatchBubbleMenuViewDirectionPriorityType_Up,
    //: WatchBubbleMenuViewDirectionPriorityType_Down
    WatchBubbleMenuViewDirectionPriorityType_Down
//: };
};

//: @interface WatchBubbleMenuView()
...
//: @property (nonatomic, assign)WatchBubbleMenuViewDirectionPriorityType directionPriority;
@property (nonatomic, assign)WatchBubbleMenuViewDirectionPriorityType directionPriority;
...
//: @implementation WatchBubbleMenuView
@implementation BroadcastView

//: + (instancetype)shareMenuView {
+ (instancetype)dismissDisappear {
    //: static WatchBubbleMenuView *menu = nil;
    static BroadcastView *menu = nil;
    ...
    _dispatch_once(&onceToken, ^{
        //: if (menu == nil) {
        if (menu == nil) {
            //: menu = [[WatchBubbleMenuView alloc] init];
            menu = [[BroadcastView alloc] init];
        }
    //: });
    });
  //: return menu;
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _bubbleBottomMargin = 20;
        //: _directionPriority = WatchBubbleMenuViewDirectionPriorityType_Up;
        _directionPriority = WatchBubbleMenuViewDirectionPriorityType_Up;

        //: UITapGestureRecognizer *singleTapbox1 = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(clickView)];
        UITapGestureRecognizer *singleTapbox1 = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tingView)];
...
- (void)array{
    ...
    for (int i = 0; i < _buttonCount; i ++) {
        ...
        if(_isfold && i==7){
            ...
            [_foldbtn addTarget:self action:@selector(pined:) forControlEvents:UIControlEventTouchUpInside];
            //: [_foldbtn setImage:[UIImage imageNamed:@"ic_fold"] forState:UIControlStateNormal];
            [_foldbtn setImage:[UIImage imageNamed:[CabinData show_genetKey]] forState:UIControlStateNormal];
        ...
        [button addTarget:self action:@selector(clickGlobal:) forControlEvents:UIControlEventTouchUpInside];
        //: [button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"ic_praise%d",i+1]] forState:UIControlStateNormal];
        [button setImage:[UIImage imageNamed:[NSString stringWithFormat:[CabinData dreamSpecificImageMeaningName],i+1]] forState:UIControlStateNormal];
...
- (void)accountIn:(NSArray *)array
                 //: cursorStartRect:(CGRect)cursorStartRect selectionTextRectInWindow:(CGRect)rect selectBlock:(void(^)(WatchMediaItem *item))block praiseBlock:(void(^)(NSInteger tag))praiseblock {
                 openBlock:(CGRect)cursorStartRect queryion:(CGRect)rect bubble:(void(^)(InfoMakeBar *item))block show:(void(^)(NSInteger tag))praiseblock {
    ...
    UIView *buttonsBgView = [self cartLikeArray:array];
    ...
    //默认的方向是优先方向
    //: WatchBubbleMenuViewDirectionPriorityType direction = _directionPriority;
    WatchBubbleMenuViewDirectionPriorityType direction = _directionPriority;
//
...
//        //优先级Up的话，先判断在上面的情况。
        ...
        if (rect.origin.y - _bubbleContentMargin - _bubbleBottomMargin - praiseheight > _bubbleHeight) {
            ...
            selfRect.origin.y = rect.origin.y - _bubbleContentMargin - _bubbleHeight - praiseheight + 15;
            //: direction = WatchBubbleMenuViewDirectionPriorityType_Up;
            direction = WatchBubbleMenuViewDirectionPriorityType_Up;
        ...
        //: } else if (screenHeight - rect.origin.y - rect.size.height - _bubbleContentMargin - _bubbleBottomMargin - praiseheight > _bubbleHeight) {
            ...
            selfRect.origin.y = rect.origin.y + rect.size.height + _bubbleContentMargin -15;
            //: direction = WatchBubbleMenuViewDirectionPriorityType_Down;
            direction = WatchBubbleMenuViewDirectionPriorityType_Down;
...
//    }
    ...
    CGRect bgViewRect = buttonsBgView.frame;
    //: bgViewRect.origin.y = direction == WatchBubbleMenuViewDirectionPriorityType_Down ? _arrowHeight : praiseheight;
    bgViewRect.origin.y = direction == WatchBubbleMenuViewDirectionPriorityType_Down ? _arrowHeight : praiseheight;
...
//    }
    ...
    //: if (direction == WatchBubbleMenuViewDirectionPriorityType_Up) {
    if (direction == WatchBubbleMenuViewDirectionPriorityType_Up) {
        ...
        _arrowView.image = [UIImage imageNamed:[CabinData dream_despiteSuggestUrl]];
    ...
    //: } else {
        ...
        _arrowView.image = [UIImage imageNamed:[CabinData mDynamicsValue]];
...
- (UIView *)cartLikeArray:(NSArray *)array {
    ...
    for (int i = 0; i < array.count; i ++) {
        ...
        //: WatchBubbleButtonModel *model = array[i];
        PageCountImage *model = array[i];
        ...
        label.textColor = [UIColor min:[CabinData showMemoryExcessFeelValue]];
    ...
    //: _buttonsBgView.backgroundColor = [UIColor colorWithHexString:@"WatchCF7"];
    _buttonsBgView.backgroundColor = [UIColor min:[CabinData kWholeKey]];
...
- (void)holdfasts:(UIButton *)button {
    //: if (_myNewContentArray.count > button.tag - 100) {
    if (_myNewContentArray.count > button.tag - 100) {
        //: WatchBubbleButtonModel *model = (WatchBubbleButtonModel *)_myNewContentArray[button.tag - 100];
        PageCountImage *model = (PageCountImage *)_myNewContentArray[button.tag - 100];
        //: NSLog(@"点击了%@",model.name);

        //: if([model.name isEqual:[WatchLanguageManager getTextWithKey:@"friend_circle_adapter_like"]]){
        if([model.name isEqual:[PaintedNaturalLanguageTo exhibit:[CabinData dream_patrolValue]]]){
...
- (void)clickGlobal:(UIButton *)button {
...
- (BOOL)dorsalVertebraChanged:(NSArray *)newArray {
    ...
    for (int i = 0; i < newArray.count; i ++) {
        //: BOOL finded = false;
        BOOL finded = false;
        //: NSInteger newId = ((WatchBubbleButtonModel *)_oldContentArray[i]).buttonId;
        NSInteger newId = ((PageCountImage *)_oldContentArray[i]).buttonId;
        //: for (int j = 0; j < _oldContentArray.count; j ++) {
        for (int j = 0; j < _oldContentArray.count; j ++) {
            //: NSInteger oldId = ((WatchBubbleButtonModel *)_oldContentArray[j]).buttonId;
            NSInteger oldId = ((PageCountImage *)_oldContentArray[j]).buttonId;
            //: if (newId == oldId) {
            if (newId == oldId) {
                //: finded = true;
                finded = true;
