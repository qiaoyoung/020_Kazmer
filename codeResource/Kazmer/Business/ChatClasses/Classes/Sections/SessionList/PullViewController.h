...
//: @interface DisplaySessionListViewController : UIViewController<UITableViewDataSource,UITableViewDelegate,NIMLoginManagerDelegate,NIMConversationManagerDelegate>
@interface PullViewController : UIViewController<UITableViewDataSource,UITableViewDelegate,NIMLoginManagerDelegate,NIMConversationManagerDelegate>
/**
 *  会话列表tableView
 */
...
- (void)graduatedTable:(NIMRecentSession *)recent
             //: atIndexPath:(NSIndexPath *)indexPath;
             caterer:(NSIndexPath *)indexPath;
/**
 *  cell显示的最近会话具体内容
 *
 *  @param recent 最近会话
 ...
 *  @return 时间戳格式化后的字符串
 *
 *  @discussion 默认实现为：最后一条消息的时间戳，具体时间戳消息格式详见DisplayKitUtil中， + (NSString*)showTime:(NSTimeInterval) msglastTime showDetail:(BOOL)showDetail 方法的实现。
 */
