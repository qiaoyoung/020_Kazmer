...
- (CGSize)intrinsicContentSize
{
    ...
    return [self session:self.addToSize].size;
}
...
- (void)setMinNumberOfLines:(NSInteger)minNumberOfLines
{
    ...
    self.minHeight = [self lowSimulate:minNumberOfLines];
...
- (void)setMaxNumberOfLines:(NSInteger)maxNumberOfLines
{
    ...
    self.maxHeight = [self lowSimulate:maxNumberOfLines];
...
- (CGFloat)lowSimulate:(NSInteger)line
{
    ...
    CGFloat height = self.addToSize.height - (textViewMargin + self.textView.contentInset.top + self.textView.contentInset.bottom);
...
- (void)change{
    ...
    if (newScrollViewFrame.size.height <= self.maxHeight && _previousTextHeight == 0) {
        ...
        if(oldScrollViewFrame.size.height != newScrollViewFrame.size.height) {
            ...
            if ([self.textViewDelegate respondsToSelector:@selector(willed:)]) {
                //: [self.textViewDelegate willChangeHeight:newScrollViewFrame.size.height];
                [self.textViewDelegate willed:newScrollViewFrame.size.height];
            }
    ...
    if (oldScrollViewFrame.size.height != newScrollViewFrame.size.height && [self.textViewDelegate respondsToSelector:@selector(valued:)]) {
        //: [self.textViewDelegate didChangeHeight:newScrollViewFrame.size.height];
        [self.textViewDelegate valued:newScrollViewFrame.size.height];
    }
...
- (CGRect)session:(CGSize)contentSize
{
    //: CGSize selfSize;
    CGSize selfSize;
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(ting:down:)]) {
        //: return [self.textViewDelegate shouldChangeTextInRange:range replacementText:text];
        return [self.textViewDelegate ting:range down:text];
    }
    //: return YES;
    return YES;
}
...
- (BOOL)textView:(UITextView *)textView shouldInteractWithURL:(NSURL *)URL inRange:(NSRange)characterRange
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(touch:bringHome:)]) {
        //: return [self.textViewDelegate shouldInteractWithURL:URL inRange:characterRange];
        return [self.textViewDelegate touch:URL bringHome:characterRange];
    }
    //: return YES;
    return YES;
}
...
- (BOOL)textView:(UITextView *)textView shouldInteractWithTextAttachment:(NSTextAttachment *)textAttachment inRange:(NSRange)characterRange
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(underName:searchKey:)]) {
        //: return [self.textViewDelegate shouldInteractWithTextAttachment:textAttachment inRange:characterRange];
        return [self.textViewDelegate underName:textAttachment searchKey:characterRange];
    }
    //: return YES;
    return YES;
}
...
- (void)textViewDidBeginEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(enabling:)]) {
        //: [self.textViewDelegate textViewDidBeginEditing:self];
        [self.textViewDelegate enabling:self];
    }
}
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(withSelection:)]) {
        //: [self.textViewDelegate textViewDidChangeSelection:self];
        [self.textViewDelegate withSelection:self];
    }
}
...
- (void)textViewDidEndEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(cleanBe:)]) {
        //: [self.textViewDelegate textViewDidEndEditing:self];
        [self.textViewDelegate cleanBe:self];
    }
}
...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(sizing:)]) {
       //: return [self.textViewDelegate textViewShouldBeginEditing:self];
       return [self.textViewDelegate sizing:self];
    }
    //: return YES;
    return YES;
}
...
- (BOOL)textViewShouldEndEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(ended:)]) {
        //: return [self.textViewDelegate textViewShouldEndEditing:self];
        return [self.textViewDelegate ended:self];
    }
    //: return YES;
    return YES;
}
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(cameraColor:)]) {
        //: [self.textViewDelegate textViewDidChange:self];
        [self.textViewDelegate cameraColor:self];
    }
    //: [self fitToScrollView];
    [self change];
}
