...
- (NSInteger)title:(NSInteger)value status:(NSInteger)min complete:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
...
- (void)finishTing:(NSInteger)currentPage
{
    ...
    NSInteger first = [self title:currentPage - 1 status:0 complete:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self title:currentPage + 1 status:0 complete:count - 1];
