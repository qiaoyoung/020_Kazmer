...
//: @interface WatchBadgeView ()
@interface NameSessionView ()
...
//: @implementation WatchBadgeView
@implementation NameSessionView

//: + (instancetype)viewWithBadgeTip:(NSString *)badgeValue{
+ (instancetype)title:(NSString *)badgeValue{
    //: if (!badgeValue) {
    if (!badgeValue) {
        //: badgeValue = @"";
        badgeValue = @"";
    }
    //: WatchBadgeView *instance = [[WatchBadgeView alloc] init];
    NameSessionView *instance = [[NameSessionView alloc] init];
    //: instance.frame = [instance frameWithStr:badgeValue];
    instance.frame = [instance visualCommunicationState:badgeValue];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self badgeValue] length]) {
        ...
        [self search:rect recent:context];
    //: }else{
    }else{
        //: [self drawWithOutContent:rect context:context];
        [self deviceAcross:rect powerContext:context];
    }
...
- (void)setBadgeValue:(NSString *)badgeValue {
    ...
    self.frame = [self visualCommunicationState:badgeValue];
...
- (CGSize)team:(NSString *)badgeValue{
...
- (CGRect)visualCommunicationState:(NSString *)badgeValue{
    //: CGSize badgeSize = [self badgeSizeWithStr:badgeValue];
    CGSize badgeSize = [self team:badgeValue];
...
- (void)search:(CGRect)rect recent:(CGContextRef)context{
...
- (void)deviceAcross:(CGRect)rect powerContext:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
