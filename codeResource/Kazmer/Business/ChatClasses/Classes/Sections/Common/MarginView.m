...
- (void)exhibitView: (NSInteger)page
{
...
- (NSInteger)image:(NSInteger)value key:(NSInteger)min quick:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
...
- (void)spectralColour:(NSInteger)currentPage
{
    ...
    NSInteger first = [self image:currentPage - 1 key:0 quick:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self image:currentPage + 1 key:0 quick:count - 1];
...
- (void)filtrate
{
    ...
    [self spectralColour:_currentPage];
