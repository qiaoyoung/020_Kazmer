...
+ (instancetype)streetSmart:(NSString *)badgeValue{
    //: if (!badgeValue) {
    if (!badgeValue) {
        //: badgeValue = @"";
        badgeValue = @"";
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self badgeValue] length]) {
        ...
        [self teamClear:rect languageBy:context];
    //: }else{
    }else{
        //: [self drawWithOutContent:rect context:context];
        [self extent:rect magnitudeOrigin:context];
    }
...
- (void)setBadgeValue:(NSString *)badgeValue {
    ...
    self.frame = [self locationStr:badgeValue];
...
- (CGSize)diam:(NSString *)badgeValue{
...
- (CGRect)locationStr:(NSString *)badgeValue{
    //: CGSize badgeSize = [self badgeSizeWithStr:badgeValue];
    CGSize badgeSize = [self diam:badgeValue];
...
- (void)teamClear:(CGRect)rect languageBy:(CGContextRef)context{
...
- (void)extent:(CGRect)rect magnitudeOrigin:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
