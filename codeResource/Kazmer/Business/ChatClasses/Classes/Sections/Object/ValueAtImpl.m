...
@property (nonatomic,weak) id<InsideDelegate> delegate;
                         ...
                         specialSession:(id<ColorConfig>)sessionConfig
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(prixFixeWith:) name:UIMenuControllerDidHideMenuNotification object:nil];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(monthsed:) name:dreamPassData object:nil];
    }
    //: return self;
    return self;
}
...
- (void)numericalQuantity
{
    ...
    self.tableView.frame = CGRectMake(0, (44.0f + [UIDevice comeDownSuperphylum]), [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice comeDownSuperphylum])-self.inputView.height);
    //: self.tableView.bottom = self.inputView.device_top;
    self.tableView.bottom = self.inputView.device_top;
    //: [self.tableView nim_scrollToBottom:YES];
    [self.tableView state:YES];
...
- (void)backgroundControl
{
    ...
    [self.refreshControl addTarget:self action:@selector(indexxed:) forControlEvents:UIControlEventValueChanged];

}

//: - (void)headerRereshing:(id)sender
- (void)indexxed:(id)sender
{
    //: if ([self.delegate respondsToSelector:@selector(onRefresh)])
    if ([self.delegate respondsToSelector:@selector(blockLight)])
    {
        //: [self.delegate onRefresh];
        [self.delegate blockLight];
    }
}

//: - (void)insert:(NSArray<NSIndexPath *> *)indexPaths animated:(BOOL)animated
- (void)green:(NSArray<NSIndexPath *> *)indexPaths topOpen:(BOOL)animated
{
    //: if (!indexPaths.count)
    if (!indexPaths.count)
    {
...
//        }];
    ...
    if (isScroll || [self tingSize:addIndexPathes])
    {
        //: [self.tableView reloadData];
        [self.tableView reloadData];
    }
...
- (void)paged:(NSArray<NSIndexPath *> *)indexPaths
{
    //: if ([self shouldReloadWhenRemoveOrUpdate:indexPaths])
    if ([self belowColor:indexPaths])
    {
        //: [self.tableView reloadData];
        [self.tableView reloadData];
        //: return;
...
- (void)pull {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (!self.inputView.replyedContent.hidden)
        {
            ...
            [self.inputView.replyedContent titleBy];
        }
    //: });
    });
}
...
- (BOOL)tingSize:(NSArray<NSIndexPath *> *)indexPaths
{
...
- (BOOL)belowColor:(NSArray<NSIndexPath *> *)indexPaths
{
    //: for(NSIndexPath * indexPath in indexPaths)
    for(NSIndexPath * indexPath in indexPaths)
    {
