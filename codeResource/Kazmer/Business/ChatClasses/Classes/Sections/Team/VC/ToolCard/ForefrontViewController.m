...
@interface PreparationData : NSObject

+ (instancetype)sharedInstance;

@end

@implementation PreparationData

+ (instancetype)sharedInstance {
    static PreparationData *instance = nil;
...
- (Byte *)PreparationDataToCache:(Byte *)data {
    int genetOho = data[0];
    Byte fumedOak = data[1];
    int nowhereOutside = data[2];
    for (int i = nowhereOutside; i < nowhereOutside + genetOho; i++) {
        int value = data[i] + fumedOak;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    }
    data[nowhereOutside + genetOho] = 0;
    return data + nowhereOutside;
}

- (NSString *)StringFromPreparationData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self PreparationDataToCache:data]];
}

//: #F6F7FA
- (NSString *)noti_heavyMsg {
    /* static */ NSString *noti_heavyMsg = nil;
    if (!noti_heavyMsg) {
        Byte value[] = {7, 20, 7, 59, 185, 120, 156, 15, 50, 34, 50, 35, 50, 45, 36};
        noti_heavyMsg = [self StringFromPreparationData:value];
    }
    return noti_heavyMsg;
}

//: contact_list_activity_complete
- (NSString *)noti_everythingValue {
    /* static */ NSString *noti_everythingValue = nil;
    if (!noti_everythingValue) {
        Byte value[] = {30, 23, 12, 146, 156, 164, 114, 170, 20, 245, 170, 220, 76, 88, 87, 93, 74, 76, 93, 72, 85, 82, 92, 93, 72, 74, 76, 93, 82, 95, 82, 93, 98, 72, 76, 88, 86, 89, 85, 78, 93, 78, 123};
        noti_everythingValue = [self StringFromPreparationData:value];
    }
    return noti_everythingValue;
}

//: #4B43DE
- (NSString *)showOverseeName {
    /* static */ NSString *showOverseeName = nil;
    if (!showOverseeName) {
        Byte value[] = {7, 29, 11, 82, 127, 155, 193, 70, 78, 175, 228, 6, 23, 37, 23, 22, 39, 40, 126};
        showOverseeName = [self StringFromPreparationData:value];
    }
    return showOverseeName;
}

//: back_arrow_bl
- (NSString *)m_shipEmployApparentlyMsg {
    /* static */ NSString *m_shipEmployApparentlyMsg = nil;
    if (!m_shipEmployApparentlyMsg) {
        Byte value[] = {13, 5, 11, 214, 226, 189, 242, 65, 129, 10, 156, 93, 92, 94, 102, 90, 92, 109, 109, 106, 114, 90, 93, 103, 93};
        m_shipEmployApparentlyMsg = [self StringFromPreparationData:value];
    }
    return m_shipEmployApparentlyMsg;
}

//: ZMONTeamCartSet%ld
- (NSString *)main_collectorKey {
    /* static */ NSString *main_collectorKey = nil;
    if (!main_collectorKey) {
        Byte value[] = {18, 16, 8, 148, 105, 9, 7, 124, 74, 61, 63, 62, 68, 85, 81, 93, 51, 81, 98, 100, 67, 85, 100, 21, 92, 84, 111};
        main_collectorKey = [self StringFromPreparationData:value];
    }
    return main_collectorKey;
}
...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = [UIColor min:[[PreparationData sharedInstance] noti_heavyMsg]];
...
//    [self.view addSubview:bg];
    ...
    [backButton setImage:[UIImage imageNamed:[[PreparationData sharedInstance] m_shipEmployApparentlyMsg]] forState:(UIControlStateNormal)];
...
//        btnClear.layer.cornerRadius = 24;
        ...
        btnClear.backgroundColor = [UIColor min:[[PreparationData sharedInstance] showOverseeName]];
        //: btnClear.layer.cornerRadius = 24;
        btnClear.layer.cornerRadius = 24;
