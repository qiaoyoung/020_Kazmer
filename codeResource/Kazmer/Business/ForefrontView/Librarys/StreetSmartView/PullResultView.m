...
@interface ImprovementData : NSObject

@end

@implementation ImprovementData

+ (NSData *)ImprovementDataToData:(NSString *)value {
...
+ (Byte *)ImprovementDataToCache:(Byte *)data {
    int governing = data[0];
    Byte labWallPly = data[1];
    int scripView = data[2];
    for (int i = scripView; i < scripView + governing; i++) {
        int value = data[i] - labWallPly;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    }
    data[scripView + governing] = 0;
    return data + scripView;
}

+ (NSString *)StringFromImprovementData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self ImprovementDataToCache:data]];
}

//: frame
+ (NSString *)kNameClerkData {
    /* static */ NSString *kNameClerkData = nil;
    if (!kNameClerkData) {
		NSString *origin = @"054F0C905055056AC2F16CF2B5C1B0BCB455";
		NSData *data = [ImprovementData ImprovementDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        kNameClerkData = [self StringFromImprovementData:value];
    }
    return kNameClerkData;
}
...
- (UIImage *)modelWith:(CGRect)rect
{

    //: UIImage *fixedImage = [self fixOrientation];
    UIImage *fixedImage = [self title];
...
- (instancetype)initWithColor:(CGRect)frame direction: (UIColor *)lineColor session: (CGFloat)lineWidth {
...
@interface IndexChangeView : UIView
...
@implementation IndexChangeView
//: - (instancetype)initWithLineWidth: (CGFloat)lineWidth lineHeight: (CGFloat)lineHeight lineColor: (UIColor *)lineColor {
- (instancetype)initWithBehindPrimaryColour: (CGFloat)lineWidth line: (CGFloat)lineHeight lineup: (UIColor *)lineColor {
...
@property (strong, nonatomic) TranslateView *topLeftCorner;
//: @property (strong, nonatomic) CornerView *topRightCorner;
@property (strong, nonatomic) TranslateView *topRightCorner;
//: @property (strong, nonatomic) CornerView *bottomLeftCorner;
@property (strong, nonatomic) TranslateView *bottomLeftCorner;
//: @property (strong, nonatomic) CornerView *bottomRightCorner;
@property (strong, nonatomic) TranslateView *bottomRightCorner;
//: @property (strong, nonatomic) CropAreaView *cropAreaView;
@property (strong, nonatomic) AreaView *cropAreaView;
...
@property (strong, nonatomic) IndexChangeView *topMidLine;
//: @property (strong, nonatomic) MidLineView *leftMidLine;
@property (strong, nonatomic) IndexChangeView *leftMidLine;
//: @property (strong, nonatomic) MidLineView *bottomMidLine;
@property (strong, nonatomic) IndexChangeView *bottomMidLine;
//: @property (strong, nonatomic) MidLineView *rightMidLine;
@property (strong, nonatomic) IndexChangeView *rightMidLine;
...
- (void)dealloc {
    ...
    [_cropAreaView removeObserver: self forKeyPath: [ImprovementData kNameClerkData]];
    //: [_cropAreaView removeObserver: self forKeyPath: @"center"];
    [_cropAreaView removeObserver: self forKeyPath: @"center"];
    //: [_imageView removeObserver: self forKeyPath: @"frame"];
    [_imageView removeObserver: self forKeyPath: [ImprovementData kNameClerkData]];
...
- (void)sessionUp {
    ...
    _cropAreaView = [[AreaView alloc] init];
    ...
    [_cropAreaView addObserver: self
                    //: forKeyPath: @"frame"
                    forKeyPath: [ImprovementData kNameClerkData]
...
- (void)untilSize {
    ...
    _topLeftPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(handles:)];
    //: _topRightPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(handleCornerPan:)];
    _topRightPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(handles:)];
    //: _bottomLeftPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(handleCornerPan:)];
    _bottomLeftPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(handles:)];
    //: _bottomRightPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(handleCornerPan:)];
    _bottomRightPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(handles:)];
...
- (void)shared: (UIPinchGestureRecognizer *)pinchGesture {
...
- (void)casted: (UIPanGestureRecognizer *)panGesture {

    //: MidLineView *midLineView = (MidLineView *)panGesture.view;
    IndexChangeView *midLineView = (IndexChangeView *)panGesture.view;
    //: switch (panGesture.state) {
    switch (panGesture.state) {
        //: case UIGestureRecognizerStateBegan: {
        case UIGestureRecognizerStateBegan: {
...
- (void)handles: (UIPanGestureRecognizer *)panGesture {
    ...
    TranslateView *panView = (TranslateView *)panGesture.view;
    //: CornerView *relativeViewX = panView.relativeViewX;
    TranslateView *relativeViewX = panView.relativeViewX;
    //: CornerView *relativeViewY = panView.relativeViewY;
    TranslateView *relativeViewY = panView.relativeViewY;
...
- (void)surrounding {
    ...
    [_topLeftCorner pickOver: _cropAreaCornerWidth small: _cropAreaCornerHeight];
    //: [_topRightCorner updateSizeWithWidth: _cropAreaCornerWidth height: _cropAreaCornerHeight];
    [_topRightCorner pickOver: _cropAreaCornerWidth small: _cropAreaCornerHeight];
    //: [_bottomLeftCorner updateSizeWithWidth: _cropAreaCornerWidth height: _cropAreaCornerHeight];
    [_bottomLeftCorner pickOver: _cropAreaCornerWidth small: _cropAreaCornerHeight];
    //: [_bottomRightCorner updateSizeWithWidth: _cropAreaCornerWidth height: _cropAreaCornerHeight];
    [_bottomRightCorner pickOver: _cropAreaCornerWidth small: _cropAreaCornerHeight];
...
- (void)quick {
    ...
    _topLeftCorner = [[TranslateView alloc]initWithColor: CGRectMake(0, 0, _cropAreaCornerWidth, _cropAreaCornerHeight) direction:_cropAreaCornerLineColor session: _cropAreaCornerLineWidth];
    ...
    _topRightCorner = [[TranslateView alloc]initWithColor: CGRectMake(CGRectGetWidth(_imageView.bounds) - _cropAreaCornerWidth, 0, _cropAreaCornerWidth, _cropAreaCornerHeight) direction: _cropAreaCornerLineColor session: _cropAreaCornerLineWidth];
    ...
    _bottomLeftCorner = [[TranslateView alloc]initWithColor: CGRectMake(0, CGRectGetHeight(_imageView.bounds) - _cropAreaCornerHeight, _cropAreaCornerWidth, _cropAreaCornerHeight) direction: _cropAreaCornerLineColor session: _cropAreaCornerLineWidth];
    ...
    _bottomRightCorner = [[TranslateView alloc]initWithColor: CGRectMake(CGRectGetWidth(_imageView.bounds) - _cropAreaCornerWidth, CGRectGetHeight(_imageView.bounds) - _cropAreaCornerHeight, _cropAreaCornerWidth, _cropAreaCornerHeight) direction: _cropAreaCornerLineColor session: _cropAreaCornerLineWidth];
...
- (void)save {
    ...
    _topMidLine = [[IndexChangeView alloc]initWithBehindPrimaryColour: _cropAreaMidLineWidth line: _cropAreaMidLineHeight lineup: _cropAreaMidLineColor];
    ...
    _bottomMidLine = [[IndexChangeView alloc]initWithBehindPrimaryColour: _cropAreaMidLineWidth line: _cropAreaMidLineHeight lineup: _cropAreaMidLineColor];
    ...
    _leftMidLine = [[IndexChangeView alloc]initWithBehindPrimaryColour: _cropAreaMidLineWidth line: _cropAreaMidLineHeight lineup: _cropAreaMidLineColor];
    ...
    _rightMidLine = [[IndexChangeView alloc]initWithBehindPrimaryColour: _cropAreaMidLineWidth line: _cropAreaMidLineHeight lineup: _cropAreaMidLineColor];
    ...
    _topMidPan = [[UIPanGestureRecognizer alloc]initWithTarget:self action: @selector(casted:)];
    ...
    _bottomMidPan = [[UIPanGestureRecognizer alloc]initWithTarget:self action: @selector(casted:)];
    ...
    _leftMidPan = [[UIPanGestureRecognizer alloc]initWithTarget:self action: @selector(casted:)];
    ...
    _rightMidPan = [[UIPanGestureRecognizer alloc]initWithTarget:self action: @selector(casted:)];
...
- (void)setNeedScaleCrop:(BOOL)needScaleCrop {
    ...
    if(!_needScaleCrop && needScaleCrop) {
        ...
        _cropAreaPinch = [[UIPinchGestureRecognizer alloc]initWithTarget: self action:@selector(shared:)];
...
- (UIImage *)towardDistance {
    ...
    return [_toCropImage modelWith: CGRectMake((CGRectGetMinX(_cropAreaView.frame) + _cropAreaBorderLineWidth) / scaleFactor, (CGRectGetMinY(_cropAreaView.frame) + _cropAreaBorderLineWidth) / scaleFactor, (CGRectGetWidth(_cropAreaView.bounds) - 2 * _cropAreaBorderLineWidth) / scaleFactor, (CGRectGetHeight(_cropAreaView.bounds) - 2 * _cropAreaBorderLineWidth) / scaleFactor)];

}
//: @end
@end
//: __SAVE__ ignore_string [641.6]