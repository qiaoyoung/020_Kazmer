...
@interface HighData : NSObject

@end

@implementation HighData

+ (Byte *)HighDataToCache:(Byte *)data {
    int brownishYellow = data[0];
    Byte literal = data[1];
    int quickRive = data[2];
    for (int i = quickRive; i < quickRive + brownishYellow; i++) {
        int value = data[i] + literal;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    }
    data[quickRive + brownishYellow] = 0;
    return data + quickRive;
}

+ (NSString *)StringFromHighData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self HighDataToCache:data]];
}

//: png
+ (NSString *)kText_retireTitle {
    /* static */ NSString *kText_retireTitle = nil;
    if (!kText_retireTitle) {
        Byte value[] = {3, 20, 3, 92, 90, 83, 151};
        kText_retireTitle = [self StringFromHighData:value];
    }
    return kText_retireTitle;
}

//: yyyy-MM-dd HH:mm:ss
+ (NSString *)kText_greetTraditionalName {
    /* static */ NSString *kText_greetTraditionalName = nil;
    if (!kText_greetTraditionalName) {
        Byte value[] = {19, 91, 11, 229, 45, 190, 169, 225, 216, 76, 44, 30, 30, 30, 30, 210, 242, 242, 210, 9, 9, 197, 237, 237, 223, 18, 18, 223, 24, 24, 15};
        kText_greetTraditionalName = [self StringFromHighData:value];
    }
    return kText_greetTraditionalName;
}

//: gif
+ (NSString *)kTitleEcosystemString {
    /* static */ NSString *kTitleEcosystemString = nil;
    if (!kTitleEcosystemString) {
        Byte value[] = {3, 92, 6, 44, 137, 1, 11, 13, 10, 45};
        kTitleEcosystemString = [self StringFromHighData:value];
    }
    return kTitleEcosystemString;
}

//: tiff
+ (NSString *)kName_riveValue {
    /* static */ NSString *kName_riveValue = nil;
    if (!kName_riveValue) {
        Byte value[] = {4, 58, 9, 172, 180, 102, 188, 151, 198, 58, 47, 44, 44, 14};
        kName_riveValue = [self StringFromHighData:value];
    }
    return kName_riveValue;
}

//: jpeg
+ (NSString *)kName_nousString {
    /* static */ NSString *kName_nousString = nil;
    if (!kName_nousString) {
        Byte value[] = {4, 39, 8, 58, 8, 58, 246, 231, 67, 73, 62, 64, 127};
        kName_nousString = [self StringFromHighData:value];
    }
    return kName_nousString;
}
...
#import "UIImage+CommentSizeLabel.h"

//: @implementation UIImage (KIAdditions)
@implementation UIImage (CommentSizeLabel)

//: - (UIImage *)resizeTo:(CGSize)size {
- (UIImage *)to:(CGSize)size {
    //: return [self resizeToWidth:size.width height:size.height];
    return [self totalry:size.width myHeight:size.height];
}
...
+ (NSString *) itemValueShared:(NSData *)data{
    ...
    switch (c) {
        ...
        case 0xFF:
            //: return @"jpeg";
            return [HighData kName_nousString];
        //: case 0x89:
        case 0x89:
            //: return @"png";
            return [HighData kText_retireTitle];
        //: case 0x47:
        case 0x47:
            //: return @"gif";
            return [HighData kTitleEcosystemString];
        //: case 0x49:
        case 0x49:
        //: case 0x4D:
        case 0x4D:
            //: return @"tiff";
            return [HighData kName_riveValue];
    }
    //: return @"";
    return @"";
}
...
- (UIImage *)holder:(BOOL)isHorizontal {
...
- (UIImage *)item {
    ...
    [df setDateFormat:[HighData kText_greetTraditionalName]];
    ...
    return [self show:dateString
               //: textColor:[UIColor blackColor]
               offSession:[UIColor blackColor]
                    //: font:[UIFont boldSystemFontOfSize:16.0f]
                    style:[UIFont boldSystemFontOfSize:16.0f]
                   //: point:CGPointMake(self.size.width-size.width-10, self.size.height-size.height-10)];
                   sawLog:CGPointMake(self.size.width-size.width-10, self.size.height-size.height-10)];
...
- (UIImage *)dimension:(CGFloat)width {
    ...
    return [self totalry:width myHeight:height];
}
...
-(NSData *)barLength:(double)maxLength{
    // Compress by quality
...
- (UIImage *)red:(CGFloat)heigh {
    ...
    return [self totalry:width myHeight:heigh];
}

//图片压缩到指定大小
//: - (UIImage*)imageCroppingForSize:(CGSize)targetSize
- (UIImage*)nameAttachSize:(CGSize)targetSize
{
    //: UIImage *sourceImage = self;
    UIImage *sourceImage = self;
    //: UIImage *newImage = nil;
...
CGFloat viewObject(CGFloat degrees) {return degrees * 3.14159265358979323846264338327950288 / 180;};
//: CGFloat KKRadiansToDegrees(CGFloat radians) {return radians * 180/3.14159265358979323846264338327950288;};
CGFloat bottomA(CGFloat radians) {return radians * 180/3.14159265358979323846264338327950288;};
...
- (UIImage *)messageIn:(CGFloat)degrees{
    ...
    CGAffineTransform t = CGAffineTransformMakeRotation(viewObject(degrees));
    ...
    CGContextRotateCTM(bitmap, viewObject(degrees));
...
- (UIImage *)searchedIn:(NSUInteger)maxLength{
    ...
    NSData *compressData = [self barLength:maxLength*1024];
