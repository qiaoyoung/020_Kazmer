...
typedef struct {
    Byte arrestSuccess;
    Byte *organicCompoundEvolve;
    unsigned int tummy;
	int employChef;
	int tailedCombination;
	int conjecture;
} StructFailureData;

@interface FailureData : NSObject

+ (instancetype)sharedInstance;

//: colors
@property (nonatomic, copy) NSString *kViewName;

//: chromateAnimate
@property (nonatomic, copy) NSString *showDropKey;
...
+ (instancetype)sharedInstance {
    static FailureData *instance = nil;
...
- (Byte *)FailureDataToByte:(StructFailureData *)data {
    for (int i = 0; i < data->tummy; i++) {
        data->organicCompoundEvolve[i] ^= data->arrestSuccess;
    }
    data->organicCompoundEvolve[data->tummy] = 0;
	if (data->tummy >= 3) {
		data->employChef = data->organicCompoundEvolve[0];
		data->tailedCombination = data->organicCompoundEvolve[1];
		data->conjecture = data->organicCompoundEvolve[2];
	}
    return data->organicCompoundEvolve;
}

- (NSString *)StringFromFailureData:(StructFailureData *)data {
    return [NSString stringWithUTF8String:(char *)[self FailureDataToByte:data]];
}

//: colors
- (NSString *)kViewName {
    if (!_kViewName) {
        StructFailureData value = (StructFailureData){140, (Byte []){239, 227, 224, 227, 254, 255, 205}, 6, 84, 17, 235};
        _kViewName = [self StringFromFailureData:&value];
    }
    return _kViewName;
}

//: chromateAnimate
- (NSString *)showDropKey {
    if (!_showDropKey) {
        StructFailureData value = (StructFailureData){78, (Byte []){45, 38, 60, 33, 35, 47, 58, 43, 15, 32, 39, 35, 47, 58, 43, 68}, 15, 235, 22, 88};
        _showDropKey = [self StringFromFailureData:&value];
    }
    return _showDropKey;
}
...
+ (UIImage *)occurrence:(UIColor *)startColor session:(UIColor *)endColor gradientType:(SNGradientDirection)directionType {
    ...
    return [self add:startColor tip:endColor bringHome:directionType openScale:CGSizeMake([[UIScreen mainScreen] bounds].size.width, (44.0f + [UIDevice tingHeight]))];
}
...
+ (UIImage *)max:(UIColor *)centerColor imageRed:(UIColor *)outColor {
    ...
    return [self app:centerColor on:outColor image:CGSizeMake([[UIScreen mainScreen] bounds].size.width, (44.0f + [UIDevice tingHeight]))];
}
...
+ (UIImage *)app:(UIColor *)centerColor on:(UIColor *)outColor image:(CGSize)size {
    ...
    [self touch:gc drawInItemColouring:path dismiss:centerColor.CGColor picture:outColor.CGColor];
    //: CGPathRelease(path);
    CGPathRelease(path);
...
+ (void)touch:(CGContextRef)context
                      //: path:(CGPathRef)path
                      drawInItemColouring:(CGPathRef)path
                //: startColor:(CGColorRef)startColor
                dismiss:(CGColorRef)startColor
                  //: endColor:(CGColorRef)endColor {
                  picture:(CGColorRef)endColor {
...
+ (void)reply:(UIView *)view {
    ...
    CAKeyframeAnimation *chromateAnimate = [self mediaIn];

    //: [chromatoLayer addAnimation:chromateAnimate forKey:@"chromateAnimate"];
    [chromatoLayer addAnimation:chromateAnimate forKey:[FailureData sharedInstance].showDropKey];
...
+ (CAKeyframeAnimation *)mediaIn {
    //: CAKeyframeAnimation *chromateAnimate = [CAKeyframeAnimation animationWithKeyPath:@"colors"];
    CAKeyframeAnimation *chromateAnimate = [CAKeyframeAnimation animationWithKeyPath:[FailureData sharedInstance].kViewName];
