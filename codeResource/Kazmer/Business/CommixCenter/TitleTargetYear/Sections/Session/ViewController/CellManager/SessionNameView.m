...
@interface SessionNameView () <TeamResponder>
...
@property (nonatomic, strong) NSMutableArray<BlendBubbleView*> *items;
...
@property (nonatomic, strong) EndowView *touchReceiver;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self = [super initWithCoder:aDecoder]) {
        ...
        _timeIntervalPerScroll = k_viewName;
        //: _timeDurationPerScroll = DEFAULT_TIME_DURATION;
        _timeDurationPerScroll = appNameData;
        //: _scrollSpeed = DEFAULT_SCROLL_SPEED;
        _scrollSpeed = kCloseMessage;
        //: _itemSpacing = DEFAULT_ITEM_SPACING;
        _itemSpacing = noti_quickValue;
        //: _touchEnabled = NO;
        _touchEnabled = NO;
...
- (void)magnituding:(NSNotification*)notification {
...
- (void)resetInsideAll {
    ...
    //: } else {
        ...
        if ([_delegate respondsToSelector:@selector(titleTool:)]) {
            //: self.visibleItemCount = [_delegate numberOfVisibleItemsForMarqueeView:self];
            self.visibleItemCount = [_delegate titleTool:self];
        ...
        //: } else {
            ...
            self.visibleItemCount = showPlayerUrl;
        }
    }
    ...
    for (int i = 0; i < _visibleItemCount + 2; i++) {
        ...
        BlendBubbleView *itemView = [[BlendBubbleView alloc] init];
    ...
    if (_direction == UUMarqueeViewDirectionLeftward) {
        ...
        for (int i = 0; i < _items.count; i++) {
            ...
            if (i == 0) {
                ...
                [self createViewInventoryItemTool:_items[index]];
            //: } else {
            } else {
                ...
                _items[index].width = [self imageIndex:_items[index].tag];
                ...
                [self size:_items[index] paradigmIndex:_items[index].tag];
            }
        }
    //: } else {
    } else {
        ...
        if (_useDynamicHeight) {
            ...
            for (int i = 0; i < _items.count; i++) {
                ...
                if (i == _items.count - 1) {
                    ...
                    _items[index].height = [self cyclorama:_items[index].tag];
        ...
        //: } else {
            ...
            if ([_delegate respondsToSelector:@selector(froming:)]) {
                //: dataCount = [_delegate numberOfDataForMarqueeView:self];
                dataCount = [_delegate froming:self];
            }
...
- (CGFloat)imageIndex:(NSInteger)index {
    //: CGFloat itemWidth = 0.0f;
    CGFloat itemWidth = 0.0f;
    //: if (index >= 0) {
    if (index >= 0) {
        //: if ([_delegate respondsToSelector:@selector(itemViewWidthAtIndex:forMarqueeView:)]) {
        if ([_delegate respondsToSelector:@selector(safetyMargin:eyeglasses:)]) {
            //: itemWidth = [_delegate itemViewWidthAtIndex:index forMarqueeView:self];
            itemWidth = [_delegate safetyMargin:index eyeglasses:self];
        }
    }
    //: return itemWidth;
    return itemWidth;
...
- (void)createViewInventoryItemTool:(BlendBubbleView*)itemView {
    //: if (!itemView.didFinishCreate) {
    if (!itemView.didFinishCreate) {
        //: if ([_delegate respondsToSelector:@selector(createItemView:forMarqueeView:)]) {
        if ([_delegate respondsToSelector:@selector(account:infoMenuView:)]) {
            //: [_delegate createItemView:itemView forMarqueeView:self];
            [_delegate account:itemView infoMenuView:self];
...
- (void)size:(BlendBubbleView*)itemView paradigmIndex:(NSInteger)index {
    //: if (index < 0) {
    if (index < 0) {
        //: [itemView clear];
        [itemView lengthNeed];
    ...
    if (index >= 0) {
        ...
        if ([_delegate respondsToSelector:@selector(load:ting:mode:)]) {
            //: [_delegate updateItemView:itemView atIndex:index forMarqueeView:self];
            [_delegate load:itemView ting:index mode:self];
        }
    }
}
...
- (void)buttons:(NSTimer *)timer {
    ...
    dispatch_async(dispatch_get_main_queue(), ^() {
        ...
        if (_direction == UUMarqueeViewDirectionLeftward) {
            ...
            _items[_firstItemIndex].width = [self imageIndex:_items[_firstItemIndex].tag];
            ...
            if (firstItemWidth != nextItemWidth) {
                ...
                [_items[_firstItemIndex] lengthNeed];
            }
            //: [self updateItemView:_items[_firstItemIndex] atIndex:_items[_firstItemIndex].tag];
            [self size:_items[_firstItemIndex] paradigmIndex:_items[_firstItemIndex].tag];
        ...
        //: } else {
            ...
            if (_useDynamicHeight) {
                ...
                _items[_firstItemIndex].height = [self cyclorama:_items[_firstItemIndex].tag];
...
- (void)inputMessage {
    ...
    if (_touchEnabled) {
        ...
        if (!_touchReceiver) {
            ...
            self.touchReceiver = [[EndowView alloc] init];
...
- (void)bulk:(CGPoint)point {
    ...
    for (BlendBubbleView *itemView in _items) {
        ...
        if ([itemView.layer.presentationLayer hitTest:point]) {
            ...
            if (dataCount > 0 && itemView.tag >= 0 && itemView.tag < dataCount) {
                ...
                if ([self.delegate respondsToSelector:@selector(lightLength:the:)]) {
                    //: [self.delegate didTouchItemViewAtIndex:itemView.tag forMarqueeView:self];
                    [self.delegate lightLength:itemView.tag the:self];
                }
            }
            //: break;
            break;
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (_touchDelegate) {
        ...
        [_touchDelegate bulk:touchLocation];
    }
}
...
@implementation BlendBubbleView
