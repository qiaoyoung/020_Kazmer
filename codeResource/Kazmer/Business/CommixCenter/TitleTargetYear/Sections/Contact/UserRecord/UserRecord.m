...
@implementation NamePair

//: - (instancetype)initWithFirst:(id)first second:(id)second {
- (instancetype)initWithTo:(id)first show:(id)second {
    //: self = [super init];
    self = [super init];
    //: if(self) {
    if(self) {
...
- (void)setMembers:(NSArray *)members {
    ...
    for (id<DisplayRow>member in members) {
    ...
    [tmp enumerateKeysAndObjectsUsingBlock:^(NSString *groupTitle, NSMutableArray *groupedMembers, BOOL *stop) {
        ...
        if (groupTitle.length) {
            ...
            [_groups addObject:[[NamePair alloc] initWithTo:groupTitle show:groupedMembers]];
        }
    //: }];
    }];
    //: [self sortGroup];
...
- (void)setSpecialMembers:(NSArray *)specialMembers {
    ...
    if (specialMembers.count > 0) {
        ...
        [_groups insertObject:[[NamePair alloc] initWithTo:@"$" show:specialMembers] atIndex:0];
        //: [self sortGroupMember];
        [self humanBeingsReply];
...
- (NSDictionary *)contentDic {
    ...
    for (int i = 0; i < _groups.count; ++i) {
        ...
        NSArray *tempArr = [self label:i];
        //: NSString *title = [self titleofGroup:i];
        NSString *title = [self should:i];
        //: [tempArr enumerateObjectsUsingBlock:^(id<NIMGroupMemberProtocol>member, NSUInteger idx, BOOL * _Nonnull stop) {
        [tempArr enumerateObjectsUsingBlock:^(id<DisplayRow>member, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NSString *)should:(NSInteger)groupIndex {
...
- (NSArray *)label:(NSInteger)groupIndex
{
    //: if(groupIndex >= 0 && groupIndex < _groups.count) {
    if(groupIndex >= 0 && groupIndex < _groups.count) {
        //: NIMKitPair *pair = [_groups objectAtIndex:groupIndex];
        NamePair *pair = [_groups objectAtIndex:groupIndex];
        //: return pair.second;
        return pair.second;
    }
    //: return nil;
...
- (void)communication
{
    ...
    [_groups sortUsingComparator:^NSComparisonResult(NamePair *pair1, NamePair *pair2) {
...
- (void)humanBeingsReply
{
    //: [_groups enumerateObjectsUsingBlock:^(NIMKitPair *obj, NSUInteger idx, BOOL *stop) {
    [_groups enumerateObjectsUsingBlock:^(NamePair *obj, NSUInteger idx, BOOL *stop) {
        ...
        [groupedMembers sortUsingComparator:^NSComparisonResult(id<DisplayRow> member1, id<DisplayRow> member2) {
            //: return _groupMemberComparator([member1 sortKey], [member2 sortKey]);
            return _groupMemberComparator([member1 overFindDown], [member2 overFindDown]);
        //: }];
        }];
    //: }];
    }];
