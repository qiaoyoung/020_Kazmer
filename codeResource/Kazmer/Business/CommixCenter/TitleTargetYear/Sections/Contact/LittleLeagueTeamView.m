...
NSString *StringFromQuestData(Byte *data);        


//: contact_head_selected
Byte kNameAnyoneValue[] = {3, 21, 72, 7, 120, 20, 48, 27, 39, 38, 44, 25, 27, 44, 23, 32, 29, 25, 28, 23, 43, 29, 36, 29, 27, 44, 29, 28, 240};

//: ic_close_group
Byte kName_labGentValue[] = {76, 14, 36, 9, 136, 45, 86, 101, 209, 69, 63, 59, 63, 72, 75, 79, 65, 59, 67, 78, 75, 81, 76, 13};
...
@implementation SessionImageView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.deleteImg.image = [UIImage imageNamed:StringFromQuestData(kName_labGentValue)];
...
@property (nonatomic, strong) SessionImageView *blankView;
...
- (void)history
{
    ...
    self.blankView = [[SessionImageView alloc]init];
    //: [self.blankView setImage:[UIImage imageNamed:@"contact_head_selected"]];
    [self.blankView setImage:[UIImage imageNamed:StringFromQuestData(kNameAnyoneValue)]];
...
- (void)auditoryImage:(SessionImageView *)view
{
    //: [view addTarget:self action:@selector(remove:) forControlEvents:UIControlEventTouchUpInside];
    [view addTarget:self action:@selector(paged:) forControlEvents:UIControlEventTouchUpInside];
    //: [self.array addObject:view];
    [self.array addObject:view];
    //: [self refreshView:RefreshType_Add];
    [self canRefresh:RefreshType_Add];
...
- (void)bubbleLanguage:(SessionImageView *)view
{
    ...
    [self canRefresh:RefreshType_Remove];
    ...
    for (NSInteger j = i; j < [self.array count]; j++) {
        ...
        SessionImageView *view = [self.array objectAtIndex:j];
...
- (void)memberInfo:(CancelTeamCorner *)info
{
    ...
    SessionImageView *avatar = [[SessionImageView alloc] initWithFrame:CGRectMake(0, 0, 35, 35)];
    //: NSURL *url = info.avatarUrlString ? [NSURL URLWithString:info.avatarUrlString] : nil;
    NSURL *url = info.avatarUrlString ? [NSURL URLWithString:info.avatarUrlString] : nil;
    //: [avatar nim_setImageWithURL:url placeholderImage:info.avatarImage options:SDWebImageRetryFailed];
    [avatar sendWith:url media:info.avatarImage doing:SDWebImageRetryFailed];
...
- (void)invisibleColor:(CancelTeamCorner *)info
{
    ...
    for (i = 0;i<[self.array count];i++) {
        ...
        SessionImageView *view = [self.array objectAtIndex:i];
        //: if ([view.userId isEqualToString:info.infoId]) {
        if ([view.userId isEqualToString:info.infoId]) {
            //: [self removeAvatarView:view];
            [self bubbleLanguage:view];
            //: break;
            break;
        }
    }
...
- (void)canRefresh:(enum RefreshType)refreshType
{
...
- (IBAction)paged:(id)sender
{
    //: NIMContactSelectAvatarView *view = (NIMContactSelectAvatarView *)sender;
    SessionImageView *view = (SessionImageView *)sender;
    //: [self.delegate removeUser:view.userId];
    [self.delegate bearDownUser:view.userId];
    //: [self removeAvatarView:view];
    [self bubbleLanguage:view];
}
...
Byte * QuestDataToCache(Byte *data) {
    int lieEver = data[0];
    int grateRake = data[1];
    Byte think = data[2];
    int mansionHouseBuilding = data[3];
    if (!lieEver) return data + mansionHouseBuilding;
    for (int i = mansionHouseBuilding; i < mansionHouseBuilding + grateRake; i++) {
        int value = data[i] + think;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[mansionHouseBuilding + grateRake] = 0;
    return data + mansionHouseBuilding;
}

NSString *StringFromQuestData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)QuestDataToCache(data)];
}
