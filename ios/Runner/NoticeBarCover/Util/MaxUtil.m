...
@implementation MaxUtil

//: + (NSString *)fileMD5:(NSString *)filepath
+ (NSString *)counterchange:(NSString *)filepath
{
    //: NSString *md5 = nil;
    NSString *md5 = nil;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filepath])
    {
        ...
        unsigned long long fileSize = [self user:filepath];
        //: if(fileSize > 200 * 1024 * 1024) {
        if(fileSize > 200 * 1024 * 1024) {
            //: md5 = [self bigFileMD5:filepath];
            md5 = [self queenSizedItem:filepath];
        //: }else {
        }else {
...
+ (void)completionFileCell:(NSString *)filepath signature:(void(^)(NSString *MD5))completion;
{
    ...
    dispatch_block_t block = ^(){
        ...
        NSString *md5 = [self counterchange:filepath];
        //: if(completion) {
        if(completion) {
            //: completion(md5);
            completion(md5);
...
+ (NSString *)queenSizedItem:(NSString *)filePath
{
...
+ (unsigned long long)user:(NSString *)filepath
{
