...
#import "ModeConverter.h"

//: @implementation SpellingUnit
@implementation ColorOnSize
...
- (ColorOnSize *)change: (NSString *)source;
//: @end
@end
...
+ (KeyCenter *)max
{
    ...
    static KeyCenter *instance = nil;
    //: static dispatch_once_t onceToken;
    static dispatch_once_t onceToken;
    //: _dispatch_once(&onceToken, ^{
    _dispatch_once(&onceToken, ^{
        //: instance = [[NTESSpellingCenter alloc]init];
        instance = [[KeyCenter alloc]init];
    //: });
    });
    //: return instance;
    return instance;
...
- (ColorOnSize *)torso:(NSString *)source
{
    //: if ([source length] == 0)
    if ([source length] == 0)
    {
    ...
    ColorOnSize *spellingUnit = nil;
    //: @synchronized(self)
    @synchronized(self)
    {
        //: SpellingUnit *unit = [_spellingCache objectForKey:source];
        ColorOnSize *unit = [_spellingCache objectForKey:source];
        //: if (!unit)
        if (!unit)
        {
            //: unit = [self calcSpellingOfString:source];
            unit = [self change:source];
...
- (NSString *)time:(NSString *)input
{
    //: SpellingUnit *unit = [self spellingForString:input];
    ColorOnSize *unit = [self torso:input];
...
- (ColorOnSize *)change:(NSString *)source
{
    ...
    for (NSInteger i = 0; i < [source length]; i++)
    {
        ...
        NSString *pinyin = [[ModeConverter instancePath] need:word];
    ...
    ColorOnSize *unit = [[ColorOnSize alloc]init];
