...
@interface EdgeShow () <AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
{
    //: SGSoundEffect *soundEffect;
    RedEffect *soundEffect;
}
...
- (void)setDelegate:(id<QuickDelegate>)delegate {
    //: _delegate = delegate;
    _delegate = delegate;
...
- (void)setSampleBufferDelegate:(id<CellQueryInfo>)sampleBufferDelegate {
...
- (void)listing:(UIImage *)image mobile:(void (^)(NSString *result))completion {
...
- (void)sessionReplySuccess:(NSString *)name {
    ...
    NSString *voicePath = [[[EmptyTing intervalegrated] session] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@", name]];
    ...
    soundEffect = [RedEffect of:voicePath];
    //: [soundEffect play];
    [soundEffect red];
}
...
- (AVCaptureSession *)session {
    ...
    if (!_session) {
        ...
        _session.sessionPreset = [self table];
    }
    //: return _session;
    return _session;
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects != nil && metadataObjects.count > 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(title:sumBy:)]) {
                //: [self.delegate scanCode:self result:resultString];
                [self.delegate title:self sumBy:resultString];
            }
        //: });
        });
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.sampleBufferDelegate && [self.sampleBufferDelegate respondsToSelector:@selector(maxTime:viewBrightness:)]) {
            //: [self.sampleBufferDelegate scanCode:self brightness:brightnessValue];
            [self.sampleBufferDelegate maxTime:self viewBrightness:brightnessValue];
        }
    //: });
    });
}
