...
static CGFloat const mainButtonFormat = 0.75;
//: static BOOL const square = NO;
static BOOL const dreamOffText = NO;
//: float IMAGE_MIN_HEIGHT = 400;
float notiRangeColorPath = 400;
//: float IMAGE_MIN_WIDTH = 400;
float notiTableUrl = 400;
...
#pragma mark CloseViewController implementation
...
- (void)viewDidLoad
{
    ...
    if (self){
        ...
        self.cropView = [[RubricView alloc] initWithFrame:view record:self.blurredBackground];
...
- (void)ranged:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(media:along:)])
    {
        //: UIImage *cropped;
        UIImage *cropped;
        //: if (self.image != nil){
        ...
        [self.delegate media:self along:cropped];
    }
...
#pragma mark BubbleImageView implementation
...
static CGFloat const noti_cellMessage = 5;
...
- (void)initSubSelect {
    ...
    self.shadeView = [[EventNameView alloc] initWithFrame:subviewFrame];
    ...
    controlPointSize = noti_cellMessage;
    ...
    topLeftPoint = [self jv:yearQuadrate(centerInView.x - initialCropAreaSize,
    ...
    bottomLeftPoint = [self jv:yearQuadrate(centerInView.x - initialCropAreaSize,
                                                                      //: centerInView.y + initialCropAreaSize,
                                                                      centerInView.y + initialCropAreaSize,
                                                                      //: controlPointSize)];
                                                                      controlPointSize)];
    ...
    bottomRightPoint = [self jv:yearQuadrate(centerInView.x + initialCropAreaSize,
    ...
    topRightPoint = [self jv:yearQuadrate(centerInView.x + initialCropAreaSize,
                                                                    //: centerInView.y - initialCropAreaSize, controlPointSize)];
                                                                    centerInView.y - initialCropAreaSize, controlPointSize)];

    //the "hole"
    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self flip];
    ...
    UIPanGestureRecognizer* dragRecognizer = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(suming:)];
    ...
    self.maskAlpha = mainButtonFormat;
...
- (BubbleImageView*)jv:(CGRect)frame {
    //: ControlPointView* point = [[ControlPointView alloc] initWithFrame:frame];
    BubbleImageView* point = [[BubbleImageView alloc] initWithFrame:frame];
    //: return point;
    return point;
}
...
- (void)unwanted:(CGPoint)topLeft selected:(CGPoint)bottomLeft top:(CGPoint)bottomRight tablePoint:(CGPoint)topRight {
...
- (void)suming:(UIPanGestureRecognizer*)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan || multiDragPoint.lastCount != count) {
        ...
        if (count > 1)
            ...
            [self pull:recognizer keyPrep:count];
        //: else
        else
            //: [self prepSingleTouchPan:recognizer];
            [self tutorial:recognizer];
    ...
    if (count > 1) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            [self label:[recognizer locationOfTouch:i inView:self] data:(i == 0 ? dragViewOne : dragViewTwo)];
    ...
    //: } else {
        ...
        [self label:[recognizer locationInView:self] data:dragViewOne];
    }
...
- (void)pull:(UIPanGestureRecognizer*)recognizer keyPrep:(NSUInteger)count
{
    //: for (int i = 0; i < count; i++) {
    for (int i = 0; i < count; i++) {
        //: if (i == 0) {
        if (i == 0) {
            ...
            dragViewOne = [self volition:[recognizer locationOfTouch:i inView:self]];
        ...
        //: } else {
            ...
            dragViewTwo = [self volition:[recognizer locationOfTouch:i inView:self]];
...
- (void)tutorial:(UIPanGestureRecognizer*)recognizer
{
    //: dragViewOne = [self checkHit:[recognizer locationInView:self]];
    dragViewOne = [self volition:[recognizer locationInView:self]];
...
- (void)setMessageViews:(CGRect)cropArea
{
...
- (void)label:(CGPoint)location data:(UIView*)view
{
    ...
    //: } else if (view == cropAreaView) {
        ...
        [self handleWith:location];
    }

    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self flip];
    //: [self setCropAreaForViews:cropArea];
    [self setMessageViews:cropArea];
}
...
- (CGSize)lifelessPoint:(CGPoint)dragLocation exceptWith:(CGPoint)draggedPoint someRandom:(CGPoint)oppositePoint {
...
- (void)message:(CGPoint)dragLocation {
    ...
    CGSize disp = [self lifelessPoint:dragLocation exceptWith:dragPoint.topLeftCenter someRandom:dragPoint.bottomRightCenter];
    ...
    if (width >= notiTableUrl && height < notiRangeColorPath) {
        //: topLeft.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - IMAGE_MIN_HEIGHT) / self.imageScale);
        topLeft.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - notiRangeColorPath) / self.imageScale);
        //: topRight.y = topLeft.y;
        topRight.y = topLeft.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < notiTableUrl && height >= notiRangeColorPath) {
        //: topLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - IMAGE_MIN_WIDTH) / self.imageScale);
        topLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - notiTableUrl) / self.imageScale);
    ...
    [self unwanted:topLeft selected:bottomLeft top:dragPoint.bottomRightCenter tablePoint:topRight];
}
//: - (void)handleDragBottomLeft:(CGPoint)dragLocation {
- (void)style:(CGPoint)dragLocation {
    ...
    CGSize disp = [self lifelessPoint:dragLocation exceptWith:dragPoint.bottomLeftCenter someRandom:dragPoint.topRightCenter];
    ...
    if (width >= notiTableUrl && height < notiRangeColorPath) {
        ...
        bottomLeft.y = cropArea.origin.y + (notiRangeColorPath / self.imageScale);
        //: bottomRight.y = bottomLeft.y;
        bottomRight.y = bottomLeft.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < notiTableUrl && height >= notiRangeColorPath) {
        //: bottomLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - IMAGE_MIN_WIDTH) / self.imageScale);
        bottomLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - notiTableUrl) / self.imageScale);
    ...
    [self unwanted:topLeft selected:bottomLeft top:bottomRight tablePoint:dragPoint.topRightCenter];
}

//: - (void)handleDragBottomRight:(CGPoint)dragLocation {
- (void)from:(CGPoint)dragLocation {
    ...
    CGSize disp = [self lifelessPoint:dragLocation exceptWith:dragPoint.bottomRightCenter someRandom:dragPoint.topLeftCenter];
    ...
    if (width >= notiTableUrl && height < notiRangeColorPath) {
        ...
        bottomRight.y = cropArea.origin.y + (notiRangeColorPath / self.imageScale);
        //: bottomLeft.y = bottomRight.y;
        bottomLeft.y = bottomRight.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < notiTableUrl && height >= notiRangeColorPath) {
        //: bottomRight.x = cropArea.origin.x + (IMAGE_MIN_WIDTH / self.imageScale);
        bottomRight.x = cropArea.origin.x + (notiTableUrl / self.imageScale);
    ...
    [self unwanted:dragPoint.topLeftCenter selected:bottomLeft top:bottomRight tablePoint:topRight];
}

//: - (void)handleDragTopRight:(CGPoint)dragLocation {
- (void)pin:(CGPoint)dragLocation {
    ...
    CGSize disp = [self lifelessPoint:dragLocation exceptWith:dragPoint.topRightCenter someRandom:dragPoint.bottomLeftCenter];
    ...
    if (width >= notiTableUrl && height < notiRangeColorPath) {
        ...
        topRight.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - notiRangeColorPath) / self.imageScale);
        //: topLeft.y = topRight.y;
        topLeft.y = topRight.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < notiTableUrl && height >= notiRangeColorPath) {
        //: topRight.x = cropArea.origin.x + (IMAGE_MIN_WIDTH / self.imageScale);
        topRight.x = cropArea.origin.x + (notiTableUrl / self.imageScale);
    ...
    [self unwanted:topLeft selected:dragPoint.bottomLeftCenter top:bottomRight tablePoint:topRight];
}

//: - (void)handleDragCropArea:(CGPoint)dragLocation {
- (void)handleWith:(CGPoint)dragLocation {
    //: CGFloat dX = dragLocation.x - dragPoint.dragStart.x;
    CGFloat dX = dragLocation.x - dragPoint.dragStart.x;
    //: CGFloat dY = dragLocation.y - dragPoint.dragStart.y;
    CGFloat dY = dragLocation.y - dragPoint.dragStart.y;
...
- (void)setImage:(UIImage *)image {
    ...
    if (self.blurred) {
        ...
        blur = [image course:30 with:1 top:[UIColor blackColor]];
    //: } else {
    } else {
        //: blur = [image blurredImageWithRadius:0 iterations:1 tintColor:[UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1.0]];
        blur = [image course:0 with:1 top:[UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1.0]];
    }
    ...
    [self unwanted:topLeftPoint.center selected:bottomLeftPoint.center top:bottomRightPoint.center tablePoint:topRightPoint.center];
    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self flip];
    //: cropAreaView.frame = cropArea;
    cropAreaView.frame = cropArea;
