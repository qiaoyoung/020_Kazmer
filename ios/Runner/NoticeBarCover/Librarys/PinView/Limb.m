...
typedef struct {
    Byte paradigm;
    Byte *electronicDevice;
    unsigned int pal;
	int chemicalElement;
} StructPlasticCrushData;

@interface PlasticCrushData : NSObject

+ (instancetype)sharedInstance;

//: backgroundColor
@property (nonatomic, copy) NSString *k_meaningQuireStr;

//: position
@property (nonatomic, copy) NSString *userUglyId;

//: linear
@property (nonatomic, copy) NSString *dreamMediumCloudKey;

//: bounds
@property (nonatomic, copy) NSString *noti_trainValue;

//: blurRadius
@property (nonatomic, copy) NSString *appAccordingFormat;

@end

@implementation PlasticCrushData

+ (instancetype)sharedInstance {
    static PlasticCrushData *instance = nil;
...
- (Byte *)PlasticCrushDataToByte:(StructPlasticCrushData *)data {
    for (int i = 0; i < data->pal; i++) {
        data->electronicDevice[i] ^= data->paradigm;
    }
    data->electronicDevice[data->pal] = 0;
	if (data->pal >= 1) {
		data->chemicalElement = data->electronicDevice[0];
	}
    return data->electronicDevice;
}

- (NSString *)StringFromPlasticCrushData:(StructPlasticCrushData *)data {
    return [NSString stringWithUTF8String:(char *)[self PlasticCrushDataToByte:data]];
}

//: bounds
- (NSString *)noti_trainValue {
    if (!_noti_trainValue) {
        StructPlasticCrushData value = (StructPlasticCrushData){133, (Byte []){231, 234, 240, 235, 225, 246, 248}, 6, 111};
        _noti_trainValue = [self StringFromPlasticCrushData:&value];
    }
    return _noti_trainValue;
}

//: linear
- (NSString *)dreamMediumCloudKey {
    if (!_dreamMediumCloudKey) {
        StructPlasticCrushData value = (StructPlasticCrushData){119, (Byte []){27, 30, 25, 18, 22, 5, 117}, 6, 238};
        _dreamMediumCloudKey = [self StringFromPlasticCrushData:&value];
    }
    return _dreamMediumCloudKey;
}

//: backgroundColor
- (NSString *)k_meaningQuireStr {
    if (!_k_meaningQuireStr) {
        StructPlasticCrushData value = (StructPlasticCrushData){177, (Byte []){211, 208, 210, 218, 214, 195, 222, 196, 223, 213, 242, 222, 221, 222, 195, 5}, 15, 103};
        _k_meaningQuireStr = [self StringFromPlasticCrushData:&value];
    }
    return _k_meaningQuireStr;
}

//: position
- (NSString *)userUglyId {
    if (!_userUglyId) {
        StructPlasticCrushData value = (StructPlasticCrushData){213, (Byte []){165, 186, 166, 188, 161, 188, 186, 187, 176}, 8, 35};
        _userUglyId = [self StringFromPlasticCrushData:&value];
    }
    return _userUglyId;
}

//: blurRadius
- (NSString *)appAccordingFormat {
    if (!_appAccordingFormat) {
        StructPlasticCrushData value = (StructPlasticCrushData){40, (Byte []){74, 68, 93, 90, 122, 73, 76, 65, 93, 91, 248}, 10, 248};
        _appAccordingFormat = [self StringFromPlasticCrushData:&value];
    }
    return _appAccordingFormat;
}
...
//
//  https://github.com/nicklockwood/Limb
//
...
@implementation UIImage (Limb)

//: - (UIImage *)blurredImageWithRadius:(CGFloat)radius iterations:(NSUInteger)iterations tintColor:(UIColor *)tintColor
- (UIImage *)course:(CGFloat)radius with:(NSUInteger)iterations top:(UIColor *)tintColor
{
...
@interface CellOptionScheduler : NSObject
...
+ (BOOL)needsDisplayForKey:(NSString *)key
{
    ...
    if ([@[[PlasticCrushData sharedInstance].appAccordingFormat, [PlasticCrushData sharedInstance].noti_trainValue, [PlasticCrushData sharedInstance].userUglyId] containsObject:key])
    {
        //: return YES;
        return YES;
    }
...
@implementation CellOptionScheduler

//: + (instancetype)sharedInstance
+ (instancetype)barConvert
{
    //: static FXBlurScheduler *sharedInstance = nil;
    static CellOptionScheduler *sharedInstance = nil;
    //: if (!sharedInstance)
    if (!sharedInstance)
    {
        //: sharedInstance = [[FXBlurScheduler alloc] init];
        sharedInstance = [[CellOptionScheduler alloc] init];
    }
...
- (void)inputSend
{
    ...
    if (self.blurEnabled && !self.updating && self.updatesEnabled > 0 && [self.views count])
    {
        ...
        for (NSUInteger i = self.viewIndex; i < [self.views count]; i++)
        {
            ...
            Limb *view = self.views[i];
            //: if (view.dynamic && !view.hidden && view.window && [view shouldUpdate])
            if (view.dynamic && !view.hidden && view.window && [view disable])
            {
        ...
        [self performSelector:@selector(inputSend)
                   //: withObject:nil
                   withObject:nil
...
+ (void)setBlurEnabled:(BOOL)blurEnabled
{
    ...
    [CellOptionScheduler barConvert].blurEnabled = blurEnabled;
}
...
+ (void)can
{
    ...
    [[CellOptionScheduler barConvert] by];
}
...
+ (void)disable
{
    ...
    [[CellOptionScheduler barConvert] disabled];
}
...
- (void)up
{
    ...
    if (!_blurRadiusSet) [self inputLine].blurRadius = 40;
    ...
    self.layer.magnificationFilter = [PlasticCrushData sharedInstance].dreamMediumCloudKey; // kCAFilterLinear
...
- (void)setBlurRadius:(CGFloat)blurRadius
{
    ...
    [self inputLine].blurRadius = blurRadius;
}
...
- (CGFloat)blurRadius
{
    ...
    return [self inputLine].blurRadius;
}
...
- (ShowLayer *)mediaTool
{
    ...
    ShowLayer *blurLayer = [self inputLine];
    //: return (FXBlurLayer *)blurLayer.presentationLayer ?: blurLayer;
    return (ShowLayer *)blurLayer.presentationLayer ?: blurLayer;
}
...
- (void)substitution
{
    ...
    if (self.window && self.dynamic && self.blurEnabled)
    {
        ...
        [[CellOptionScheduler barConvert] on:self];
    }
    //: else
    else
    {
        //: [[FXBlurScheduler sharedInstance] removeView:self];
        [[CellOptionScheduler barConvert] disable:self];
    }
}
...
- (BOOL)disable
{
    ...
    __strong CALayer *underlyingLayer = [self inexplicit];
    ...
    self.blurEnabled && [CellOptionScheduler barConvert].blurEnabled &&
...
- (id<CAAction>)actionForLayer:(CALayer *)layer forKey:(NSString *)key
{
    ...
    if ([key isEqualToString:[PlasticCrushData sharedInstance].appAccordingFormat])
    {
        //animations are enabled
        //: CAAnimation *action = (CAAnimation *)[super actionForLayer:layer forKey:@"backgroundColor"];
        CAAnimation *action = (CAAnimation *)[super actionForLayer:layer forKey:[PlasticCrushData sharedInstance].k_meaningQuireStr];
...
- (UIImage *)toolView
{
    ...
    __strong ShowLayer *blurLayer = [self mediaTool];
    //: __strong CALayer *underlyingLayer = [self underlyingLayer];
    __strong CALayer *underlyingLayer = [self inexplicit];
    ...
    NSArray *hiddenViews = [self unwantedSize];
    //: [underlyingLayer renderInContext:context];
    [underlyingLayer renderInContext:context];
    //: [self restoreSuperviewAfterSnapshot:hiddenViews];
    [self show:hiddenViews];
...
- (NSArray *)unwantedSize
{
    //: __strong CALayer *blurlayer = [self blurLayer];
    __strong CALayer *blurlayer = [self inputLine];
    //: __strong CALayer *underlyingLayer = [self underlyingLayer];
    __strong CALayer *underlyingLayer = [self inexplicit];
...
- (void)show:(NSArray *)hiddenLayers
{
...
- (UIImage *)green:(UIImage *)snapshot shared:(CGFloat)blurRadius
{
    //: return [snapshot blurredImageWithRadius:blurRadius
    return [snapshot course:blurRadius
...
- (void)setQuestionNames:(UIImage *)image
{
...
- (void)send:(BOOL)async need:(void (^)())completion
{
    //: if ([self shouldUpdate])
    if ([self disable])
    {
        //: UIImage *snapshot = [self snapshotOfUnderlyingView];
        UIImage *snapshot = [self toolView];
        //: if (async)
        if (async)
        {
            ...
            dispatch_async(dispatch_get_global_queue((-2), 0), ^{
                ...
                UIImage *blurredImage = [self green:snapshot shared:self.blurRadius];
                //: dispatch_sync(dispatch_get_main_queue(), ^{
                dispatch_sync(dispatch_get_main_queue(), ^{

                    //: [self setLayerContents:blurredImage];
                    [self setQuestionNames:blurredImage];
        ...
        else
        {
            ...
            [self setQuestionNames:[self green:snapshot shared:[self mediaTool].blurRadius]];
            //: if (completion) completion();
            if (completion) completion();
        }
    }
