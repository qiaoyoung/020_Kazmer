...
// __CLOSE_PRINT__
//
//  UINavigationController+TopPath.m
//  NIM
//
...
#import "UINavigationController+TopPath.h"
...
@implementation UINavigationController (TopPath)
...
+ (void)load{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        swizzling_exchangeMethod([UINavigationController class] ,@selector(supportedInterfaceOrientations), @selector(messageFor));
        //: swizzling_exchangeMethod([UINavigationController class] ,@selector(shouldAutorotate), @selector(swizzling_shouldAutorotate));
        swizzling_exchangeMethod([UINavigationController class] ,@selector(shouldAutorotate), @selector(snubInsert));
    //: });
    });
}
...
- (UIInterfaceOrientationMask)messageFor{
