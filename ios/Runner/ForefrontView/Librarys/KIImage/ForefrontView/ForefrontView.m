...
- (void)setFrame:(CGRect)frame {
    ...
    if (__CGSizeEqualToSize(_cropSize, CGSizeZero)) {
        ...
        [self setShow:CGSizeMake(100, 100)];
    }
}

//: - (UIScrollView *)scrollView {
- (UIScrollView *)progressView {
    //: if (_scrollView == nil) {
    if (_scrollView == nil) {
...
- (UIImageView *)value {
    ...
    if (_imageView == nil) {
        ...
        [[self progressView] addSubview:_imageView];
    }
    //: return _imageView;
    return _imageView;
}
...
- (void)counterval {
    ...
    [[self value] setFrame:CGRectMake(0, 0, width, height)];
...
//    }
    ...
    [[self progressView] setMinimumZoomScale:min];
    //: [[self scrollView] setMaximumZoomScale:max + 5.0f];
    [[self progressView] setMaximumZoomScale:max + 5.0f];

    //: [[self scrollView] setZoomScale:min animated:YES];
    [[self progressView] setZoomScale:min animated:YES];
}

//: - (void)setCropSize:(CGSize)size {
- (void)setShow:(CGSize)size {
    //: _cropSize = size;
    _cropSize = size;
    //: [self updateZoomScale];
    [self counterval];
    ...
    [(MaskView *)[self maskView] setMisplace:_cropSize];
    ...
    [[self progressView] setContentInset:_imageInset];

    //: [[self scrollView] setContentOffset:CGPointMake(0, 0)];
    [[self progressView] setContentOffset:CGPointMake(0, 0)];
}

//: - (UIImage *)cropImage {
- (UIImage *)openNeed {
    //: CGFloat zoomScale = [self scrollView].zoomScale;
    CGFloat zoomScale = [self progressView].zoomScale;

    //: CGFloat offsetX = [self scrollView].contentOffset.x;
    CGFloat offsetX = [self progressView].contentOffset.x;
    //: CGFloat offsetY = [self scrollView].contentOffset.y;
    CGFloat offsetY = [self progressView].contentOffset.y;
    ...
    image = [image totalry:resizeWidth myHeight:resizeheight];
