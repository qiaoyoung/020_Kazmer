...
@interface RowSessionView : UIView {
//: @private
@private
    //: CGRect _maskRect;
    CGRect _maskRect;
...
- (void)setHorizonHolder:(CGSize)size;
//: - (CGSize)maskSize;
- (CGSize)horizonHolder;
//: @end
@end

//: @interface KIImageCropperView() {
@interface ValueImageView() {
    //: KIImageCropperMaskView *_maskView;
    RowSessionView *_maskView;
...
- (RowSessionView *)maskView {
    //: if (_maskView == nil) {
    if (_maskView == nil) {
        //: _maskView = [[KIImageCropperMaskView alloc] initWithFrame:self.bounds];
        _maskView = [[RowSessionView alloc] initWithFrame:self.bounds];
...
- (UIImageView *)createView {
    ...
    if (_imageView == nil) {
        ...
        [[self off] addSubview:_imageView];
    }
    //: return _imageView;
    return _imageView;
}
...
- (void)sound:(KIImageCropRatio)ratio {
...
- (void)setImage:(UIImage *)image {
    ...
    if (_image != image) {
        ...
        [[self createView] setImage:_image];

        //: [[self imageView] setFrame:CGRectMake(0, 0, _image.size.width, _image.size.height)];
        [[self createView] setFrame:CGRectMake(0, 0, _image.size.width, _image.size.height)];
...
- (void)cross {
    ...
    [[self off] setMinimumZoomScale:min];
    //: [[self scrollView] setMaximumZoomScale:max+5.0f];
    [[self off] setMaximumZoomScale:max+5.0f];

    //: [[self scrollView] setZoomScale:min animated:YES];
    [[self off] setZoomScale:min animated:YES];

    //: [[self scrollView] setContentInset:_imageViewInsets];
    [[self off] setContentInset:_imageViewInsets];
    //: [[self scrollView] setContentOffset:CGPointMake(-_imageViewInsets.left, -_imageViewInsets.top) animated:YES];
    [[self off] setContentOffset:CGPointMake(-_imageViewInsets.left, -_imageViewInsets.top) animated:YES];
}
...
- (void)layoutSubviews {
    ...
    [(RowSessionView*)[self maskView] setHorizonHolder:[self readClean]];
    //: [[self scrollView] setFrame:self.bounds];
    [[self off] setFrame:self.bounds];
...
- (UIImage *)recordDate {
    ...
    CGFloat zoomScale = [self off].zoomScale;

    //: CGFloat offsetX = [self scrollView].contentOffset.x;
    CGFloat offsetX = [self off].contentOffset.x;
    //: CGFloat offsetY = [self scrollView].contentOffset.y;
    CGFloat offsetY = [self off].contentOffset.y;
    ...
    UIImage *image = [_image model:aX filename:aY representation:aWidth netFloat:aHeight];
    ...
    image = [self border:image tapShow:imgSize imageSessionOpen:YES];
...
- (UIImage*)border:(UIImage*)image tapShow:(CGSize)newSize imageSessionOpen:(BOOL)opaque
{
