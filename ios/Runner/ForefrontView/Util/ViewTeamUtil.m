...
@implementation ViewTeamUtil

//: + (NSString *)fileMD5:(NSString *)filepath
+ (NSString *)from:(NSString *)filepath
{
    //: NSString *md5 = nil;
    NSString *md5 = nil;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filepath])
    {
        ...
        unsigned long long fileSize = [self alongSize:filepath];
        //: if(fileSize > 200 * 1024 * 1024) {
        if(fileSize > 200 * 1024 * 1024) {
            //: md5 = [self bigFileMD5:filepath];
            md5 = [self labelContent:filepath];
        //: }else {
        }else {
...
+ (unsigned long long)alongSize:(NSString *)filepath
{
...
+ (void)praise:(NSString *)filepath kit:(void(^)(NSString *MD5))completion;
{
    //: dispatch_block_t block = ^(){
    dispatch_block_t block = ^(){
        //: NSString *md5 = [self fileMD5:filepath];
        NSString *md5 = [self from:filepath];
        //: if(completion) {
        if(completion) {
            //: completion(md5);
            completion(md5);
...
+ (NSString *)labelContent:(NSString *)filePath
{
