...
@interface FasteningControl : UIButton
...
- (void)consumerAuto
{
    ...
    [self towardKey:self pressMore:self.model.contentViewInsets];

    //: UIEdgeInsets contentInsets = self.model.contentViewInsets;
    UIEdgeInsets contentInsets = self.model.contentViewInsets;
    //: CGFloat top = [NTESSessionRobotContentView itemSpacing] + contentInsets.top;
    CGFloat top = [RedComment itemSpacing] + contentInsets.top;
    ...
    for (UIView *view in self.subviews)
    {
        ...
        top = view.bottom + [RedComment itemSpacing];
    }
}


//: - (void)recycleAllSubViews:(UIView *)view
- (void)title:(UIView *)view
{
    ...
    for (UIView *subView in view.subviews)
    {
        ...
        if ([subView isKindOfClass:[FasteningControl class]])
        {
            //: NTESSessionRobotButton *btn = (NTESSessionRobotButton *)subView;
            FasteningControl *btn = (FasteningControl *)subView;
            //: btn.target = nil;
            btn.target = nil;
            //: btn.url = nil;
            btn.url = nil;
...
- (void)toeButton:(FasteningControl *)button
{
    ...
    else
    {
        ...
        for (LabelNameScrollView *label in button.subviews)
        {
            //: if ([label isKindOfClass:[M80AttributedLabel class]])
            if ([label isKindOfClass:[LabelNameScrollView class]])
            {
...
- (UIImage *)subSessionDetect:(UIControlState)state domain:(BOOL)outgoing
{
    //: return nil;
    return nil;
}
...
- (void)keepsake:(NIMMessageModel *)data
{
    //: [super refresh:data];
    [super refresh:data];
    //: [self setupRobot:data];
    [self languagePicture:data];
}
...
- (FasteningControl *)of
{
    ...
    else
    {
        ...
        button = [[FasteningControl alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
...
- (CGSize)color:(CGSize)size
{
    //: CGFloat height = [NTESSessionRobotContentView itemSpacing];
    CGFloat height = [RedComment itemSpacing];
    //: CGFloat width = 0;;
    CGFloat width = 0;;
    //: for (UIView *view in self.subviews) {
    for (UIView *view in self.subviews) {
        ...
        if (view == self.continueButton)
        {
            ...
            height += [RedComment continueItemSpacing];
        }
        //: else
        else
        {
            //: height += [NTESSessionRobotContentView itemSpacing];
            height += [RedComment itemSpacing];
        }
    }
    //: UIEdgeInsets contentInsets = self.model.contentViewInsets;
    UIEdgeInsets contentInsets = self.model.contentViewInsets;
...
- (LabelNameScrollView *)item
{
    //: M80AttributedLabel *label = nil;
    LabelNameScrollView *label = nil;
    ...
    else
    {
        ...
        label = [[LabelNameScrollView alloc] initWithFrame:CGRectZero];
...
- (void)max:(NIMKitRobotTemplateItem *)item
           //: inView:(UIView *)view
           straddleLabel:(UIView *)view
{
    ...
    switch (item.itemType) {
        ...
        case NIMKitRobotTemplateItemTypeText:
        {
            ...
            LabelNameScrollView *label = [self genLabel];
        ...
        case NIMKitRobotTemplateItemTypeLinkBlock:
        {
            ...
            FasteningControl *button = [self of];
...
- (void)towardKey:(UIView *)superView pressMore:(UIEdgeInsets)insets
{
    ...
    for (UIView *subView in superView.subviews)
    {
        ...
        if (subView.height == 0)
        {
            ...
            if ([subView isKindOfClass:[LabelNameScrollView class]])
            {
                //: M80AttributedLabel *label = (M80AttributedLabel *)subView;
                LabelNameScrollView *label = (LabelNameScrollView *)subView;
            ...
            else if ([subView isKindOfClass:[FasteningControl class]])
            {
                //: NTESSessionRobotButton *button = (NTESSessionRobotButton *)subView;
                FasteningControl *button = (FasteningControl *)subView;
                //: button.width = width;
                button.width = width;
...
@implementation FasteningControl
