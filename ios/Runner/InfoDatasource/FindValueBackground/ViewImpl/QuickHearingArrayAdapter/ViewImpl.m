...
- (PinResult *)nameUp:(NSArray *)models
{
    ...
    NSArray *indexpaths = [self.dataSource searchModels:models];
    //: NIMSessionMessageOperateResult *result = [[NIMSessionMessageOperateResult alloc] init];
    PinResult *result = [[PinResult alloc] init];
...
- (PinResult *)notice:(NSArray *)models
{
    ...
    NSArray *indexpaths = [self.dataSource message:models];
    //: NIMSessionMessageOperateResult *result = [[NIMSessionMessageOperateResult alloc] init];
    PinResult *result = [[PinResult alloc] init];
    //: result.indexpaths = indexpaths;
    result.indexpaths = indexpaths;
...
- (PinResult *)clean:(CentralProcessingUnitModel *)model
{
    ...
    NSArray *indexs = [self.dataSource onModel:model];
    //: NIMSessionMessageOperateResult *result = [[NIMSessionMessageOperateResult alloc] init];
    PinResult *result = [[PinResult alloc] init];
...
- (PinResult *)anTheoreticalAccount:(CentralProcessingUnitModel *)model
{
    ...
    NSInteger index = [self.dataSource title:model];
...
- (NSInteger)current:(CentralProcessingUnitModel *)model
{
    ...
    return [self.dataSource title:model];
}
...
- (void)trademark:(void(^)(NSError *error, NSArray *))handler
{
    ...
    [self.dataSource dateBar:handler];
}
...
- (void)outTable:(CentralProcessingUnitModel *)model
{
    ...
    [self.dataSource barTagModel:model];
}
...
- (void)more:(NIMMessage *)message message:(void (^)(NSError *))handler
{
    ...
    [self.dataSource phoneMessageCallback:message exist:handler];
}
...
- (void)removeWithinStyle:(NIMMessage *)message immobilize:(void (^)(NSError *))handler
{
    ...
    [self.dataSource prolusion:message domain:handler];
}
        ...
        if ([item isKindOfClass:[CentralProcessingUnitModel class]]) {
            ...
            if (message.isOutgoingMsg) {
                ...
                if (!findLastReceipt) {
                    ...
                    if (message.isRemoteRead && hasConfig && [self.sessionConfig tabling:message])
                    {
...
- (NSDictionary *)by:(NSArray<NIMMessageReceipt *> *)receipts
{
        ...
        if ([item isKindOfClass:[CentralProcessingUnitModel class]])
        {
            ...
            if (message.isOutgoingMsg)
            {
                ...
                if (message.setting.teamReceiptEnabled &&
                    ...
                    [self.sessionConfig tabling:message])
                {
                    //: model.shouldShowReadLabel = YES;
                    model.shouldShowReadLabel = YES;
                    //: dict[@(i)] = model;
    ...
    for (NSInteger i = [messages count] - 1; i >= 0; i--) {
        ...
        if (message)
        {
            ...
            if (!message.isOutgoingMsg &&
                ...
                [self.sessionConfig respondsToSelector:@selector(tabling:)] &&
                //: [self.sessionConfig shouldHandleReceiptForMessage:message])
                [self.sessionConfig tabling:message])
            {
