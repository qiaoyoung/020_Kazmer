...
+ (instancetype)sharedInstance {
    static CompoundData *instance = nil;
...
- (Byte *)CompoundDataToCache:(Byte *)data {
    int racial = data[0];
    Byte centerSpecific = data[1];
    int ovaCrowded = data[2];
    for (int i = ovaCrowded; i < ovaCrowded + racial; i++) {
        int value = data[i] - centerSpecific;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    }
    data[ovaCrowded + racial] = 0;
    return data + ovaCrowded;
}
...
//: click
- (NSString *)appApproximatelyTitle {
    /* static */ NSString *appApproximatelyTitle = nil;
    if (!appApproximatelyTitle) {
        Byte value[] = {5, 56, 6, 169, 59, 24, 155, 164, 161, 155, 163, 204};
        appApproximatelyTitle = [self StringFromCompoundData:value];
    }
    return appApproximatelyTitle;
}

//: speaker
- (NSString *)main_mealMsg {
    /* static */ NSString *main_mealMsg = nil;
    if (!main_mealMsg) {
        Byte value[] = {7, 10, 12, 246, 36, 208, 16, 199, 101, 255, 10, 166, 125, 122, 111, 107, 117, 111, 124, 220};
        main_mealMsg = [self StringFromCompoundData:value];
    }
    return main_mealMsg;
}
...
//: lead_close
- (NSString *)m_methId {
    /* static */ NSString *m_methId = nil;
    if (!m_methId) {
        Byte value[] = {10, 8, 13, 215, 15, 36, 113, 227, 195, 192, 241, 63, 84, 116, 109, 105, 108, 103, 107, 116, 119, 123, 109, 39};
        m_methId = [self StringFromCompoundData:value];
    }
    return m_methId;
}
...
static NSAttributedString *libraryTitle(__unsafe_unretained NSString *title)
{
    //: if (nil == title) {
    if (nil == title) {
        //: return nil;
...
@interface ColumnView () <ViewTimeDelegate>
...
CGFloat videoObject(void) {
    //: return SNStatusBarHeight() + 44;
    return squareBarHeight() + 44;
}
...
+ (instancetype)after:(float)delay
    ...
    ColumnView *tipView = [[ColumnView alloc] initWithFrame:(CGRect) {
        //: .origin.x = 5,
        .origin.x = 5,
        //: .origin.y = (44.0f + [UIDevice vg_statusBarHeight])+5,
        .origin.y = (44.0f + [UIDevice tingHeight])+5,
    ...
    [tipView threadBar:^(ColumnView *tipView) {
        ...
        switch (type) {
            ...
            case CompletingUserInfoType_headicon:
                ...
                tipView.actionTitle = [[CompoundData sharedInstance] appApproximatelyTitle];
                //: break;
                break;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.userInteractionEnabled = YES;
        {
            ...
            self.leftwardMarqueeView = [[BubbleEmptyView alloc] initWithFramework:CGRectMake(54, 0, [UIScreen mainScreen].bounds.size.width- 54 - 30 - 10, 46) quick:UUMarqueeViewDirectionLeftward];
...
//            button.backgroundColor = [UIColor redColor];
            ...
            [button addTarget:self action:@selector(actionned:) forControlEvents:UIControlEventTouchUpInside];
//            [button.titleLabel setFont:[UIFont systemFontOfSize:14]];
...

        {
            ...
            noticeImageview.image = [UIImage imageNamed:[[CompoundData sharedInstance] main_mealMsg]];
...
- (void)threadBar:(void (^)(ColumnView *tipView))transactionBlock
{
    ...
    [_actionButton setImage:[UIImage imageNamed:[[CompoundData sharedInstance] m_methId]] forState:(UIControlStateNormal)];
    ...
    [_leftwardMarqueeView file];
    //: [_leftwardMarqueeView start];
    [_leftwardMarqueeView detectShouldState];
}
...
- (void)container:(UIView *)superView
{
    ...
    [UIView animateWithDuration:0.5 delay:0.3 options:UIViewAnimationOptionCurveLinear animations:^{
        ...
        frame.origin.y = squareBarHeight() + 44 + 5;
        //: self.frame = frame;
        self.frame = frame;
    //: } completion:nil];
    } completion:nil];
...
- (void)pin:(BOOL)callback
{
    //: [self.leftwardMarqueeView pause];
    [self.leftwardMarqueeView mediaDomain];
    ...
    if (!self.hidden && nil != self.superview) {
        ...
        [UIView animateWithDuration:0.25 delay:0 usingSpringWithDamping:0.90 initialSpringVelocity:5 options:UIViewAnimationOptionCurveEaseOut animations:^{
            ...
            frame.origin.y = squareBarHeight();
            //: self.frame = frame;
            self.frame = frame;
...
- (void)actionned:(UIButton *)button {
    //: [self p_dismissWith:NO];
    [self pin:NO];
}
...
- (NSUInteger)nameView:(BubbleEmptyView*)marqueeView {
    //: return 1;
    return 1;
}
...
- (CGFloat)blueAppInfo:(NSUInteger)index placeKey:(BubbleEmptyView*)marqueeView {
...
- (CGFloat)anMenuLabel:(NSUInteger)index handle:(BubbleEmptyView*)marqueeView {
    // for leftwardMarqueeView
    //: UILabel *content = [[UILabel alloc] init];
    UILabel *content = [[UILabel alloc] init];
    //: content.font = [UIFont systemFontOfSize:13.0f];
