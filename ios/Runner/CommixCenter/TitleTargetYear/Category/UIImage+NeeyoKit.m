...
+ (CGSize)displayUser:(CGSize)originSize
                              //: minSize:(CGSize)imageMinSize
                              cut:(CGSize)imageMinSize
                              //: maxSize:(CGSize)imageMaxSiz{
                              message:(CGSize)imageMaxSiz{
    //: CGSize size;
    CGSize size;
...
+ (UIImage *)mode:(NSString *)imageName {
    //: if (!imageName) {
    if (!imageName) {
        //: return nil;
        return nil;
    ...
    NSString *emojiPath = [[PersonShould sharedManager] getEmojiPath];
...
- (UIImage *)recordImageSize:(CGSize)targetSize
{
    // 裁剪两边
...
- (UIImage *)bluish: (CGFloat)suggestPixels
{
...
- (UIImage *)fast
{
    ...
    CGFloat pixels = [[MaxDevice currentDevice] suggestImagePixels];
...
- (UIImage *)program: (CGFloat)maxPixels
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
...
+ (UIImage *)betweenBar:(NSString *)imageName book:(NSString *)chartletId{
