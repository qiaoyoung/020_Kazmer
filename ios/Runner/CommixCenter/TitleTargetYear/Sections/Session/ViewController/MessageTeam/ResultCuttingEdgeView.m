...
NSString *StringFromRuralData(Byte *data);        


//: ic_praise%d
Byte kContent_degreeData[] = {49, 11, 93, 13, 229, 105, 150, 168, 62, 36, 64, 111, 224, 12, 6, 2, 19, 21, 4, 12, 22, 8, 200, 7, 172};

//: friend_circle_adapter_like
Byte kNameDateData[] = {48, 26, 68, 6, 180, 69, 34, 46, 37, 33, 42, 32, 27, 31, 37, 46, 31, 40, 33, 27, 29, 32, 29, 44, 48, 33, 46, 27, 40, 37, 39, 33, 47};

//: arrowUp
Byte kTitleShoppingString[] = {72, 7, 22, 4, 75, 92, 92, 89, 97, 63, 90, 150};
...
//: ic_fold
Byte kName_commitTankSheetData[] = {76, 7, 10, 9, 253, 249, 221, 42, 106, 95, 89, 85, 92, 101, 98, 90, 206};

//: arrowDown
Byte kTitle_squareName[] = {24, 9, 53, 11, 139, 170, 136, 91, 234, 175, 37, 44, 61, 61, 58, 66, 15, 58, 66, 57, 91};
...
- (void)lightReplacement{
    ...
    for (int i = 0; i < _buttonCount; i ++) {
        ...
        if(_isfold && i==7){
            ...
            [_foldbtn addTarget:self action:@selector(plicating:) forControlEvents:UIControlEventTouchUpInside];
            //: [_foldbtn setImage:[UIImage imageNamed:@"ic_fold"] forState:UIControlStateNormal];
            [_foldbtn setImage:[UIImage imageNamed:StringFromRuralData(kName_commitTankSheetData)] forState:UIControlStateNormal];
        ...
        [button addTarget:self action:@selector(beforeInstanceOf:) forControlEvents:UIControlEventTouchUpInside];
        //: [button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"ic_praise%d",i+1]] forState:UIControlStateNormal];
        [button setImage:[UIImage imageNamed:[NSString stringWithFormat:StringFromRuralData(kContent_degreeData),i+1]] forState:UIControlStateNormal];
...
- (void)panoramicView:(NSArray *)array
    ...
    UIView *buttonsBgView = [self prop:array];
...
//    }
    ...
    if (direction == DisplayBubbleMenuViewDirectionPriorityType_Up) {
        ...
        _arrowView.image = [UIImage imageNamed:StringFromRuralData(kTitle_squareName)];
    ...
    //: } else {
        ...
        _arrowView.image = [UIImage imageNamed:StringFromRuralData(kTitleShoppingString)];
...
- (UIView *)prop:(NSArray *)array {
    ...
    for (int i = 0; i < array.count; i ++) {
        ...
        label.textColor = [UIColor cell:StringFromRuralData(kTitle_houseData)];
...
- (void)beforeInstanceOf:(UIButton *)button {
...
- (void)plicating:(UIButton *)button {
    //: _isfold = NO;
    _isfold = NO;
...
Byte * RuralDataToCache(Byte *data) {
    int suspicionWith = data[0];
    int rigorous = data[1];
    Byte aircraftAssume = data[2];
    int sheet = data[3];
    if (!suspicionWith) return data + sheet;
    for (int i = sheet; i < sheet + rigorous; i++) {
        int value = data[i] + aircraftAssume;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[sheet + rigorous] = 0;
    return data + sheet;
}

NSString *StringFromRuralData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)RuralDataToCache(data)];
}
