 ...
 *************************简要说明************************
 ...
 [ControlTag configMainWindow:self.window];
 
 
 ContentLabel 使用方法
 
 [ControlTag alert].cofing.XXXXX.XXXXX.LeeShow();
 
 Expanse 使用方法
 
 [ControlTag actionSheet].cofing.XXXXX.XXXXX.LeeShow();
...
+ (void)bar:(NSString *)identifier fundamentalCount:(BOOL)force sinceName:(void (^ _Nullable)(void))completionBlock;

//: + (nonnull LEEAlertConfig *)alert;
+ (nonnull TwelvemonthBaseConfig *)corner;
...
+ (void)dateNext:(void (^ _Nullable)(void))completionBlock;
...
+ (nonnull UserBaseConfig *)showVertical;
...
@interface LaunchWithModelPraise: NSObject
...
/** 设置 添加动作 -> 格式: .LeeAddAction(^(WithAction *){ //code.. }) */
...
/** 设置 添加自定义视图 -> 格式: .LeeAddCustomView(^(AnglicismMessage *){ //code.. }) */
//: @property (nonatomic, copy, readonly) LEEConfigToCustomView LeeAddCustomView;
@property (nonatomic, copy, readonly) LEEConfigToCustomView LeeAddCustomView;

/** 设置 添加一项 -> 格式: .LeeAddItem(^(EnableStandard *){ //code.. }) */
...
@interface LaunchWithModelPraise(Presentation)

/** 设置 层级 -> 格式: .LeePresentation([TargetPresentation windowLevel:1000] / [TargetPresentation viewController:xxxx]) */
...
@interface LaunchWithModelPraise(Queue)
...
@interface LaunchWithModelPraise(Layout)
...
@interface LaunchWithModelPraise(Animation)
...
@interface LaunchWithModelPraise(Callback)
...
@interface LaunchWithModelPraise(ContentLabel)
...
@interface LaunchWithModelPraise(Expanse)
...
@interface EnableStandard: NSObject
...
@interface AnglicismMessage: NSObject
...
@interface TargetPresentation: NSObject

//: + (LEEPresentation *)windowLevel: (UIWindowLevel)level;
+ (TargetPresentation *)fillIn: (UIWindowLevel)level;

//: + (LEEPresentation *)viewController: (UIViewController *)controller;
+ (TargetPresentation *)bar: (UIViewController *)controller;

//: @end
@end

//: @interface LEEBaseConfig: NSObject
@interface EyeTitle: NSObject

//: @property (nonatomic, strong, nonnull) LEEBaseConfigModel *config;
@property (nonatomic, strong, nonnull) LaunchWithModelPraise *config;

//: @end
@end

//: @interface LEEAlertConfig: LEEBaseConfig
@interface TwelvemonthBaseConfig: EyeTitle

//: @end
@end

//: @interface LEEActionSheetConfig: LEEBaseConfig
@interface UserBaseConfig: EyeTitle

//: @end
@end

//: @interface LEEAlertWindow: UIWindow @end
@interface AstatineView: UIWindow @end

//: @interface LEEBaseViewController: UIViewController @end
@interface InsideViewController: UIViewController @end

//: @interface LEEAlertViewController: LEEBaseViewController @end
@interface PromoteViewController: InsideViewController @end

//: @interface LEEActionSheetViewController: LEEBaseViewController @end
@interface IndexViewController: InsideViewController @end


//: @interface UIView (CornerRadii)
@interface UIView (MagnitudeRelation)
...
MagnitudeRelation areaRefreshZero(void);

//: CornerRadii CornerRadiiNull(void);
MagnitudeRelation blockFile(void);
